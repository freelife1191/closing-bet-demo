...
- 구성 2026-09-24 23:36 | 실행 2026-09-24 23:36:32 시작(수정 전), 23:37:33 시작(수정 뒤)
- 검증 기준 커밋: `708e757`(이 문서를 담은 첫 커밋). 대조는 수정 전 커밋 `8c59e59`. 사본은 각 커밋의 `git archive` 다
...
- 실행: 세션 스크래치의 `infra100qa_run.sh <사본> <Next 포트> <표식>`(agent-browser `--namespace infra100qa`, 사본 gunicorn 58101·Next dev 58102, 두 사본을 차례로). Turbopack 이 사본 밖을 가리키는 `node_modules` 심볼릭 링크를 거부해 `cp -Rc`(APFS 클론)로 사본 안에 두었다
- 반복(iteration): 1회
- 결과: 통과 (필수 3/3)
- 증거: 아래 「실제」 줄(스크립트 출력 원문), scratchpad `qa-infra100-{old,new}-{flask,next}.log`, 스크린샷 `infra100qa-old-s2.png`·`infra100qa-new-s1.png`·`infra100qa-new-s2.png`(열어 확인)·`infra100qa-old-s1.png`(세션 임시)
...
- 실제: 두 사본 모두 관리자 화면 확인(「전체 데이터 업데이트」 1, 「관리자 전용」 0, 카드의 「업데이트」 버튼 6). 수정 전: `POST http://localhost:58102/api/system/start-update (Fetch) 400`, 화면 「업데이트 중복」 0·「업데이트 오류」 0·「Already running」 0, 「확인」 버튼 없음(`no modal`), 콘솔 `Update failed for Daily Prices: {status: 400 … message: "Already running"}`. 수정 뒤: 같은 요청 `409`, 「업데이트 중복」 1·「이미 다른 업데이트가 진행 중」 1·「업데이트 오류」 0·「Already running」 0, 스크린샷에서 「업데이트 중복」 모달 확인. 통과
...
- 실제: 수정 전: 요청 `400`, 「업데이트 오류」 1·「Already running」 1·「업데이트 중복」 0, 스크린샷에서 「업데이트 오류」 모달 본문 「Already running」 확인(결함 재현). 수정 뒤: 요청 `409`, 「업데이트 중복」 1·「업데이트 오류」 0·「Already running」 0, 버튼 「전체 데이터 업데이트」 1·「업데이트 중...」 0, 스크린샷에서 「업데이트 중복」 모달 확인. 콘솔의 `Update All failed: {status: 409 …}` 는 종전부터 있는 `console.error` 줄이며 Next dev 오버레이 「1 Issue」 배지가 이것이다. 통과
...
- 실제: 두 사본 모두 상태 파일 SHA-256 앞 12자리 `365bba9637aa` 가 클릭 전후 같음(`isRunning` true·`startTime` 2026-09-24T23:40:00·`items` 그대로), `data/` 는 기동 때 생긴 `runtime_cache.db`·`scheduler_runtime_status.json`·`update_status.json`·`v2_screener_status.json`·`vcp_status.json` 뿐, Flask 로그의 `Background Update`·`Daily Prices`·`init_data`·`KRX 로그인` 줄 0. 통과

## 정리
- 실제: agent-browser 두 세션 닫음, 사본 gunicorn(58101)·Next dev(58102) 종료 뒤 58101·58102·3500·5501 리스너 0·사본 경로 프로세스 0. 사본 둘(`q100old`·`q100new`)과 더미 세션 토큰 파일을 리터럴 경로로 지우고 없음 확인. 원본 `data/update_status.json` 수정 시각(1777948277) 그대로, 저장소 루트 `node_modules` 없음. 원본 `.env` 는 읽지 않았고 더미 비밀은 사본 프로세스 환경 변수로만 줬다. 결과: 필수 3/3 통과
