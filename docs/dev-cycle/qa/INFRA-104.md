...
- 구성 2026-09-25 09:47 | 실행 2026-09-25 09:48:53(S-1 수정 전), 09:52:15(S-1 수정 뒤), 09:53:24~(S-2~S-4 수정 전), 09:53:51~(S-2~S-4 수정 뒤)
- 검증 기준 커밋: `80978fa0`(이 문서를 담은 첫 커밋). 대조는 수정 전 커밋 `3dc2558b`. 사본은 각 커밋의 `git archive` 다
...
- 반복(iteration): 1회(수정 전 사본의 브라우저 첫 시도 09:52:30 은 상태 파일을 실행 중으로 쓴 뒤 페이지를 다시 열어 마운트 조회가 버튼을 비활성화했고, S-3 클릭이 요청을 보내지 않아 무효. 페이지를 먼저 열고 상태를 쓰는 `[INFRA-102]` 순서로 고쳐 다시 실행했다. 수정 뒤 사본은 고친 스크립트로만 실행. 사본 `common_update_status_service.py` 의 `INFRA-104` 표시는 수정 전 0건·수정 뒤 1건)
- 결과: 통과 (필수 4/4)
- 증거: 아래 「실제」 줄(스크립트 출력 원문, scratchpad `qa-infra104-{old,new}-s1.txt`·`qa-infra104-{old,new}-run.txt`, 무효 시도 `qa-infra104-old-run-invalid1.txt`), 스크린샷 `infra104qa-new-s3.png`(열어 확인: 「업데이트 중복」 모달)과 `infra104qa-{old,new}-s{2,3}.png`(세션 임시), 모든 실행 exit 0
...
- 실제: 수정 뒤(09:52:15) 첫 시작 `True`·`LOCAL_PIPELINE_ACTIVE=True`, 중단 뒤 두 번째 시작 `False` 이고 상태는 중단된 첫 실행 그대로(`isRunning:false`·`stopRequested:true`·첫 `startTime`). 옛 스레드는 첫 실행의 시각을 묶었고(`LOCAL_RUN_START_TIME==first True`) 끝난 뒤 플래그 `False`, 세 번째 시작 `True`, 마지막 상태 `isRunning:false`. 수정 전(09:48:53) 두 번째 시작 `True` 로 새 `startTime` 이 기록되고 옛 스레드가 그 시각을 묶어(`==first False`) 끝나며 두 번째 실행을 `isRunning:false` 로 내림(결함 재현). 통과
...
- 실제: 수정 뒤(09:53:57) 두 요청 모두 200, 각각 216ms·6ms 뒤 `isRunning:false`, 다시 연 화면의 진행 표시 0·오류 0. 수정 전(09:53:28)도 200·200, 217ms·6ms, 진행 표시 0. 플래그가 남지 않음. 통과
...
- 실제: 두 사본 모두 「업데이트 중복」 1·「업데이트 오류」 0, `POST http://localhost:58102/api/system/start-update (Fetch) 409` 한 건, 상태 파일 해시 클릭 전후 `f7aa5d1856b1` 로 같음. 화면 왼쪽 아래 Next dev 오버레이 「1 Issue」 는 409 뒤 콘솔 오류 표시로 종전과 같음. 통과
...
- 실제: 두 사본 모두 Flask 로그의 `Daily Prices`·`init_data`·KRX 로그인·`Background Update Failed` 줄 0, 사본 `data/` 에는 서버 기동이 만든 `runtime_cache.db`·`scheduler_runtime_status.json`·`v2_screener_status.json`·`vcp_status.json` 과 상태 파일 `update_status.json`·`update_status.json.lock` 만 있음. 원본 `data/` 수정 시각 불변(아래 정리). 통과

## 정리
- 실제: agent-browser 세션 둘 닫음, 사본 Next·gunicorn 정지 뒤 58101·58102 수신 0, 사본 경로 프로세스 0, 사본 둘(`q104old`·`q104new`) 리터럴 경로로 삭제 뒤 없음 확인. 원본 `data/kr_ai_analysis.json`(1788316016)·`signals_log.csv`(1789994506)·`update_status.json`(1777948277) 수정 시각 그대로, 저장소 루트 `node_modules` 없음. 원본 3500·5501·`.env`·운영 주소 사용 없음. 결과: 필수 4/4 통과
