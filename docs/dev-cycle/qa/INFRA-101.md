...
- 구성 2026-09-25 06:34 | 실행 2026-09-25 06:39:00~06:39:19
- 검증 기준 커밋: `61c30c0`(이 문서를 담은 첫 커밋). 대조는 수정 전 커밋 `086a807`. 사본은 각 커밋의 `git archive` 다
...
- 반복(iteration): 1회
- 결과: 통과 (필수 3/3)
- 증거: 아래 「실제」 줄(실행기 출력 원문의 요약). 사본의 `app/routes/common.py` 에서 `run_start_time` 은 수정 전 0건·수정 뒤 2건으로, 두 사본이 서로 다른 커밋임을 확인했다
- 무효 실행(기록): 06:38:39 의 첫 시도는 zsh 에서 `set -- $pair` 가 나뉘지 않아 사본이 비었고, 실행기가 `data/` 를 찾지 못해 하네스 기동 전에 끝났다. 호출 실수라 판정에서 제외했고 잘못 만든 디렉터리 둘은 비어 있음(`data/` 뿐)을 확인한 뒤 지웠다
...
- 실제: 06:39:00 실행. 수정 전: `final_startTime_is_mine` false(최종 상태는 B 의 새 실행 `06:39:01.838448`), `final_items` `Daily Prices: done`, `A_skipped_writes` 0(결함 재현). 수정 뒤: `final_startTime_is_mine` false(B 의 `06:39:06.083456`), `final_items` `Daily Prices: pending`, `A_skipped_writes` 1. 통과
...
- 실제: 06:39:09 실행, 약 10초 뒤 종료. `final_isRunning` false, `final_startTime_is_mine` true, `final_items` `Daily Prices: done`, `A_skipped_writes` 0, `stop_seen_after` null, exit 0·Traceback 없음. 통과
...
- 실제: 수정 뒤 A 가 1.03초에 중단을 봄(수정 전 1.04초), 최종 `isRunning` true·`startTime` 이 B 것, A `flag_final` false, B `flag_after_stop` false, 두 프로세스 exit 0·Traceback 없음. 통과

## 정리
- 실제: 사본 둘(`q101old`·`q101new`)을 리터럴 경로로 지우고 없음 확인, 하네스 프로세스 0. 서버를 띄우지 않았고 3500·5501 리스너 0. 원본 `data/update_status.json` 수정 시각(1777948277) 실행 전후 그대로. 소켓 차단으로 네트워크 없음, KRX 로그인·LLM 호출 없음. 결과: 필수 3/3 통과
