...
- 구성 2026-09-25(설계 승인 14:35 뒤, 실행 전, 분 단위 시각은 기록하지 않음) | 실행 2026-09-25 14:41:53~14:42:15
- 검증 기준 커밋: `7e3a8b98`(이 문서를 담은 첫 커밋). 대조는 수정 전 커밋 `025b51c5`. 사본은 각 커밋의 `git archive` 다
...
- 반복(iteration): 1회
- 결과: 통과 (필수 2/2)
- 증거: 아래 「실제」 줄(하네스 `RESULT` 줄 원문, scratchpad `qa-infra118-old.txt`·`qa-infra118-new.txt`), 두 실행 모두 exit 0, stderr 소켓 차단 예외 0건
...
- 실제: 수정 뒤 체인이 도는 동안 True, 두 번째 수동 409, 분석 시작 없음, 체인 True, 체인이 수동 끝 뒤에 시작, 끝난 뒤 False. 수정 전 체인이 도는 동안 False, 두 번째 수동 200, 분석 시작(결함 재현), 체인 True, 끝난 뒤 False. 통과
...
- 실제: 수정 전후 모두 200, 끝난 뒤 False, 다시 200, 분석 시작 2회, 최종 False. 통과

## 정리
- 실제: 두 사본(`q118old`·`q118new`)을 리터럴 경로로 삭제(남은 사본 0), 하네스 프로세스 0. 원본 `data/` 에서 QA 직전(14:41:45) 뒤 수정된 파일 0개(`find data -newermt`, 14:42:23 확인). 서버·포트·브라우저는 쓰지 않았다. 원본 `data/`·`.env` 사용 없음. 결과: 필수 2/2 통과
