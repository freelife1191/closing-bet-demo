...
- 구성 2026-09-05 15:42 | 실행 2026-09-05 15:50
...
- 기대: HTTP 200. 콘솔 오류 **0건**. 버튼 **25개**, 포커스 가능 요소 **28개**,
  ...
  나온다.
- 결과: 통과. 콘솔 오류 0건, 버튼 25개, 포커스 가능 요소 28개, `h1` 1개,
  `alt` 없는 이미지 0개. 인사말 두 줄과 「저장된 대화가 없습니다.」 모두 기대와 같았다.
...
- 기대: 캐시 **히트**. `stock_map` 과 `ticker_map` 이 각각 **1,997건**이고,
  ...
  히트가 정상이다.
- 결과: 통과. 캐시 히트로 `stock_map` 과 `ticker_map` 이 각각 1,997건 읽혔고
  `삼성전자 → 005930`, `005930 → 삼성전자` 였다. 현재 CSV 서명도 저장된 행의 서명과
  같은 `(1770785642634316972, 58248)` 이었다.
...
- 기대: 세 건 모두 **히트**. 반환 문자열의 길이가 차례로 **270**, **283**, **15** 다.
  ...
  이다.
- 결과: 통과. 세 건 모두 히트했고 길이가 270 / 283 / 15 로 기대와 같았다. 첫 건은
  `- 2026-05-04: 종가 232,500` 으로, 셋째 건은 `과거 VCP 포착 이력 없음` 으로 시작했다.
...
- 결과: 통과. 두 API 모두 HTTP 200 이고, `welcome` 은 「안녕하세요! **스마트머니봇**
  입니다」로 시작했다. `suggestions` 는 5건이고 모든 항목이 `desc`·`icon`·`prompt`·`title`
  네 필드를 갖고 있었다.
...
- 결과: 통과. 드롭다운을 열면 항목 3개가 `gemini-3.5-flash-lite`,
  `gemini-3.7-flash`, `gemini-3.6-flash` 순서로 나오고 `gemini-3.7-flash` 에만 선택
  아이콘이 붙어 있었다. 닫힌 상태에서는 여섯 항목이 모두 `visibility: hidden` 이다.
  DOM 에 세 항목이 두 벌 있는 것은 데스크톱과 모바일 분기 때문이며 보이는 것은 늘 한 벌이다.
...
- 결과: 통과. 링크 7개가 모두 기대한 주소를 가리켰고 각각 HTTP 200 을 돌려주었다.
...
- 기대: 추천 질문 카드 **4개**로 「마켓게이트 상태와 투자 전략」, 「AI 분석 기반 매수
  ...
  「내 관심종목」이다.
- 결과: 통과. 추천 질문 카드 4개와 빠른 조회 버튼 5개의 이름이 모두 기대와 같았다.
  누르지 않고 읽기만 했다.
...
- 통과 8 / 전체 8
- 실패한 시나리오: 없음
- 시나리오 밖에서 새로 발견: 한 건 있으나 고치지 않았습니다. `/chatbot` 을 연 직후
  하이드레이션이 끝나기 전 약 3초 동안 인사말이 「안녕하세요, 님」으로 보입니다. 사용자
  이름이 빠진 상태이고, 같은 순간 `h1` 이 두 개이며 DOM 의 버튼 수가 43개로 부풀어
  있습니다. 3초 뒤에는 「안녕하세요, 흑기사님」과 `h1` 하나, 버튼 25개로 정착합니다.
  이번 항목의 범위 밖이고 백엔드 캐시와도 무관하므로 고치지 않았습니다. `[FE-008]`
  「대시보드 진입 시의 하이드레이션 불일치 제거」가 같은 성격의 결함을 다루고 있어,
  그 사이클에서 챗봇 화면도 함께 보면 됩니다.

`/qa` 가 고친 것은 없습니다. 여덟 시나리오가 모두 처음 실행에서 통과했으므로 수정 커밋도
회귀 테스트도 생기지 않았습니다.
