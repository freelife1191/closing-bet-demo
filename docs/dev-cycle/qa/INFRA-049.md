...
### S-5b. export 제거가 어느 키의 실효값도 바꾸지 않는다 (리포트)
- 조작: `.env` 의 모든 키를 bash 의 `set -a; source` 와 python-dotenv 로 각각 읽어 값의
  길이를 비교한다. 값 자체는 비교 프로세스로 넘기지 않고 길이만 센다.
- 기대: 갈리는 키가 없다.
- 필수 여부(required): 예
- 실제: 57개 키 전부 길이가 같았다. 갈리는 키 0개.
- 근거: `load_dotenv()` 는 `override=False` 라 종전에는 셸이 넣어 준 bash 파싱 결과가
  이겼고, export 를 없앤 지금은 python-dotenv 파싱 결과가 이긴다. **두 파서가 갈리는 값이
  하나라도 있으면 그 키의 실효값이 조용히 바뀐다.** 세 키는 S-5 가 묶고 있지만 나머지
  55개는 묶여 있지 않아 `oh-my-claudecode:security-reviewer` 가 확신도 중간으로 남긴
  관찰이었다. 실제로 재어 「영향 없음」으로 확정했다.
- 결과: 통과
- 증거: 위 비교 스크립트의 출력 「비교한 키: 57개 / 갈리는 키: 없음」
- 정리(cleanup): 읽기만 했으므로 없음
