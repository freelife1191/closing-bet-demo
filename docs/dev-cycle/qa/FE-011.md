...
| S-2 | 경합·정체 | npm test가 전체 검사를 수행하고 종료 코드 0으로 종료한다. test:watch --help로 별도 감시 진입 명령이 해석되는지 확인한다. | 명시한 종료/중간상태 계약 유지 | 통과: stop/stream 각10회; watch help0 | 없음 | ../evidence/batch-2026-09-09/qa-repeat-results.json 및 압축로그 | 완료 | 예 |
