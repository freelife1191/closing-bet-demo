...
- 구성 2026-09-24 22:52:06 | 1차 실행 2026-09-24 22:56:29~22:57:05(`3a87c29`, S-3 실패) | 2차 실행 2026-09-24 23:06:41~23:07:22(`e7cfc0a`)
- 검증 기준 커밋: 2차 `e7cfc0a`(QA 수정 커밋). 1차 `3a87c29`(이 문서를 담은 첫 커밋). 대조는 수정 전 커밋 `d30ced0`. 사본은 각 커밋의 `git archive` 다
...
- 실제(2차, `e7cfc0a`): 23:06 실행, 두 사본 모두 exit 0·Traceback 0. 수정 전 A `stop_seen_after` null(10초), 수정 뒤 1.02초, 최종 플래그 false, `isRunning` false. 통과
...
- 실제(2차, `e7cfc0a`): 수정 전 B `flag_after_stop` true·`stopRequested` null, 수정 뒤 false·true. 통과
...
- 실제(2차, `e7cfc0a`): 수정 전 A null(10초), 최종 `isRunning` false(새 실행 상태를 끝냄). 수정 뒤 A 가 1.02초에 중단을 봄(대체 판정), 최종 `isRunning` true·`startTime` 이 B 의 새 실행 것, A 최종 플래그 false. 통과(1차 실패는 Task 2 로 수정)
...
- 실제: 수정 전 A 가 0.06초에 중단을 봄(같은 프로세스 중단이 플래그를 곧바로 켬). 수정 뒤 A 는 null(10초를 채움)로 새 실행과 함께 돌았고, 끝날 때 새 실행의 항목 상태를 `done` 으로 덮음. 알려진 한계 `[INFRA-099]` 의 실측이며 수정 전보다 이 경우가 나빠졌음을 확인. 기록용

## 정리
- 실제: 사본 둘(`q097old`·`q097new`, `q097new` 는 2차 전에 `e7cfc0a` 로 다시 만듦)을 리터럴 경로로 지우고 없음 확인. 1차 전 시험 실행 한 번(수정 전 사본, S-1)은 기대대로여서 정식 실행과 같은 결과. 서버를 띄우지 않았고 3500·5501 리스너 0. 원본 `data/update_status.json` 수정 시각(1777948277) 그대로. 소켓 차단으로 네트워크 없음, KRX 로그인·LLM 호출 없음, 모든 실행 Traceback 0. 결과: 필수 3/3 통과(S-3 은 1회 실패 뒤 수정), 기록용 S-4 는 한계 실측
