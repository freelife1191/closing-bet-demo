...
- engine: ultraqa | lifecycle: app-adapted | phase: qa | iteration: 1 | same_failure_count: 0
...
| S-3 | 사용자 실행·실패복구 | 실제 종가UI 실행/확인모달/상태조회, 합성서버 | 정상결과표시, 실패도 성공으로 만들지 않음 | 실패: 정상200 뒤버튼잠김, TypeError500은성공으로표시안함 | 추가범위 사용자선택대기 | ../evidence/batch-2026-09-09/jongga-ui-baseline-failure.json | 서버는VCP검사에재사용중 | 예 |
