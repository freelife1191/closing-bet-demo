...
- 검증 기준 커밋: 새 코드는 이 문서를 담은 첫 커밋 `a709984b`, 대조용 수정 전 코드는 `14ede4f2`(첫 커밋의 부모). 사본은 각 커밋의 `git archive` 다
- 구성 2026-09-25(첫 커밋 `a709984b` 뒤) | 실행 2026-09-25 23:09:52(서버 기동)~23:12:30(마지막 CLI 하네스), 정리 확인 23:13:07
...
- 결과: 통과 (필수 8/8)
- 증거: 각 시나리오의 「실제」 줄. 모두 scratchpad `flow025/qa/` 아래에 있다. API 응답 `qa-api.txt`(23:10:28, 원문 `raw-<포트>-<종목>.json`), 모달 글자·콘솔·네트워크·MCP·로그 집계
  `qa-browser.txt`(스크립트 `browse_run.sh`), 스크린샷 `flow025-QA<이름>-modal.png` 다섯 장(QA우선·QA창빈날·QA네이버는 열어 확인), CLI 출력 `qa-cli.txt`,
  격리 로그 `qa-new-flask.log`·`qa-old-flask.log`·`qa-new-next.log`. 로그의 `QA-PYKRX`·`QA-TOSS`·`QA-NAVER` 줄과 CLI 의 `QA-CODE` 줄로 각 사본이 대역과 자기 코드를 거쳤음을 확인했다
- 정리(cleanup): browse 서버 정지(「Server stopped」), 격리 gunicorn(5755·5756)·Next(3755) 종료 뒤 23:12:50 에 3755/5755/5756/3500/5501 리스너 0, 사본 경로 프로세스 0,
  사본 `flow025/qa/new/`·`flow025/qa/old/` 를 리터럴 절대 경로로 삭제, 원본 `data/` 에서 기동 전 표식 이후 바뀐 파일 0, 저장소 루트 `node_modules/.vite` 없음,
  `git status` 는 깨끗했다(이 기록 전). 원본 3500/5501 은 처음부터 떠 있지 않았다
...
- 필수 여부(required): 예
- 실제: 새 코드 API `investorTrend5Day` `{foreign: 500000000, institution: 1000000000}`, 모달 「외국인+5억」「기관+10억」(스크린샷 `flow025-QA창빈날-modal.png`,
  제목 옆 코드 900252). 새 코드 로그 `QA-PYKRX trading_value 900252` 1줄. 수정 전 코드 API `{foreign: 1800000000, institution: 1200000000}`, 로그 0줄
- 결과: 통과
...
- 필수 여부(required): 예
- 실제: 새 코드 API `{foreign: 1500000000, institution: 1000000000}`, 모달 「외국인+15억」「기관+10억」. 수정 전 코드 API `{foreign: 2100000000, institution: 1400000000}`.
  두 사본 모두 `QA-PYKRX trading_value 900251` 0줄
- 결과: 통과
...
- 필수 여부(required): 예
- 실제: 새 코드 `000680` 은 `code` 000680·`name` QA보통·`{2000000000, 500000000}`, 모달 「QA보통」「외국인+20억」「기관+5억」. `00680K` 는 `code` 00680K·`name` QA우선·
  `{-500000000, -1000000000}`, 모달 「QA우선」「외국인-5억」「기관-10억」(스크린샷 `flow025-QA우선-modal.png` 에서 제목 옆 코드 00680K, 카드 목록의 QA우선 카드도 00680K).
  새 코드 로그에 `QA-TOSS trading-trend 00680K` 1줄로 자기 코드로 조회했다. 수정 전 코드 `00680K` 는 `code` 000680·`name` QA보통이고, 두 요청의 `investorTrend5Day` 가
  모두 `{500000000, -400000000}`(두 종목의 행 다섯 개가 섞인 합)이며 `QA-TOSS trading-trend 00680K` 는 0줄이다
- 결과: 통과
...
- 필수 여부(required): 예
- 실제: 새 코드 API `investorTrend` `foreign` 0·`institution` 0·`foreignDays` None, `investorTrend5Day` 없음. 모달 「외국인-」「기관-」「개인자료없음」(스크린샷
  `flow025-QA네이버-modal.png`). 새 코드 로그 순서는 `QA-TOSS trading-trend 900253`(Toss 상세) → `QA-NAVER main 900253` → `QA-PYKRX trading_value 900253`(참조, 버림)
  → `QA-TOSS trading-trend 900253`(참조)다. 기대 문구의 「Toss 1줄」은 상세 조회가 종목 정보 없이도 투자자 동향을 한 번 부르는 것을 빠뜨린 것으로, 실제는
  새 코드 2줄(상세 1·참조 1), 수정 전 코드 1줄(상세)이다. 이 시나리오가 가르려던 「pykrx 를 버린 뒤 참조로 넘어가고 부분합을 쓰지 않는다」는 충족되어 통과로 판정한다.
  수정 전 코드 API `foreign` 1200000000·`institution` 500000000 이고, 수정 전 사본 `data/.krx_collector_cache/pykrx_supply_5d` 에 `900253__20260925`
  `{foreign_buy_5d: 1200000000, inst_buy_5d: 500000000}` 요약 캐시가 남았다. 새 코드 사본에는 그 캐시가 없다
- 결과: 통과
...
- 필수 여부(required): 예
- 실제: 모달 다섯 개를 연 뒤 browse `console --errors` 는 「(no console errors)」, `/_next/mcp` `get_errors` 는 `configErrors:[]`·`sessionErrors:[]`,
  `get_compilation_issues` 는 `issues:[]`(23:12:01). `browse network` 의 `stock-detail` 요청 10건(dev 모드에서 한 번 열 때 2건) 모두 200. 두 Flask 로그 error 0·
  traceback 0·`QA-BLOCKED` 0. `QA-PYKRX unexpected` 는 새 코드 3줄(Naver 펀더멘탈 `get_market_fundamental_by_ticker` 2, 페이지가 부른 `jongga-v2` 라우트의
  `get_nearest_business_day_in_a_week` 1), 수정 전 코드 2줄(펀더멘탈 2)이다. 브라우저는 새 코드만 열었으므로 `jongga-v2` 줄은 새 코드에만 있다
- 결과: 통과
...
- 필수 여부(required): 예
- 실제(새 코드 → 수정 전 코드, `qa-cli.txt` 23:12:25·23:12:28):
  - 900261: None, 호출 1 → `[400000000, 400000000]`, 호출 1
  - 900262: None, 호출 1 → `[800000000, 500000000]`, 호출 1
  - 900263: None, 호출 1 → `[0, 0]`, 호출 1
  - 900264: `[1000000000, 1500000000]`, 호출 1 → 같음
  - 900265: `[2500000000, 500000000]`, 호출 1 → 같음
  - 900266: None, 호출 1 → `[500000000, 500000000]`, 호출 1
  - 900264 기준일 없음: `[1000000000, 1500000000]`, 호출 0 → 같음
  - 새 코드 사본 `data/` 에 `pykrx_supply_5d` 없음. 수정 전 사본에는 900261·900262·900263·900265 의 요약 캐시 4행(`[4억, 4억]`·`[8억, 5억]`·`[0, 0]`·`[25억, 5억]`)과
    S-4 의 900253 행이 남았다. 새 코드 로그에 「수급 CSV 최근 5거래일 2026-09-17~2026-09-23: 5일 값 5종목, 빠진 날·빈 칸으로 제외 2종목」
  - 관찰: 새 코드 사본의 pykrx 참조 SQLite 캐시에 S-4 에서 버린 900253 참조(외국인 하루 null)가 기준일 키 `900253__20260925` 로 남았다. `/review` M2 의 동작이며 `[FLOW-029]` 에 기록했다
- 결과: 통과
...
- 필수 여부(required): 예
- 실제: 새 코드 결과 000680 `2000000000·500000000`·90, 900251 `1500000000·1000000000`·100, 900264 `1000000000·1500000000`·85, 900265 `500000000·500000000`·60,
  로그 「최근 5거래일에 빠진 날이 있어 점수에서 제외: 2개 종목」. 수정 전 코드 결과 900251 `2100000000·1400000000`·100, 900252 `1800000000·1200000000`·100,
  900264·85, 900265·60, 900266 `500000000·500000000`·60 이고 000680 이 없다
- 결과: 통과
...
- 필수 여부(required): 예
- 실제: 새 코드 `00680K`→`00680K`, `45226K`→`45226K`, `0220WL`→`0220WL`. 수정 전 코드 `000680`·`045226`·`000220`. `0007C0`→`0007C0`, `005930`·`A005930`·
  `005930.KS`·`5930`→`005930`, `20260211`→빈 문자열은 두 코드가 같다
- 결과: 통과
...
- 필수 시나리오: 통과 8 / 전체 8. 수정 전 코드는 S-1~S-4·H-1~H-3 에서 표의 수정 전 기대값을 냈고(H-1 의 900264·900265 와 기준일 없음은 같음), 시나리오가 수정 전후를 가른다
- 미통과 필수: 없음
- 재개 판정: 완료 가능
- 시나리오 밖에서 새로 발견: 없음(S-4 의 Toss 줄 수 차이는 기대 문구의 누락, 새 코드 참조 캐시에 남은 거부 참조는 이미 `[FLOW-029]` 에 기록한 동작이다)
