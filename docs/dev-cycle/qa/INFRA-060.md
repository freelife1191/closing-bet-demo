...
- engine: ultraqa | lifecycle: app-adapted | phase: precommit-verified | iteration: 1 | same_failure_count: 0
...
|---|---|---|---|---|---|---|---|---|---|
| Q1 | 로그인 A/B 계정 | 임시 DB·서명 신원 | 서비스/Flask pytest | 각 1억, A/B 모든 조회·거래·입금·reset 격리 | 대기 | owner SQL/route | 테스트 결과 | tmp DB 삭제 | 예 |
| Q2 | 익명·공격자 | 위조/만료/일반 신원 헤더·owner body/query | HTTP 경계 pytest | 8 routes 401·서비스 미접근, 유효서명은 자기 owner만 | 33개 경계 검사 PASS, 통합 대기 | 인증 decorator | RED/GREEN 기록 | app 미기동 | 예 |
| Q3 | 기존 사용자 자료 | 구형 DB·기존 price_cache | migration pytest | legacy 내용 보존·접근 차단·active 가격캐시 비움·반복 안전 | 대기 | atomic migration | 테스트 결과 | tmp DB 삭제 | 예 |
| Q4 | 동시 사용자/중단 | 여러 연결·동시 초기화/매수·실패 주입 | concurrency/rollback pytest | 자금 중복 지급·음수잔고·다른owner변경·부분 migration 없음 | 대기 | transaction/조건부 SQL | 테스트 결과 | 연결 종료 | 예 |
| Q5 | 가격 오염 시도 | 같은 ticker, A의 입력가격, mock provider | cache/valuation pytest | B는 공급자 시세/자기 원가만 사용, reset은 공유cache 보존 | 대기 | 거래의 공유캐시 쓰기 제거 | 테스트 결과 | 외부호출 없음 | 예 |
| Q6 | 로그인 전환/늦은응답 | 익명→A→B, 지연응답·401 | vitest·격리 UI | 로그인 안내, A 자료/응답이 B 화면에 재등장하지 않음 | 대기 | UX/session 경계 | 테스트/UI 결과 | 소유서버 정리 | 예 |
| Q7 | 회귀·불변성 | 독립 clone, 원본 파일 hash | 전체 pytest/vitest/typecheck·diff | 기존 계약 및 원본 data/env/package 보존 | 대기 | 필요한 contract 갱신 | 최종 기록 | clone 정리 | 예 |
...
개발·검증 중. 원본 DB migration/서비스 재기동/배포는 수행하지 않는다. 임시 자원은 마감 시 정리한다.

## Residual risks

정적 검증 완료, 첫 구현 커밋과 최종 동적 QA·정리 판정 전이므로 완료 아님. 신원 서명의 경로/메서드 재생 방어는 기존 INFRA-062 범위이며 이번 변경에서 서명 형식을 바꾸지 않는다.
...
- 전체 pytest: 네트워크 outbound를 sandbox-exec로 차단하고 스케줄러를 끈 독립 clone에서 `python -m pytest -q` → exit 0, **1914 passed, 3 skipped, 26.70s**. 3 skip은 기존 수동 Gemini/project-showcase 제외 정책이며 이번 owner 시나리오는 skip이 없다.
