...
### S-12. 배포 설정에 비밀이 들어 있다 (설정 오류)
- 조작: `.env` 와 `.env.production` 양쪽에 `INTERNAL_IDENTITY_SECRET` 이 값과 함께 있는지 본다.
- 기대: 둘 다 있고 서로 다른 값이다. 없으면 그 환경의 로그인 사용자 전원이 익명이 된다.
- 필수 여부(required): 예
- 실제: 보안 리뷰가 `.env.production` 에 이 키가 없음을 찾아냈다. 실패 방향은 안전하지만
  예외도 경고 로그도 없이 「로그인했는데 대화 목록이 비어 있다」로만 나타난다. 사용자 승인을
  받아 두 파일에 각각 다른 값을 넣었다. 값이 있는 키가 `.env` 는 56 → 57,
  `.env.production` 은 55 → 56 으로 하나씩 늘었고 기존 키 목록은 그대로다.
- 결과: **통과**
- 증거: 키 목록 `diff` 가 `INTERNAL_IDENTITY_SECRET=` 한 줄만 추가된 것으로 나오고,
  두 값의 해시 앞 8자가 서로 다르다. 값 자체는 어디에도 출력하지 않았다.
- 정리(cleanup): 두 파일의 변경 전 사본을 스크래치패드에 두었다

**저장소에서 `.env.production` 을 읽는 배포 절차를 찾지 못했다.** 셸 스크립트·`Procfile`·
CI 설정 어디에도 참조가 없다. 그래서 이 파일이 실제 배포에 쓰이는지는 확인하지 못했고,
쓰이지 않는다면 운영 환경의 `.env` 에 같은 키를 따로 넣어야 한다.
...
- 필수 시나리오: 통과 12 / 전체 12 (S-9 는 대체 수단)
- 미통과 필수: 없음
- 재개 판정: 완료 가능
- 시나리오 밖에서 새로 발견: 보안 리뷰가 다섯 가지를 찾았다. 셋은 이번에 고쳤고
  (익명 ID 판정 좁히기, `verify_identity_header` 의 `@` 대칭, `.env.production` 의 비밀),
  둘은 이월했다(`[INFRA-039]` 바인딩 주소, `[INFRA-040]` CSRF). 그 밖에 `[INFRA-041]` 도
  함께 이월했다. 이번 사이클에서 더한 검사는 파이썬 3건과 `proxy.ts` 7건이다.
- 차단 등급: 없음 (보안 리뷰 판정)
