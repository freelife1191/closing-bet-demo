...
- **서명이 요청에 묶이지 않은 120초 bearer 다.** 경로·메서드·nonce 어디에도 묶여 있지 않아 한 번
  새면 그동안 아무 엔드포인트에나 쓸 수 있다. proxy 와 Flask 사이가 loopback 이고 헤더를 로그에
  남기지 않으므로 이 규모에서는 받아들였다. nonce 저장소는 과잉이다.
