...
- 구성 2026-09-24 22:41:35 | 실행 2026-09-24 22:42:11~22:42:18
- 검증 기준 커밋: `17d216a` (이 문서를 담은 첫 커밋). 대조는 수정 전 커밋 `55799f7`. 사본은 각 커밋의 `git archive` 다
...
- 실제: 22:42 실행, 두 사본 모두 exit 0·Traceback 0. 수정 전(`55799f7`)은 가격·VCP·종가베팅·알림 단계가 모두 `true` 를 봄, 수급 로그 「사용자 요청으로 수급 데이터 수집 중단」「중단되어 수급 Toss 백필을 건너뜁니다」, pykrx 호출 0, 수급 파일 없음, 최종 플래그 `true`, 경고 로그 없음. 수정 뒤(`17d216a`)는 네 단계 모두 `false`, 경고 로그 「[Scheduler] 남아 있던 사용자 중단 요청을 해제하고 정기 분석을 시작합니다.」 1회, pykrx 호출 22일, 수급 44행(08-24~09-22, 005930 `10/20`·000660 `20/40`), 최종 플래그 `false`. 두 사본 모두 상태 `isRunning` `false`. 통과

### S-2. 중단 요청이 없으면 17:00 체인 결과가 수정 전과 같다 (하네스, 필수, 회귀 확인)
- 조작: `plain` 모드
- 기대: 두 사본의 JSON 출력이 `cmp` 로 같다. 경고 로그 없음
- 실제: 두 사본 모두 exit 0·Traceback 0. 두 JSON 출력이 `cmp` 로 같음(`PLAIN_IDENTICAL`). 네 단계 모두 `false`, pykrx 22일, 수급 44행, 경고 로그 없음. 통과
...
- 실제: 두 사본 모두 exit 0·Traceback 0, 파이프라인 중 pykrx 호출 0(`[]`), 상태 `isRunning` `false`. 수정 전은 파이프라인 뒤 플래그 `true`, 개별 실행 `False`(수급 로그 「수집 중단」「Toss 백필을 건너뜁니다」), pykrx 0, 수급 파일 없음. 수정 뒤는 파이프라인 뒤 플래그 `false`, 개별 실행 `True`, pykrx 22일, 수급 44행. 통과

## 정리
- 실제: 사본 넷(시험 실행 `d096old`·`d096new`, 정식 `q096a`·`q096b`)을 리터럴 경로로 지우고 없음 확인. 시험 실행 첫 두 번은 하네스 경로 문제(`PYTHONPATH` 누락)와 zsh 가 변수 경로의 glob 한정자를 해석하지 않아 사본 `data/` 가 비워지지 않은 문제로 다시 돌렸고, 하네스가 사본 경로에서만 돌며 사본 `data/` 를 직접 비우도록 고친 뒤 정식 실행함. 서버를 띄우지 않았고 3500·5501 리스너 0. 원본 `data/all_institutional_trend_data.csv` 수정 시각(1789994453) 그대로. 소켓 차단으로 네트워크 없음, KRX 로그인·LLM 호출 없음, 여섯 실행 로그 Traceback 0. 결과: 필수 3/3 통과
