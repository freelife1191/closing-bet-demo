...
- 구성 2026-09-24 11:47 | 실행 2026-09-24 11:47
- 검증 기준 커밋: `31bff52` (첫 커밋). 사본은 이 커밋의 `git archive` 이며 실행 중 범위 파일은 바뀌지 않았다
...
- 반복(iteration): 1회
- 결과: 통과 (필수 3/3)
- 증거: 아래 각 시나리오의 「실제」 줄(하네스 출력 원문), scratchpad `chat046qa_harness.py`·`chat046qa-after.log`·`chat046qa-before.log`. 두 사본 모두 exit 0
- 정리(cleanup): 사본 둘(`31bff52`·`f2eb1c8`)과 임시 `data` 삭제 뒤 없음 확인. 서버·브라우저는 띄우지 않았다. 원본 `data/kr_ai_analysis.json` 수정 시각(09-02 11:26) 그대로
...
- 실제: 머리글 「분석 5건 (기준일 2026-09-24, 0일 경과), 매수 추천 4건」. `- **KT**: 65점 (GPT 매수)`, `- **한화3우B**: 72점 (Gemini 관망 · GPT 매수)` 와 사유 「GPT-HANWHA …」, `- **카카오** (GPT 매수)` 가 있다. 수정 전 사본은 「매수 추천 1건」이고 SK하이닉스만 「(매수 추천)」으로 나와 KT·한화3우B·카카오가 빠졌다. 통과

### S-2. Perplexity 판정은 쓰지 않는다
- 조작: S-1 과 같은 실행
- 기대: 삼성전자가 목록에 없고 「PPLX-SAMSUNG」 문자열이 출력 어디에도 없다. 수정 전 사본에서는 Gemini HOLD 가 있어 역시 빠진다(대조로만 기록)
- 실제: 삼성전자가 목록에 없고 수정 뒤 로그에서 「PPLX」 검색 결과 0건. 수정 전 사본도 삼성전자가 없다(Gemini HOLD 가 먼저 선택됨). 통과
...
- 실제: `- **SK하이닉스**: 90점 (Gemini 매수 · GPT 매수)` 와 사유 「GEM-HYNIX …」. `build_vcp_intent_context` 는 「[VCP AI 분석 결과]」 뒤에 같은 요약과 지시문(「VCP 데이터에 근거해 …」)을 담는다. 통과
