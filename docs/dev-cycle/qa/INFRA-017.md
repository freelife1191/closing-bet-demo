...
|---|---|---|---|---|---|---|---|---|
| N1 | 사용자/공격자: 실제 Next 대시보드 설정 모달 | 격리 factory/Next + agent-browser 또는 명시된 pytest | 기존 정상 조회200/관리자 설정 표시 | 미실행 | 없음 | 실행 후 연결 | pending | 필수 |
| A1 | 사용자/공격자: 없는 API 경로·인코딩 경로·긴 경로 GET | 격리 factory/Next + agent-browser 또는 명시된 pytest | 404, critical 로그 없음 | 미실행 | 없음 | 실행 후 연결 | pending | 필수 |
| A2 | 사용자/공격자: 등록 GET 전용 API에 POST, env에 DELETE | 격리 factory/Next + agent-browser 또는 명시된 pytest | 405 및 Allow 유지, critical 로그 없음 | 미실행 | 없음 | 실행 후 연결 | pending | 필수 |
| A3 | 사용자/공격자: 실제 factory HTTPException 400/429와 일반 RuntimeError | 격리 factory/Next + agent-browser 또는 명시된 pytest | 400/429+Retry-After, 일반 오류만500 | 미실행 | 없음 | 실행 후 연결 | pending | 필수 |
