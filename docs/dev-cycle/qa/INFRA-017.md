...
- updated: 2026-09-09T11:25:48.705902
...
|---|---|---|---|---|---|---|---|---|
| N1 | 실제 Next 설정 모달 진입 | admin settings; GET /api/system/env | 200·알림 센터·마스킹 | PASS: 200 확인, 정상 스크린샷 열어 대조 | 아래 기록 | settings-normal.png; requests.jsonl.gz | 런타임 완료 | 필수 |
| A1 | 공격자: 없는·Unicode·긴 API 경로 | agent-browser eval fetch 3종 + 실제 데이터 상태 화면404 | 404·critical 로그 없음 | PASS: 3종404, UI fetchAPI가 API Error:404 처리 | 아래 기록 | http-probes.json; data-missing.json; data-missing.png | 런타임 완료 | 필수 |
| A2 | 공격자: 잘못된 메서드 | POST /api/admin/check; DELETE /api/system%2Fenv | 405·Allow 유지 | PASS: 모두405, GET/POST Allow 확인 | 아래 기록 | http-probes.json | 런타임 완료 | 필수 |
| A3 | 공격자: malformed/form, 임의 HTTP 응답과 서버장애 | 실제 log-event POST malformedJSON/form; 400/429/custom418; RuntimeError | 400/415, Retry-After17, custom본문/헤더 보존; 일반오류만500 | PASS: 모두일치, 일반오류500은 data-runtime에서 확인 | 아래 기록 | http-probes.json; data-runtime.json; pytest-errors-v2-full.log.gz | 런타임 완료 | 필수 |
