...
class Handler(BaseHTTPRequestHandler):
    def relay_upgrade(self):
        ...
        upstream=socket.create_connection(('127.0.0.1',57991),timeout=10)
        try:
            headers={k:v for k,v in self.headers.items() if k.lower() not in ('host','cookie','authorization')}
            headers['Host']='127.0.0.1:57990'
    ...
    def relay(self):
        ...
        target=57992 if path.startswith('/api/') or path.startswith('/__qa/') else 57991
...
ThreadingHTTPServer(('127.0.0.1',57990),Handler).serve_forever()
