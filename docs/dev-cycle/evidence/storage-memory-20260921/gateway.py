...
class Handler(BaseHTTPRequestHandler):
    def relay_upgrade(self):
        ...
        upstream=socket.create_connection(('127.0.0.1',57921),timeout=10)
        try:
            headers={k:v for k,v in self.headers.items() if k.lower() not in ('host','cookie','authorization')}
            headers['Host']='127.0.0.1:57920'
    ...
    def relay(self):
        ...
        target=57922 if path.startswith('/api/') or path.startswith('/__qa/') else 57921
...
ThreadingHTTPServer(('127.0.0.1',57920),Handler).serve_forever()
