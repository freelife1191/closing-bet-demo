...
for port in (57920,57921,57922):
 with socket.socket() as sock:sock.bind(('127.0.0.1',port))
D=S/'docs/dev-cycle/evidence/storage-memory-20260921';D.mkdir(parents=True,exist_ok=True)
...
env.update(PYTHON_DOTENV_DISABLED='1',PYTHONDONTWRITEBYTECODE='1',TMPDIR=str(S/'tmp'),PYTHONPATH=str(S),CI='true',API_URL='http://127.0.0.1:57922',NEXTAUTH_URL='http://127.0.0.1:57920',NEXTAUTH_SECRET='ui-qa-only-synthetic')
commands=[('fixture',[str(S/'venv/bin/python'),str(D/'fixture.py'),'--repo',str(S),'--port','57922','--reset-log'],S),('frontend',['node','node_modules/next/dist/bin/next','dev','--hostname','127.0.0.1','--port','57921'],S/'frontend'),('gateway',[str(S/'venv/bin/python'),str(D/'gateway.py')],S)]
started=[]
