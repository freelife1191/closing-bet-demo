...
assert ROOT.name.startswith("jongga-metrics-20260921-") and not (ROOT / ".git").exists()
...
from engine.toss_collector_metric_parsers import parse_investor_trend, parse_financials, parse_investment_indicators
...
app = Flask("jongga-metrics-fixture")
logger = logging.getLogger(__name__)
state = {"mode": "known", "version": 0}
cache_epoch = uuid.uuid4().hex
data_dir = ROOT / ".qa-metrics" / "data"
data_dir.mkdir(parents=True, exist_ok=True)
log = ROOT / ".qa-metrics" / "requests.jsonl"
...
def synthetic_detail(self, ticker):
    mode = state["mode"]
    period = {"known": "2026Q1", "legacy": None, "invalid": {"unexpected": "object"}}.get(mode)
    financials = parse_financials({"result": {"table": [
        {"period": "2025Q4", "netProfitKrw": -5},
        {"period": period, "revenueKrw": 10_000_000_000, "netProfitKrw": 2_600_000_000}]}},
        {"result": {"table": [{"period": "2025" if mode == "known" else None, "operatingIncomeKrw": 1_000_000_000}]}})
    indicators = parse_investment_indicators({"result": {"indicatorSections": [
        {"sectionName": "가치평가", "data": {"displayPer": "-1147.60배"}},
        {"sectionName": "수익", "data": {"epsKrw": -266}}]}})
    return {"code": ticker, "name": "재무 검증", "market": "KOSPI",
            ...
            "investor_trend": {"foreign": 0, "institution": 0, "individual": None},
            "financials": financials, "stability": {}}
...
def control():
    ...
    mode = body.get("mode")
    if mode not in {"known", "invalid", "error", "legacy"}:
...
def stock_detail(ticker):
    ...
    if state["mode"] == "legacy":
        ...
        for field in ("revenuePeriod", "operatingProfitPeriod", "netIncomePeriod"):
            payload["financials"].pop(field, None)
...
def jongga():
    ...
    signal = {"stock_code": "005930", "stock_name": "재무 검증", "market": "KOSPI", "sector": "검증",
              ...
              "score_details": {"bonus_score": bonus, "bonus_breakdown": {"volume": 4 if bonus > 7 else 5, "candle": bonus - 4 if bonus > 7 else 1, "limit_up": 0 if bonus > 7 else 1}, "foreign_net_buy": 1_000_000_000, "inst_net_buy": 500_000_000}, "news": []}
...
if __name__ == "__main__":
    ...
    assert Path(args.repo).resolve() == ROOT and args.port == 57952
