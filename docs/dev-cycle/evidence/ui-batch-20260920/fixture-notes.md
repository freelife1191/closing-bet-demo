...
- d-only: 원자료에는 D등급 종목 하나가 있고 화면에서는 정책상 제외되는 상태. 원자료 없음·사용자 필터0과 구분한다.
- zero: 정상 목록과 상세 수급 확정 집계 누락/실시간0 조합. 화면이 `+-` 대신 중립 `-`를 보이는지 검사한다.
- 종가 fixture점수는19점체계18/15/12로맞췄다. 이전QA1의100점형태표시는점수검증근거로사용하지않는다.
- gateway는HMR websocket만Next에전달하고API/auth는계속모든메서드를fixture로보낸다. 초기HMR404원문은qa1로그에보존한다.
