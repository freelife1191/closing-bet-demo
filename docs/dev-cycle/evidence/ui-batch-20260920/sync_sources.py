...
changed=subprocess.check_output(['git','diff','--name-only',m['base'],'--','frontend/src'],cwd=r,text=True).splitlines()
new=subprocess.check_output(['git','ls-files','--others','--exclude-standard','--','frontend/src'],cwd=r,text=True).splitlines()
