...
제품 모듈, 실제 데이터베이스, 환경 변수 파일을 전혀 불러오지 않는다. 모든 제품
변경 요청은 405로 막고, QA 제어와 합성 현재가 조회만 별도로 허용한다.
"""
...
def _vcp_signal(ticker: str, *, score: int, foreign_5d: int, inst_5d: int, signal_date: str) -> dict[str, Any]:
    ...
    return {
        ...
        "gemini_recommendation": _recommendation("BUY", 82, "합성 VCP 수축을 확인했습니다."),
...
def build_app(repo: Path, *, reset_log: bool = False) -> Flask:
    ...
    app = Flask(__name__)
    state: dict[str, Any] = {"admin": True, "mode": "normal"}
    ...
    def block_product_mutations() -> Any:
        allowed = (
            request.method == "POST"
            and request.path in {"/__qa/control", "/api/kr/realtime-prices"}
        )
    ...
    def qa_control() -> Any:
        ...
        unknown = set(payload) - {"admin", "mode"}
        if unknown:
    ...
    def ai_analysis() -> Any:
        payload = _vcp_payload(state["mode"], requested_date=request.args.get("date"))
        rows = [{key: value for key, value in signal.items()
                 if key in {"ticker", "gpt_recommendation", "gemini_recommendation", "news"}}
                for signal in payload["signals"]]
        return jsonify({"signals": rows, "generated_at": payload["generated_at"]})
