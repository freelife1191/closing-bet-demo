...
command, cwd, timeout = commands[stage]
environment = {key: os.environ[key] for key in ("PATH", "HOME", "USER", "TMPDIR", "LANG") if key in os.environ}
environment.update(PYTHONDONTWRITEBYTECODE="1", PYTHONPATH=str(scratch), CI="true", PYTHON_DOTENV_DISABLED="1")
...
with (evidence / (stage + ".log")).open("w") as output:
    ...
    try:
        ...
        process.wait()
        code = 124
result = {"stage": stage, "command": command, "cwd": str(cwd), "timeout_seconds": timeout, "timed_out": timed_out, "exit_code": code, "elapsed_seconds": round(time.monotonic() - started, 2)}
