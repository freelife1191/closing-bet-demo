...
class Handler(BaseHTTPRequestHandler):
    def relay_upgrade(self):
        ...
        upstream=socket.create_connection(('127.0.0.1',57971),timeout=10)
        try:
            headers={k:v for k,v in self.headers.items() if k.lower() not in ('host','cookie','authorization')}
            headers['Host']='127.0.0.1:57970'
    ...
    def relay(self):
        ...
        target=57972 if path.startswith('/api/') or path.startswith('/__qa/') else 57971
...
ThreadingHTTPServer(('127.0.0.1',57970),Handler).serve_forever()
