...
def generate_content(**kwargs):
 calls.append(kwargs['model']);return SimpleNamespace(text=json.dumps({'action':'HOLD','confidence':75,'reason':model_reason}))
...
def recommendation():
 ...
 return {'action':'HOLD','confidence':75,'reason':'K-칩스법과 수급 영향을 비교한 정상 분석입니다. 과거 템플릿의 전체 문장이 아닙니다.'}
def signal():
...
if __name__=='__main__':
 ...
 assert Path(args.repo).resolve()==ROOT and args.port==58002
