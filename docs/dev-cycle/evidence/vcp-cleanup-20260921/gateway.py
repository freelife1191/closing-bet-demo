...
class Handler(BaseHTTPRequestHandler):
    def relay_upgrade(self):
        ...
        upstream=socket.create_connection(('127.0.0.1',57961),timeout=10)
        try:
            headers={k:v for k,v in self.headers.items() if k.lower() not in ('host','cookie','authorization')}
            headers['Host']='127.0.0.1:57960'
    ...
    def relay(self):
        ...
        target=57962 if path.startswith('/api/') or path.startswith('/__qa/') else 57961
...
ThreadingHTTPServer(('127.0.0.1',57960),Handler).serve_forever()
