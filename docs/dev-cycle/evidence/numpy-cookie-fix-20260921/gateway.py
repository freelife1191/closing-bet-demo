...
class Handler(BaseHTTPRequestHandler):
    def relay_upgrade(self):
        ...
        upstream=socket.create_connection(('127.0.0.1',58011),timeout=10)
        try:
            headers={k:v for k,v in self.headers.items() if k.lower() not in ('host','cookie','authorization')}
            headers['Host']='127.0.0.1:58010'
    ...
    def relay(self):
        ...
        target=58011 if path.startswith('/api/auth/') else (58012 if path.startswith('/api/') or path.startswith('/__qa/') else 58011)
...
ThreadingHTTPServer(('127.0.0.1',58010),Handler).serve_forever()
