const task=await taskSpace(25),page=task.page('p1'),fs=await import('node:fs/promises');
const out='/Users/freelife/vibe/lecture/hodu/closing-bet-demo/docs/dev-cycle/evidence/numpy-cookie-fix-20260921';
...
for(const mode of ['normal','legacy','raw','generated']) {
 await fetch('http://127.0.0.1:58012/__qa/control',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({mode})});
