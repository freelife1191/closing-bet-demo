...
req=urllib.request.Request('https://api.osv.dev/v1/querybatch',data=json.dumps({'queries':[{'package':{'name':d['name'],'ecosystem':'PyPI'},'version':d['version'].split('+')[0] if d['name'].lower()=='pykrx' else d['version']} for d in deps]}).encode(),headers={'Content-Type':'application/json'})
