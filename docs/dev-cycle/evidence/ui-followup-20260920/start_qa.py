...
for port in (57720,57721,57722):
 with socket.socket() as sock:sock.bind(('127.0.0.1',port))
D=S/'docs/dev-cycle/evidence/ui-followup-20260920';D.mkdir(parents=True,exist_ok=True)
...
env.update(PYTHON_DOTENV_DISABLED='1',PYTHONDONTWRITEBYTECODE='1',TMPDIR=str(S/'tmp'),PYTHONPATH=str(S),CI='true',API_URL='http://127.0.0.1:57722',NEXTAUTH_URL='http://127.0.0.1:57720',NEXTAUTH_SECRET='ui-qa-only-synthetic')
commands=[('fixture',[str(S/'venv/bin/python'),str(D/'fixture.py'),'--repo',str(S),'--port','57722','--reset-log'],S),('frontend',['node','node_modules/next/dist/bin/next','dev','--hostname','127.0.0.1','--port','57721'],S/'frontend'),('gateway',[str(S/'venv/bin/python'),str(D/'gateway.py')],S)]
started=[]
