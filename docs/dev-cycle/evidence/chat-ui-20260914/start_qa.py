...
for port in (57601,57602):
 with socket.socket() as s: s.bind(('127.0.0.1',port))
env={k:os.environ[k] for k in ('PATH','HOME','USER','TMPDIR','LANG') if k in os.environ}
env.update(PYTHONDONTWRITEBYTECODE='1',PYTHONPATH=str(scratch),PYTHON_DOTENV_DISABLED='1',TMPDIR=str(scratch/'tmp'),CI='true',API_URL='http://127.0.0.1:57602',NEXTAUTH_URL='http://127.0.0.1:57601',NEXTAUTH_SECRET='synthetic-local-qa-only')
commands=[('backend',[str(scratch/'venv/bin/python'),str(scratch/'docs/dev-cycle/evidence/chat-ui-20260914/fixture.py'),'--repo',str(scratch),'--reset-state','serve','--port','57602'],scratch),('frontend',['node','node_modules/next/dist/bin/next','dev','--hostname','127.0.0.1','--port','57601'],scratch/'frontend')]
started=[]
