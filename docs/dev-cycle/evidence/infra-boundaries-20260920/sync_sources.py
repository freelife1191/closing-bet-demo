#!/usr/bin/env python3
"""Sync only this approved batch's tracked changes and explicit new regression files."""
...
files += [str(p.relative_to(R)) for p in (R/'tests/app').glob('test_infra_boundary_*.py')]
files += ['tests/test_restart_cleanup_contract.py']
files=sorted(set(files))
assert 'package.json' not in files
for f in files:
 assert (R/f).is_file(); (S/f).parent.mkdir(parents=True,exist_ok=True);shutil.copy2(R/f,S/f)
