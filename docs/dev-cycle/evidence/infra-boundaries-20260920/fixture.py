...
FIXTURE_PATH = Path("docs/dev-cycle/evidence/infra-boundaries-20260920/fixture.py")
SCRATCH_PREFIX = "infra-boundaries-20260920-"
...
ALLOWED_PORTFOLIO_MODES = {"success", "error-500"}
...
def validate_scratch_repo(raw_repo: str) -> Path:
    ...
    _assert(repo.name.startswith(SCRATCH_PREFIX), "infra-boundaries 전용 scratch checkout만 허용합니다.")
...
def _build_boundary_test_client(state: dict[str, Any]):
    """실제 Flask 등록 라우트를 합성 outer dependency에만 연결한다."""
    from app.routes import common_portfolio_routes, kr_market_system_http_routes, route_guards
    ...
    class SyntheticPortfolio:
        ...
        def get_portfolio_valuation(self, *, owner_id: str) -> dict[str, Any]:
            if state["portfolio_mode"] == "error-500":
                raise RuntimeError("/private/infra-boundary-sentinel")
            return _portfolio_payload()

    boundary = Flask("infra-boundaries-route-client")
    boundary.testing = True

    @boundary.before_request
    def install_fake_identity() -> None:
        g.user_email = "fakeadmin@example.test"
        g.user_api_key = "fixture-key"

    route_guards.is_admin_email = lambda _email: True
    common_portfolio_routes.verify_identity_header = lambda *_args, **_kwargs: "fake-owner"
    kr_market_system_http_routes._COMMON_UPDATE_HANDLERS = (
        lambda: {"isRunning": False},
        lambda _items: None,
        lambda *_args, **_kwargs: None,
    )
    kr_blueprint = Blueprint("infra-boundaries-kr", __name__)
    _register_refresh_route(
        kr_blueprint,
        logger=logging.getLogger(__name__),
        deps={"launch_background_update_job": lambda **_kwargs: _boundary_refresh_result(state)},
    )
    common_blueprint = Blueprint("infra-boundaries-common", __name__)
    context = CommonRouteContext(
        logger=logging.getLogger(__name__),
        update_lock=threading.Lock(),
        update_status_file="synthetic-status.json",
        load_update_status=lambda: {"isRunning": False},
        start_update=lambda _items: None,
        update_item_status=lambda _item, _status: None,
        stop_update=lambda: None,
        finish_update=lambda: None,
        run_background_update=lambda *_args, **_kwargs: None,
        paper_trading=SyntheticPortfolio(),
    )
    register_common_portfolio_routes(common_blueprint, context)
    boundary.register_blueprint(kr_blueprint, url_prefix="/api/kr")
    boundary.register_blueprint(common_blueprint, url_prefix="/api")
    return boundary.test_client()


def _boundary_refresh_result(state: dict[str, Any]) -> tuple[int, dict[str, Any]]:
    if state["refresh_mode"] == "error-500":
        raise RuntimeError("/private/infra-boundary-sentinel")
    return 200, {"status": "success", "source": "registered-route-fixture"}
...
def build_app(repo: Path, *, reset_log: bool = False) -> Flask:
    ...
    state: dict[str, Any] = {
        ...
        "price_mode": "success",
        "portfolio_mode": "success",
    }
    boundary_client = _build_boundary_test_client(state)
    log_dir = repo / ".qa-infra-boundaries"
    ...
    def qa_control() -> Any:
        ...
        unknown = set(payload) - {"admin", "mode", "refresh_mode", "price_mode", "portfolio_mode"}
        if unknown:
        ...
        portfolio_mode = payload.get("portfolio_mode")
        if portfolio_mode is not None and portfolio_mode not in ALLOWED_PORTFOLIO_MODES:
            return jsonify({"error": "portfolio_mode must be success or error-500"}), 400
        ...
        if portfolio_mode is not None:
            state["portfolio_mode"] = portfolio_mode
    ...
    def synthetic_refresh() -> Any:
        """refresh는 실제 등록 라우트와 합성 outer dependency를 이어서 돌린다."""
        if request.path == "/api/kr/refresh":
            response = boundary_client.post("/api/kr/refresh", json=request.get_json(silent=True) or {})
            return app.response_class(response.data, status=response.status_code, mimetype="application/json")

        """Market Gate 개별 갱신은 외부 효과 없이 기존 합성 응답을 사용한다."""
    ...
    def portfolio() -> Any:
        response = boundary_client.get("/api/portfolio")
        return app.response_class(response.data, status=response.status_code, mimetype="application/json")
