...
### [CHAT-043] 짧은 종목명 신호어 「주식·매수·매도」 가 일반 명사 뒤에서 오탐을 남긴다
- 카테고리: 챗봇 | 티어: T2 | 근거: `[CHAT-038]` 코드 리뷰 발견 1·2(2026-09-23). `[CHAT-038]` 은 두 글자 이하 종목명을 바로 뒤에 신호어가 올 때만 잡도록 했다. 실제 종목 맵으로 탐침하니 「VCP 분석 대상 주식 알려줘」·「투자 대상 주식 추천」·「관심 대상 매수 타이밍」→대상, 「한창 매수 중」→한창, 「남성 매수세」→남성, 「동양 주식 시장」→동양, 「전방 수급」→전방 이 여전히 잡힌다. 신호어 뒤에 경계가 없어 「대상 주가지수 흐름」 도 잡힌다. 짧은 이름 목록에는 선진·나노·알트·노을·진도처럼 같은 표현을 만드는 단어가 더 있다.
- 범위: 신호어 목록을 좁힐지(주식·매수·매도·수급 제외 등), 신호어 뒤 경계를 둘지(「주가는」·「어때요」 활용형 허용 필요) 결정하고 회귀 테스트를 더한다.
- 설계 승인: 2026-09-23 22:56 사용자 승인(bounded, 대화의 AskUserQuestion 「승인 (Recommended)」). 신호어에서 주식·매수·매도·수급을 빼고 `주가(?!지수)` 로 주가지수만 막는다. 신호어 뒤 일반 경계는 「실적발표」·「배당금」·「전망해줘」 를 놓치므로 두지 않는다. 대가: 「기아 주식 사도 돼?」·「기아 매수 타이밍」·「기아 수급」 은 문맥이 빠진다(`ponytail:` 주석). 변경 파일 `chatbot/stock_query_service.py`·`tests/chatbot/test_stock_query_service.py`, 위험 경로 아님
- [x] 설계 승인(bounded) - [x] 구현·RED→GREEN: 오탐 8건 추가 후 1 failed(「VCP 분석 대상 주식 알려줘」→대상), 구현 후 `tests/chatbot/` 283 passed - [x] `/ponytail-review` → `closing-bet-reviewer`: ponytail-review(oh-my-claudecode:code-reviewer 레인) 본문 지적 없음, 테스트 중복 사례 3건(「투자 대상 주식 추천」·「관심 대상 매수 타이밍」·「남성 매수세」) 삭제 반영. closing-bet-reviewer APPROVE(max low, 변이 6종 검출): L1 「매도」 재추가 변이가 통과 → 「대상 매도 시점」 추가, L2 띄어 쓴 「주가 지수」 가 잡힘 → `주가(?!\s*지수)` 와 「대상 주가 지수 흐름」 추가, L3 남은 신호어가 「분석 대상 전망」 류에서 오탐 → 신호어 방식의 한계라 `[CHAT-044]` 로 이월 - [ ] QA

### [CHAT-044] 짧은 종목명이 남은 신호어 「전망·실적·차트·어때」 앞에서도 일반 명사로 쓰인다
- 카테고리: 챗봇 | 티어: T2 | 근거: `[CHAT-043]` 코드 리뷰 L3(2026-09-23). 실제 짧은 이름 147개로 탐침하니 「대상 전망」·「대상 실적」·「대상 차트」·「대상 배당」→대상, 「한창 전망」→한창, 「동양 차트」→동양, 「노을 어때」·「진도 어때」→노을·진도, 「나노 실적」→나노 가 잡힌다. 가장 현실적인 오탐은 「분석 대상 전망」 류다. 「시장·금리·환율·코스피 전망」·「미국 주가」·「배당주 추천」 은 잡히지 않는다.
- 범위: 신호어 목록을 더 줄이는 방식은 정상 질문을 잃으므로, 짧은 이름 앞 문맥(「분석·투자·관심 대상」 같은 앞말)이나 짧은 이름 자체의 제외 목록 등 다른 판정 근거를 쓸지 결정한다. 잘못 싣는 비용(오답)과 놓치는 비용(일반 답변)을 함께 본다.
- [ ] 설계 승인(bounded) - [ ] 구현·RED→GREEN - [ ] `/ponytail-review` → `closing-bet-reviewer` - [ ] QA
...
- 추가 관찰(`[INFRA-083]`, 2026-09-23): 테스트가 원본을 쓰던 모듈 기준 경로가 위 목록 밖에도 있었다. `engine/collectors/{krx,news,naver_pykrx_mixin,krx_local_data_mixin}.py` 의 `BASE_DIR`(`.krx_collector_cache`), `engine/market_schedule.py:99`(`.market_schedule_cache`), `services/paper_trading.py:93`(`paper_trading.db`). `[INFRA-083]` 은 `tests/conftest.py` 에서만 돌렸고 운영 코드는 그대로다. 또 `engine/services/usage_tracker.py:204`(모듈 수준 `UsageTracker()`)와 `app/routes/kr_market.py:189`(`FileBackedStatus`)는 import 할 때 파일이 없으면 원본 `data/usage.db`·`vcp_status.json` 을 만든다. 두 파일이 있는 트리에서는 바뀌지 않지만 `data/` 가 없는 새 클론에서 전체 pytest 를 돌리면 이 두 파일이 생겨 conftest 의 세션 끝 검사가 실패한다
