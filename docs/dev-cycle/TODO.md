...
- 그런데 그 구간이 loopback 이라는 보장이 코드에 없습니다. `restart_all.sh:80` 의
  `gunicorn --bind 0.0.0.0:$FLASK_PORT`, `Procfile`, `app/__init__.py:299` 의
  `app.run(host='0.0.0.0')` 이 모두 모든 인터페이스에 바인딩하고, Flask 는 요청이 proxy 를
  거쳤는지 확인하지 않습니다(`remote_addr` 검사가 없습니다). 서명만 유효하면 출발지가
  어디든 통과하므로, 무해한 `GET` 에서 얻은 헤더를 120초 안에
  `DELETE /api/kr/chatbot/history?session_id=…` 에 그대로 붙일 수 있습니다.
- 검토자는 서명에 경로와 메서드를 넣는 것보다 바인딩 주소를 좁히는 편이 훨씬 싸다고
  했습니다. 전제를 관례가 아니라 구조로 만들고 proxy 와 Flask 양쪽 서명 형식을 함께 고칠
...
- [ ] 인가에 쓰는 값을 워커 사이에서 일치시키는 방식을 정함
- [ ] `POST /api/system/env` 에 키 허용 목록을 둘지 결정
- [ ] 워커 둘로 판정이 갈리지 않는 것을 확인하는 검사 추가
