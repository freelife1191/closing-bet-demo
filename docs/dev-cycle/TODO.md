...
- [ ] 운영자 확인: 운영 `.env` 에 `KRX_ID`·`KRX_PW` 존재 여부(값은 옮기지 않음), 오류 시각의 「KRX 로그인」「재로그인」 로그와 동시에 로그인한 프로세스 수
  ...
  - 2차(2026-09-25): pykrx `print` 는 워커 종료 때 한꺼번에 기록되어 시각을 믿을 수 없다. 41건은 09-23 종가베팅 수급 조회 중, 클라이언트가 유효하다고 본 세션에서 났다. 로컬 4차에서 무효 세션·미인증이 같은 문구를 내는 것을 확인했다. 원인은 「서버가 세션을 끊었는데 pykrx 가 다시 로그인하지 않음」이며, 서버가 끊은 이유는 재현되지 않았다. 설계 입력 (a) 빈 응답 때 잠금 아래 재로그인 1회 (b) 재로그인 실패 사유 로깅 (c) 워커 출력 버퍼링 해제(`qa/INFRA-089.md` 4차)
