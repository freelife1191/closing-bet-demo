...
- `app/routes/common_update_routes.py:219-252` 의 `POST /api/system/env` 는 키 허용 목록
  없이 임의의 키를 `.env` 와 **요청을 처리한 워커의** `os.environ` 에 씁니다. `ADMIN_EMAILS`
  도 쓰기 대상입니다.
- 종전에는 이 값이 화면 표시와 쿼터 면제에만 쓰였으나, `[INFRA-037]` 이후로는
  `/api/notification/send` 의 **인가 판정 근거**입니다. `CLAUDE.md` 가 적어 둔 대로 워커가
  둘 이상이면 이 값이 워커마다 갈리므로, 재기동 전까지 같은 관리자의 같은 요청이 어느
  워커에 닿느냐에 따라 통과와 거부가 갈립니다. `[INFRA-042]` 가 다른 발송 라우트에도 같은
  게이트를 세우면 영향 범위가 더 넓어집니다.
- 두 게이트가 모두 관리자를 요구하므로 권한 상승은 아닙니다. 문제는 **판정이 요청마다
  달라지는 것**이며, 관리자에게는 원인 없는 간헐적 403 으로 보입니다.
- 키 허용 목록을 두는 것과 인가에 쓰는 값을 요청 시점에 다시 읽는 것 두 방향이 있습니다.
  어느 쪽을 택할지 설계에서 정합니다.
- 설계 승인: 승인 일자 2026-09-07 | 승인 확인 시각 2026-09-07 23:55
  | 범위: `tests/app/test_common_env_service.py` 에 회귀 검사만 추가. 실행 코드는 바꾸지 않음
  | 실제 대화 근거: 2026-09-07 사용자가 「다음 라운드 진행해야할 사항 검토해서 진행해줘」로
  라운드를 열고, 조사 결과를 받은 뒤 「회귀 검사만 더하고 마감」을 선택
- QA 시나리오: 워커 둘을 띄우고 설정 화면에서 `ADMIN_EMAILS` 를 바꾼 뒤 같은 요청을
  반복하면 통과와 거부가 섞이지 않는다
- [x] 인가에 쓰는 값을 워커 사이에서 일치시키는 방식을 정함 — 결정할 것이 없었다.
  `is_admin_email` 은 매 요청 `os.environ` 을 다시 읽고, 그 값을 런타임에 바꾸는 경로가
  닫혀 있어 워커가 몇 개든 같은 값을 본다. 저장소에서 `os.environ` 에 쓰는 자리는
  `engine/config.py:289`·`services/scheduler.py:63`·`update_env_file` 셋뿐이고,
  임의 키를 받는 것은 마지막 하나인데 거기서 걸러진다.
- [x] `POST /api/system/env` 에 키 허용 목록을 둘지 결정 — 이미 있었다.
  `[INFRA-025]`(`95b022c`)가 `services/common_env_service.py:96-100` 에 넣었고
  `ADMIN_EMAILS` 는 `EDITABLE_ENV_KEYS` 밖이다. 이 항목의 근거가 된
  `[INFRA-037]` 사이클 보안 리뷰가 바로 직전 커밋인 그 필터를 보지 못했다.
- [x] 워커 둘로 판정이 갈리지 않는 것을 확인하는 검사 추가 —
  `tests/app/test_common_env_service.py` 에 읽기·쓰기 두 방향으로 더했다. 워커를 실제로
  둘 띄우는 대신, `os.environ` 의 인가 키를 바꿀 수 있는 유일한 경로인 `update_env_file`
  이 그 키를 거르는지를 잰다. 필터가 서 있으면 워커 수와 무관하게 값이 같으므로 원인을
  직접 겨누는 쪽이다. 두 검사가 실제로 결함을 잡는 것도 확인했다. `EDITABLE_ENV_KEYS` 에
  인가 키를 넣으면 둘 다 실패하고, 쓰기 필터 세 줄을 걷어내면 쓰기 검사가 실패한다.
- [x] 리뷰: `/ponytail-review` — `Lean already. Ship.` 중복으로 보이던 어서트 둘은 기존
  검사와 입력이 달라(인가 키가 섞인 딕셔너리·파일) 다른 보장을 재므로 유지했다.
- [x] 시크릿 확인 세 가지 — `.env*` 는 `.env.example` 만 추적됨. 인가 키를 로그나 응답
  본문에 싣는 자리 없음. `NEXT_PUBLIC_` 은 `API_URL`·`GOOGLE_CLIENT_ID` 둘뿐.
- [x] 보안 리뷰(`oh-my-claudecode:security-reviewer`) — **[F1] 로 위 두 체크의 전제가
  뒤집혔다.** 필터가 키만 보고 값은 보지 않아, 값에 개행을 넣으면 `.env` 에 목록 밖의 줄을
  쓸 수 있었다. 두 쓰기 경로 모두 뚫렸고 `restart_all.sh:11`·`stop_all.sh:16` 이
  `set -a; source .env` 를 하므로 셸 실행까지 이어졌다. 재현해 확인한 뒤 사용자 승인을 받아
  이번 범위에 넣어 고쳤다(`services/common_env_service.py:98`). [F2]·[F3] 은 이월했다.
  키 이름의 대소문자·공백·유니코드 변형은 허용 목록이 정확 일치 기반이라 우회되지 않음을
  리뷰가 직접 실행해 확인했다. `frontend/.env` 가 루트 `.env` 의 심볼릭 링크라는 사실도
  이 리뷰가 알려 주었다(`restart_all.sh:17`). 그래서 주입은 Flask 와 Next 두 게이트를
  한꺼번에 오염시킨다.
- [x] 코드 리뷰(`feature-dev:code-reviewer`) — 개행 필터 자체는 닫혔고 검사 셋이 보장을
  충분히 잰다고 판정했다. 다만 **개행 없이도 `$(...)` 명령 치환으로 임의 코드 실행이
  된다**는 더 큰 결함을 재현해 보고했다. 이번 변경이 만든 것이 아니고 근본 수정이 쓰기
  인용과 기동 스크립트 양쪽에 걸치므로 사용자 판단에 따라 `[INFRA-049]`(P0) 로 이월했다.
  편집 가능한 열두 키가 모두 단일행 입력이라 개행 검사에 기능 손실이 없다는 것도 확인해
  주었다(`SettingsModal.tsx`). 조용한 거부는 `[INFRA-051]` 로 이월했다.
- [x] 검증 — pytest 1798 통과·2 skip (exit 0), QA 필수 11/11 통과
  (`docs/dev-cycle/qa/INFRA-044.md`)
