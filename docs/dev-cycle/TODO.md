...
- 카테고리: 프론트엔드 공통 | 티어: T2 | 근거: `[FE-003]` 사이클의 `/qa-only` 실측 (ISSUE-001)
- 보유 종목 행에 붙은 클릭 핸들러로 이벤트가 버블링되어, 매수·매도 버튼을 눌러도 주문
  모달(`z-110`)과 종목 상세 패널(`z-120`)이 동시에 열립니다. 상세가 위를 덮어 주문 화면이
  보이지 않고, 상세를 닫으면 주문 모달도 함께 사라져 목록에서 주문을 넣을 수 없습니다.
  `[FE-003]` 이전부터 있던 결함임을 `git stash` 로 확인했습니다.
- [ ] 매수·매도 버튼의 클릭 이벤트가 행 핸들러로 전파되지 않도록 차단
- [ ] 두 버튼이 각각 주문 모달만 열고 상세 패널은 열지 않는지 vitest 로 검사
- [ ] 실측으로 매수·매도 모달이 가려지지 않고 보이는지 확인
