...
- [ ] 원인 확정 2(실측): 09-24 16:00~17:30 사이 yfinance 로 당일 하루치를 시각별로 조회해 봉이 있는 종목 수를 센다(조회만, 사본에만 씀). 운영자 확인: 운영 `backend.log` 09-16·09-17 17:00 무렵 「yfinance 진행」「백업 수집 완료」 줄과 운영 `korean_stocks_list.csv` 종목 수
