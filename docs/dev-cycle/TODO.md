...
### [INFRA-059] `/api/kr/refresh` 가 방금 잠근 라우트와 같은 함수·상태 파일을 무인증으로 쓴다
- 카테고리: 인프라 | 티어: T3(인가 경계) | 근거: 2026-09-08 `[INFRA-042]` 사이클의 적대적
  리뷰가 F1 로 찾았습니다. **같은 라운드의 계획 검토·코드 리뷰·보안 리뷰 셋이 모두 놓쳤습니다.**
- `[INFRA-042]` 가 `/api/kr/init-data` 와 `/api/system/start-update` 를 닫았는데,
  `POST /api/kr/refresh` 는 게이트 없이 남아 `_resolve_common_update_handlers()` 로 **같은
  핸들러 함수와 같은 상태 파일**에 닿습니다. 잠근 문 옆에 열린 문이 남은 모양입니다.
- 그래서 익명 요청이 둘을 할 수 있습니다. LLM 호출 비용을 유발하고, `isRunning` 을 세워
  관리자 전용으로 만든 자리를 **거꾸로 잠급니다.**
- **분류 기준의 결함이 원인입니다.** `[INFRA-042]` 는 라우트를 「화면이 그 버튼을 누구에게
  보이는가」로 갈랐는데, 그 기준은 화면에 없더라도 서버에서 같은 함수를 공유하는 형제
  라우트를 세지 못합니다. 이 항목을 다룰 때는 화면이 아니라 **핸들러와 상태 파일을 기준으로**
  호출 그래프를 훑습니다.
- `[INFRA-042]` 가 `kr/refresh` 를 부류 B(일반 사용자 노출)로 두어 이월했으나, 그 판단은
  화면만 본 것이라 이 결함을 담지 못합니다. 화면 노출을 어떻게 할지와 무관하게 서버 쪽
  공유가 남습니다.
- QA 시나리오: 신원 없이 `POST /api/kr/refresh` 를 보낸 뒤 `isRunning` 이 서지 않고 관리자
  요청이 계속 통과하는지 확인
- [ ] 호출 그래프를 핸들러 기준으로 훑어 같은 상태를 공유하는 라우트를 전부 찾음
- [ ] 화면 쪽 노출 조건과 오류 처리를 함께 정함 (일반 사용자 버튼이 403 을 받게 되므로)
...
- 2026-09-07 `[INFRA-040]` 사이클의 `oh-my-claudecode:security-reviewer` 가 같은 성질의
  라우트를 더 찾았습니다(확신도 높음). `POST /api/portfolio/reset`·`buy`·`sell`,
  `POST /api/kr/signals/run`, `POST /api/kr/reanalyze/gemini`,
  `POST /api/kr/market-gate/update`, `POST /api/kr/refresh`, `POST /api/kr/init-data`,
  `POST /api/kr/config/interval` 이 신원을 보지 않습니다. 위 세 라우트와 같은 데코레이터로
  덮을 수 있으므로 이 항목의 범위에 넣습니다.
- `[INFRA-040]` 의 교차 출처 차단은 이 문제를 줄이지 못합니다. 그 차단은 쿠키에서 신원이
...
- 2026-09-08 `[INFRA-044]` 사이클의 `oh-my-claudecode:security-reviewer` 가 셋을 보탰습니다
  (확신도 높음). 첫째, 위 목록의 `POST /api/kr/config/interval` 은 단순 설정 변경이 아니라
  `services/kr_market_interval_service.py:22` 가 **`.env` 파일 전체를 다시 씁니다.** 값은
  `int()` 변환과 1..1440 범위 검사를 거치므로 내용 주입은 되지 않지만, 인증 없는 요청이
  서버 설정 파일을 다시 쓰는 것 자체가 남습니다. 둘째, 관리자 판정 규칙이
  `frontend/src/lib/adminEmails.ts` 와 `services/admin_helpers.py:20` 에 **두 언어로 두 벌**
  존재하는데 그 일치를 강제하는 검사가 없습니다. 한쪽만 정규화를 늘리면 같은 사용자가 한
  게이트는 통과하고 다른 게이트는 막힙니다. 같은 입력 목록을 `.py` 와 `.ts` 검사가 공유하게
  하면 표류가 곧바로 드러납니다. 셋째, `ADMIN_API_TOKEN` 은 **만료도 폐기 절차도 없는 순수
  소지 비밀**입니다. `ADMIN_EMAILS` 에서 관리자를 지워도 토큰 값을 아는 사람은 loopback 으로
  Flask 에 직접 요청해 통과합니다. 신원 서명 쪽에는 `IDENTITY_TTL_SECONDS` 가 있어 두 게이트의
  폐기 특성이 다릅니다.
- **2026-09-08 범위 확정.** 이 항목이 한 덩어리로 묶은 라우트가 실측하면 네 부류로
  갈립니다. 부류를 가르는 것은 서버가 아니라 **화면이 그 버튼을 누구에게 보이는가**입니다.
  - **A. 화면이 이미 관리자 전용이고 서버만 열린 여섯** — `jongga-v2/run`(`:98`),
    `jongga-v2/reanalyze-gemini`(`:167`), `jongga-v2/message`(`:194`),
    `signals/run`(`kr_market_data_signals_routes.py:206`),
    `init-data`(`kr_market_system_http_routes.py:214`, 화면 호출자 없음),
    `system/start-update`(`common_update_routes.py:124`). 게이트를 세워도 화면은 한 줄도
    바뀌지 않습니다. **이번 범위입니다.**
  - **B. 일반 사용자에게 노출된 셋** — `kr/refresh`, `market-gate/update`,
    `config/interval`(POST). `frontend/src/app/dashboard/kr/page.tsx` 는 `useAdmin` 을
    import 조차 하지 않습니다. 게이트를 세우면 일반 사용자 버튼이 403 을 받으므로 화면 쪽
    노출 조건과 오류 처리를 함께 고쳐야 하고, 그것은 제품 정책 결정입니다.
    `market-gate/update` 와 `config/interval` 은 `[INFRA-061]` 로 이월했습니다.
    `kr/refresh` 는 적대적 리뷰 F1 이 서버 쪽 공유를 찾아내 성격이 달라졌으므로
    `[INFRA-059]` 로 따로 세웠습니다.
  - **C. 모의투자 다섯** — `portfolio/reset`·`buy`·`buy/bulk`·`sell`·`deposit`.
    처음에 「넷」으로 적었으나 전수 조사에서 `deposit`(`common_portfolio_routes.py:149`)이
    빠져 있던 것이 드러났습니다. `reset_account()` 와 `buy_stock()` 에 사용자 인자가 없어
    **계정이 전역 하나**입니다. 관리자 게이트로 막으면 기능이 죽으므로 해법이 소유자
    분리이고, `services/paper_trading.py` 는 `tier-rules.md` §2 의 위험 경로입니다.
    `[INFRA-060]` 으로 이월했습니다.
  - **D. 이미 401 로 닫혀 있는 하나** — `POST /api/kr/reanalyze/gemini`
    (`kr_market_system_http_routes.py:129`). **`jongga-v2/reanalyze-gemini` 와 이름이
    비슷하지만 다른 라우트입니다.** 이 항목의 원래 목록에 있었으나 처음 분류할 때 제가
    빠뜨렸고, 계획 검토(`critic`)가 셈이 맞지 않는 것을 짚어 드러났습니다.
    **근거가 된 리뷰의 「신원을 보지 않습니다」가 이 라우트에 대해서는 틀렸습니다.**
    `:141` 이 `g.get('user_email')` 을 읽어 넘기고,
    `services/kr_market_route_service.py:153-159` 가 `user_api_key` 도 `user_email` 도
    없으면 `401 UNAUTHORIZED` 를 돌려줍니다. Vertex 전환 이후 `app/__init__.py:175` 가
    `g.user_api_key = None` 으로 고정하므로 개인 키 갈래는 **도달할 수 없고**, 결과적으로
    익명 요청은 예외 없이 401 을 받습니다. 게다가 `frontend/src` 전체에 이 경로를 부르는
    자리가 하나도 없습니다(`init-data` 와 같은 상태). 그러므로 이 라우트에는 손댈 것이
    없습니다. 남는 물음은 쿼터가 실제로 새지 않는지인데 그것은 인가가 아니라 쿼터의
    문제이고, 새 항목을 세울 때 `g.user_api_key` 가 상수 `None` 이라 「개인 키 있으면
    무제한」 갈래가 도달 불가라는 사실을 근거에 함께 넣습니다. 그 갈래가 살아 있는 줄 알고
    읽으면 실제보다 큰 위험으로 보입니다.
- **게이트 표준은 `is_admin_email(g.get("user_email"))` 입니다.** 부류 A 는 전부 브라우저가
  직접 부르고, 화면의 `useAdmin` 훅이 `/api/admin/check` → `common_admin_routes.py:30` 을
  거쳐 같은 함수로 판정합니다. `verify_admin_api_token` 은 Next 라우트 핸들러가 중계하는
  `/api/system/env` 전용이라 이 자리에 맞지 않습니다.
- **`OPTIONS` 를 통과시켜야 합니다.** `app/__init__.py:171` 의 `before_request` 가 OPTIONS
  에서 일찍 반환해 `g.user_email` 을 세우지 않으므로, 예외를 두지 않으면
  `reanalyze-gemini`(`methods=["POST", "OPTIONS"]`)의 preflight 가 403 을 받아 브라우저가
  본 요청을 아예 보내지 않습니다. **그렇다고 뷰로 넘기지도 않습니다.** Flask 는 `methods`
  에 OPTIONS 를 적은 라우트에 한해 자동 응답을 끄고 뷰를 부르는데, 그 요청에는 본문을 실을
  수 있어 `curl -X OPTIONS -d ...` 한 줄이 무인증으로 뷰에 닿습니다(실측 재현). 지금은 그런
  라우트가 하나뿐이고 그 뷰가 첫 줄에서 빠지지만, 불변식을 지키는 것이 주석뿐이면 다음에
  조기 반환을 빠뜨린 라우트가 생기는 순간 그 자리가 열립니다. 그래서 게이트가
  `current_app.make_default_options_response()` 로 직접 응답하고 끝냅니다. 세 리뷰(보안·
  적대적·Codex)가 각각 독립적으로 같은 결론을 냈습니다.
- QA 시나리오: 신원 없이 보낸 `POST /api/kr/jongga-v2/message` 가 403 을 받고 발송이 없다
- [x] 세 라우트에 요구할 권한 수준을 정함. 기존 게이트 둘의 판정 방식이 다르므로 어느
  쪽을 표준으로 삼을지 함께 정한다 → 위의 「게이트 표준」에 적었습니다. 대상은 셋이 아니라
  여섯입니다.
- [x] 두 언어의 관리자 판정 구현이 같은 입력에 같은 답을 내는지 재는 검사 추가 →
  `tests/fixtures/admin_email_cases.json` 케이스 13개를 `tests/services/test_admin_helpers.py`
  와 `frontend/src/lib/adminEmails.test.ts` 가 함께 읽습니다. 어느 쪽 규칙을 고쳐도 반대쪽이
  실패합니다.
- [x] 관리자를 `ADMIN_EMAILS` 에서 지웠을 때 실제로 무엇이 닫히는지 문서화하고
  `ADMIN_API_TOKEN` 회전 절차를 함께 적음 → `CLAUDE.md` 의 해당 절에 폐기 특성과 회전 다섯
  단계를 적었습니다. **처음 쓴 설명이 두 번 틀려 두 번 고쳤습니다.** 「설정 화면으로 바꾸면
  워커 하나만 바뀐다」는 막다른 길이었습니다. 두 키 모두 `EDITABLE_ENV_KEYS` 밖이라 그
  화면으로 아예 바꿀 수 없습니다(관리자가 화면에서 자기 자신을 잠그는 것을 막는 의도적
  설계). 「TTL 120 초라 `ADMIN_EMAILS` 를 지운 효과가 그 안에 반영된다」도 틀렸습니다.
  `proxy.ts` 는 `ADMIN_EMAILS` 를 전혀 보지 않으므로 인과가 없습니다. **실제 차이는 폐기
  수단이 하나인가 둘인가입니다.** `ADMIN_EMAILS` 는 NextAuth 로그인을 끊으면 매 요청
  `getToken` 하는 `proxy.ts` 때문에 재기동 없이 즉시 듣고, 토큰 뒤에는 계정이 없어 그 길이
  없습니다. `[INFRA-042]` 이후로는 `INTERNAL_IDENTITY_SECRET` 이 더 넓은 것을 지킨다는
  사실도 함께 적었습니다.
- [x] 게이트를 데코레이터로 묶고 부류 A 여섯에 적용 → `app/routes/route_guards.py` 의
  `require_admin`. **최종 적용은 열두 자리입니다.** 승인 범위인 여섯에, 리뷰가 찾은 여섯을
  더했습니다. 적대적 리뷰 F2 가 짚은 중단 계열 셋(`stop-update`, `finish-update`,
  `update-item-status`)은 `start-update` 와 같은 상태 파일을 쓰므로 들어가는 문만 잠그면
  의미가 없습니다. 보안 리뷰 4-2 와 전수 조사가 찾은 셋(`signals/reanalyze-failed-ai`,
  그 `/stop`, `jongga-v2/analyze`)도 같은 성질입니다. 확대 두 번을 각각 사용자에게 확인받아
  진행했습니다. 열두 자리 모두 `@route` → `@require_admin` → `def` 순서를 확인했습니다.
- [x] 게이트 목록과 실제가 일치하는지 재는 불변식 검사 추가 (적대적 리뷰 F7) →
  `test_gated_route_list_matches_reality`. `url_map` 을 훑어 `is_admin_email` 로 판정하는
  POST 라우트를 모아 선언 목록 열셋과 양방향 대조합니다. 종전부터 인라인 `if` 로 닫혀 있던
  `/api/notification/send` 도 목록에 넣어, 그 게이트를 지우는 변경도 잡히게 했습니다.
- [x] `{"force": true}` 로도 우회되지 않는 것을 확인하는 테스트 추가 →
  `test_jongga_message_refuses_anonymous_even_with_force`. 의존성을 「불리면 `AssertionError`
  를 내는 가짜」로 두어 403 과 「발송 준비에 닿지 않았다」를 함께 잽니다.
- [x] 발송 없이 확인할 수 있는 QA 수단을 정함 (`.env` 에 실제 자격 증명이 들어 있음) →
  Flask 검사 클라이언트로 같은 등록 함수를 세우고, 의존성을 「불리면 `AssertionError` 를
  내는 가짜」로 넣었습니다. 게이트가 없을 때 호출이 실제로 일어난다는 것을 검사가 직접
  증명하므로 막는 쪽과 통과하는 쪽을 함께 잽니다. 실제 발송 함수는 어느 시나리오에서도
  부르지 않았습니다. 근거는 `docs/dev-cycle/qa/INFRA-042.md` 의 「화면 대신 하네스를 쓰는
  이유」에 있습니다.
- **이 게이트의 강도가 무엇에 묶여 있는지 적어 둡니다.** 판정 근거인 `g.user_email` 은
  `services/identity_helpers.py` 의 `verify_identity_header` 가 꺼낸 값이고, 그 서명은 경로·
  메서드·nonce 에 묶이지 않습니다(그 함수의 `ponytail:` 주석에 이미 적혀 있습니다). 즉 한
  경로에서 얻은 서명을 유효 기간 안에 다른 경로로 재생할 수 있습니다. 이번 변경이 만든 결함이
  아니라 물려받은 성질이며, `IDENTITY_TTL_SECONDS` 가 120 초라 창이 좁습니다. 코드 리뷰가
  확신도 낮음으로 참고 삼아 짚어 주었습니다. **이번 변경으로 이 서명이 지키는 범위가
  넓어졌으므로** `[INFRA-062]` 로 세웠습니다.
- 설계 승인: 2026-09-08 대화에서 「부류 A 여섯만」과 「두 언어 판정 일치 검사 + 회전 절차
  문서화 둘 다 포함」, 그리고 「이 설계로 진행」을 각각 승인받았습니다. 기존 게이트 두 자리를
  데코레이터로 전환하는 선택지는 제시했으나 채택되지 않았습니다.
- 티어: **T3**. `tier-rules.md` §2 의 파일 목록에는 걸리지 않지만 인가 게이트를 새로 세우는
  변경이고, §1 의 「인증이나 시크릿에 닿으면 티어와 무관하게 더한다」는 시크릿 확인 세 가지와
  보안 리뷰가 함께 붙습니다. 하향하지 않습니다.
- 계획: `docs/superpowers/plans/2026-09-08-infra-042-admin-route-guard.md`
- 계획 검토: `oh-my-claudecode:critic` 판정 `REVISE` → 지적 전부 반영 후 재확인에서
  `ACCEPT`. 잡힌 것은 열이며 성격이 셋으로 갈립니다.
  - **실행을 막던 것 둘.** 뷰 함수 이름을 `run_jongga_v2_route` 로 적었으나 실제는
    `run_jongga_v2_screener_route`(`:99`)라 데코레이터를 붙일 자리를 못 찾습니다. 그리고
    `git add` 목록에 아직 없는 `docs/dev-cycle/qa/INFRA-042.md` 가 들어 있어, 실행하면
    `fatal` 로 아무것도 스테이징하지 않으면서 **종료 코드 0** 을 내고 이어지는
    `git diff --cached --check` 도 빈 인덱스를 보고 `exit=0` 을 냅니다. 두 신호가 모두
    통과로 보여 빈 커밋이 만들어집니다. 실측으로 재현했고, QA 문서를 스테이징 앞 단계에서
    만들도록 하고 `--name-only` 의 줄 수를 함께 세도록 고쳤습니다.
  - **제가 빠뜨린 라우트 하나.** 「14개를 세 부류로」라고 적고 13개만 분류했습니다. 빠진
    것이 위의 부류 D 이고, 그 근거를 「일반 사용자 기능이라서」에서 「이미 401 로 닫혀
    있어서」로 바꿨습니다. 앞의 서술로 두면 다음 감사가 같은 라우트를 열린 것으로 보고 다시
    올립니다.
  - **사실 오류와 인용 오차.** Task 4 의 「Next 라우트 핸들러가 기동 시점의 값을 들고
    있다」가 틀렸습니다. `route.ts:31` 은 함수 안이라 매번 읽고, 모듈 최상위인 것은 `:17`
    의 `FLASK_BASE` 입니다. 재기동이 필요한 진짜 이유는 `process.env` 를 `.env` 에서 채우는
    것이 기동 한 번뿐이고 `next start` 는 다시 읽지 않기 때문입니다. 인용 위치 오차 일곱도
    함께 정정했습니다.
  - **검사 설계 보강 둘.** `start-update` 검사에서 `load_update_status` 까지 실패 가짜로
    두면 첫 호출에서 터져 「스레드가 뜨지 않았다」를 증명하지 못하므로 그것만 통과시킵니다.
    그리고 `assert ... is expected` 를 `==` 로 바꿨습니다. 구현이 truthy 한 비 `bool` 을
    돌려주게 바뀌면 옳은 구현에도 실패하기 때문입니다.
  - **두 구현이 오늘 갈리는 입력을 찾아 주었습니다.** `str.strip()` 과 `trim()` 의 공백
    집합이 달라 U+001C·U+0085 는 파이썬만, U+FEFF(BOM) 는 자바스크립트만 제거합니다(양쪽
    런타임에서 실측). BOM 이 섞이면 「버튼은 보이는데 403」이 됩니다. 다만 `ADMIN_EMAILS` 가
    `EDITABLE_ENV_KEYS` 에 없어 유입 경로가 수작업 편집뿐이므로, 별도 항목을 세우지 않고
    공유 케이스 파일의 `_ceiling` 에 천장으로만 적기로 했습니다.
- 과잉설계 리뷰(`/ponytail-review`): -18줄 가능. 넷 전부 적용해 `route_guards.py` 를
  54 → 49줄로 줄였습니다.
- 코드 리뷰(`feature-dev:code-reviewer`): 결함 없음. 확신도 낮은 관찰 셋 가운데 둘을
  반영했습니다. 나머지 하나는 아래 「이 게이트의 강도」에 적은 서명 재생 성질이며 이번
  변경이 만든 결함이 아니라 물려받은 것이라 별도 항목으로 넘깁니다.
- 보안 리뷰(`oh-my-claudecode:security-reviewer`): 지적 있음. 4-2 가
  `POST /api/kr/signals/reanalyze-failed-ai` 에 서버 검사가 없다는 것을 짚었습니다.
  `vcp/page.tsx:713` 의 `if (!isAdmin)` 뒤에 있어 `/signals/run` 호출부와 같은 모양인데
  서버 게이트만 빠져 있었습니다. 게이트를 붙였고 `/stop` 도 함께 닫았습니다.
- 적대적 리뷰(`general-purpose`): 지적 있음. 셋을 반영했습니다.
  - **F1 은 앞선 세 리뷰가 모두 놓친 것입니다.** `/api/kr/refresh` 에 게이트가 없는데
    `_resolve_common_update_handlers()` 로 `/init-data`·`/start-update` 와 **같은 함수와 같은
    상태 파일**을 씁니다. 익명 요청이 LLM 비용을 유발하고 `isRunning` 을 세워 방금 잠근
    자리를 거꾸로 잠글 수 있습니다. 제 분류 기준이 「화면이 버튼을 누구에게 보이는가」라
    서버 쪽 형제 라우트를 세지 못한 것이 원인입니다. 아래 이월 항목으로 세웠습니다.
  - F2(중단 계열 비대칭)와 F7(목록 불변식 부재)은 이번 범위에서 닫았습니다.
- Codex 교차 검토: OPTIONS 갈래에서 뷰 호출을 제거하라는 지적. 반영했습니다. 첫 패스는
  540초를 넘겨(exit 124) 롤아웃 로그 없이 끝났고, 범위 밖 `engine/generator.py` 를 탐색
  중이었습니다. 파일 목록을 명시하고 탐색을 금지한 재시도가 성공했습니다(exit 0).

### [INFRA-060] 모의투자 계정이 전역 하나라 아무나 남의 잔고를 바꾼다
- 카테고리: 인프라 | 티어: T3(`services/paper_trading.py` 는 위험 경로) | 근거: 2026-09-07
  `[INFRA-040]` 사이클의 보안 리뷰가 찾았고 `[INFRA-042]` 가 부류 C 로 이월했습니다.
- POST 라우트가 **다섯**입니다. `portfolio/reset`·`buy`·`buy/bulk`·`sell`·`deposit`
  (`app/routes/common_portfolio_routes.py:72`·`:85`·`:108`·`:127`·`:149`). `[INFRA-042]` 는
  「넷」으로 적었는데 `deposit` 이 빠져 있었고, 전수 조사에서 드러났습니다.
- 다섯 다 신원을 보지 않고, `reset_account()` 와 `buy_stock()` 에 사용자 인자가 없어
  **계정이 전역 하나**입니다. 익명 요청이 남의 잔고를 초기화하거나 입금액을 바꿉니다.
- **관리자 게이트로는 못 막습니다.** 일반 사용자가 쓰는 기능이라 막으면 기능이 죽습니다.
  해법은 인가가 아니라 **소유자 분리**입니다. 저장 계층에 사용자 식별자를 넣고 기존 자료를
  어디에 귀속시킬지 정하는 것이 본체입니다.
- QA 시나리오: 서로 다른 두 신원이 각자의 잔고만 보고 바꾸는지 확인
- [ ] 기존 전역 계정 자료의 귀속 방침을 정함 (마이그레이션 유무)
- [ ] 저장 계층에 소유자 식별자 도입
- [ ] 다섯 라우트가 자기 계정에만 닿는지 재는 검사 추가

### [INFRA-061] 무인증 요청이 서버 설정과 Market Gate 를 바꾼다
- 카테고리: 인프라 | 티어: T3(`.env` 접촉) | 근거: 2026-09-07 `[INFRA-040]` 과 2026-09-08
  `[INFRA-044]` 사이클의 보안 리뷰. `[INFRA-042]` 가 부류 B 로 이월했습니다.
- `POST /api/kr/config/interval` 은 단순 설정 변경이 아니라
  `services/kr_market_interval_service.py:22` 가 **`.env` 파일 전체를 다시 씁니다.** 값은
  `int()` 변환과 1..1440 범위 검사를 거치므로 내용 주입은 되지 않지만, 인증 없는 요청이 서버
  설정 파일을 다시 쓰는 것 자체가 남습니다.
- `POST /api/kr/market-gate/update` 도 신원을 보지 않습니다.
- **화면 쪽 결정이 함께 필요합니다.** `frontend/src/app/dashboard/kr/page.tsx` 가 `useAdmin`
  을 import 조차 하지 않아 두 버튼이 일반 사용자에게 보입니다. 게이트를 세우면 그 버튼이
  403 을 받으므로 노출 조건과 오류 처리를 함께 고쳐야 하고, 관리자 전용으로 바꿀지가 제품
  정책 결정입니다.
- `[INFRA-056]` 이 `config/interval` 의 저장 경로 버그를 따로 다룹니다. **순서를 뒤집지
  않습니다.** 저장이 실제로 동작하게 만든 뒤 인가를 여는 것은 없던 노출을 만드는 일입니다.
- QA 시나리오: 신원 없이 두 라우트를 쳤을 때 `.env` 와 Market Gate 상태가 바뀌지 않는지 확인
- [ ] 두 버튼의 노출 대상을 정함
- [ ] 게이트를 세우고 화면 오류 처리를 함께 고침
- [ ] `GATED_ROUTES` 목록 갱신
...
- 카테고리: 인프라 | 티어: T3(인증) | 근거: 2026-09-08 `[INFRA-042]` 사이클의
  ...
  물려받은 성질입니다.
- `services/identity_helpers.py` 의 `verify_identity_header` 가 꺼내는 서명이 경로·메서드·
  nonce 에 묶이지 않습니다(그 함수의 `ponytail:` 주석에 이미 적혀 있습니다). 한 경로에서
  얻은 서명을 유효 기간 안에 다른 경로로 재생할 수 있다는 뜻입니다.
- **`[INFRA-042]` 이후로 이 서명이 지키는 범위가 넓어졌습니다.** 관리자 전용으로 닫은 라우트
  열둘의 판정이 전부 이 값에 걸려 있고, 그 안에 구독자에게 실제 메시지를 보내는 경로가
  들어 있습니다.
...
- 카테고리: 인프라 | 티어: T1 | 근거: 2026-09-08 `[INFRA-044]` 사이클의
  `feature-dev:code-reviewer` 지적(확신도 높음)
- `app/routes/common_update_routes.py:242-246` 의 `_handle_post` 는 `update_env_file` 의
  반환값을 보지 않고 언제나 `{"status": "ok"}` 를 돌려줍니다. `update_env_file` 은 허용 목록
  밖의 키와 개행이 섞인 값을 **소리 없이 버립니다.**
- 관리자가 붙여넣기 실수로 값 끝에 개행을 넣으면 저장했다고 믿고 넘어가는데 실제로는 아무
  것도 바뀌지 않습니다. `[INFRA-044]` 가 개행 검사를 넣으면서 이 침묵의 범위가 넓어졌습니다.
- 고치는 자리가 라우트 쪽이라 `[INFRA-044]` 와 diff 성격이 달라 별도 항목으로 둡니다.
- QA 시나리오: 값 끝에 개행을 넣어 저장하면 화면이 저장 실패를 알린다
- [ ] `update_env_file` 이 무엇을 버렸는지 돌려주게 함
- [ ] 라우트가 그것을 응답에 싣고 화면이 알리게 함
- [ ] 버려진 키가 응답에 실리는 것을 확인하는 검사 추가
