...
- 설계 승인: 2026-09-23 사용자가 항목 선택 질문에서 「INFRA-083 (Recommended)」를 골랐다(질문에 「선택하시면 그 항목의 설계안을 승인하신 것으로 기록」 명시, 설계안은 미리보기로 제시). bounded, T1(`tests/` 만 변경, 위험 경로 밖). `tests/conftest.py` 에 autouse fixture 로 `monkeypatch.chdir(tmp_path)`, 전체 실행에서 깨지는 저장소 상대 경로 읽기는 `PROJECT_ROOT` 기준으로 고치되 기대값은 바꾸지 않는다, 세션 시작·끝에 `PROJECT_ROOT/data`·`logs` 수정 시각을 비교해 바뀌면 실패. 모듈 기준 절대 경로는 `[INFRA-082]` 소관. 동적 QA 제외(테스트 인프라만 변경). 범위 변경(같은 날 16:58): cwd 이동 후에도 모듈 기준 절대 경로와 import 시점 경로로 원본 쓰기 19개가 남아(테스트별 탐지 12파일 27건) 질문했고, 사용자가 「테스트 쪽에서 막기 (Recommended)」를 골랐다. `tests/` 만 고친다(T1 유지): 세션 시작 때 cwd 를 세션 tmp 로 옮기고, conftest autouse fixture 로 모듈 경로 상수(`BASE_DIR`·`DB_PATH`·`VCP_STATUS` 경로)를 tmp 로 돌리며, `__file__` 로 직접 계산하는 곳은 해당 테스트에서 monkeypatch 한다. 운영 코드는 바꾸지 않는다
- [x] 설계 승인(bounded) - [x] 구현·RED→GREEN(RED: 세션 끝 검사만 넣은 격리 사본 전체 실행에서 `data/`·`logs/` 20개 변경 → cwd 격리만으로 19개 → 모듈 경로 격리와 테스트 세 곳 수정 뒤 0개. 원본 트리 첫 실행에서 검사가 `data/runtime_cache.db` 1개를 잡음(원본에 `update_status.json` 이 있어야 타는 `app/routes/common.py:44` 경로, `test_common_routes_refactor.py::test_system_update_status_returns_debug_fields`) → conftest 에 `UPDATE_STATUS_FILE` 격리 추가. 세션 임시 cwd 잔존 → 세션 끝에서 삭제) - [x] `/ponytail-review`(`if before is None` 가드 삭제, 값이 모두 `BASE_DIR` 인 dict → 튜플, 둘 다 반영) - [x] 정적 검증(원본 트리 `venv/bin/python -m pytest -q -p no:cacheprovider` exit 0, 2692 passed 2 skipped, 전후 `data/`·`logs/` 27,564개 파일 수정 시각 비교 변화 0, `pytest-cwd-*` 잔존 0, `tests/scripts/test_skill_set.py` 6 passed(`.claude/skills/closing-bet-python/SKILL.md` 의 pytest 관례에 새 동작 한 문장 추가). 원본 `data/`·`logs/` 복제 사본에서도 2690 passed 1 failed(사본이 git 저장소가 아닌 기존 gitignore 테스트) 변화 0)
