...
- [x] 설계 승인(2026-09-25 16:47, 대화 「승인」, bounded): 부분 일수 참조를 버린다. 범위: `_reference_reject_reason` 이 다섯 행 가운데 외국인·기관 수량이 하나라도 None 이면 기존 사유 `insufficient_days` 로 거부한다. 공유 판정 함수 한 곳이라 상세 API·점수 경로가 함께 바뀐다. pykrx 참조는 `_safe_int` 로 만들어져 None 이 없으므로 영향은 Toss 참조뿐이다. 캐시된 참조도 조회 뒤 같은 판정을 지나므로 캐시 무효화는 없다. 버린 뒤에는 기존 갈래를 따른다(CSV 가 있으면 플래그 붙은 CSV, 없으면 확정 집계 없음 → 모달은 `[FE-048]` 의 Toss 합계 「(N일)」). 일수를 페이로드로 넘기는 대안은 `days: 5` 계약과 `_score_supply_core` 까지 바뀌어 채택하지 않았다. 티어 T3(§2 위험 경로). 스킬: `.claude/skills/closing-bet-python/`, frontend 변경 없음
- [x] 테스트 먼저(`tests/services/test_investor_trend_5day_service.py`): 외국인·기관 각각 하루 None 이면 `insufficient_days`(단위), 실제 Toss 파서·정규화를 거친 하루 빈 참조가 버려지고 CSV 가 남는지(통합, `discarded_references == ["toss:insufficient_days"]`). 두 건 모두 RED(`None` 반환, `'toss' == 'csv'`). 실제 0 인 날을 유지하는 검사는 기존 `test_reference_reject_reason_keeps_a_payload_whose_sum_cancels_out` 가 맡는다. 첫 RED 실행(16:49)에서 통합 검사가 캐시 토큰을 만들며 `_resolve_pykrx_latest_market_date` 로 실제 pykrx 지수 조회를 불러 **KRX 로그인이 한 번 일어났다**(출력 「KRX 로그인 완료. 로그인 시간: 2026-09-25 16:49:01」). 그 함수를 가짜로 바꿨고 이후 pytest 는 `KRX_ID= KRX_PW=` 로 돌린다. 기존 검사도 같은 경로를 지나 `[INFRA-121]` 로 등록했다
- [x] 구현: `_reference_reject_reason` 의 행 순회에 None 검사 한 줄. 결과 40 passed(`KRX_ID= KRX_PW= venv/bin/python -m pytest -q -p no:cacheprovider tests/services/test_investor_trend_5day_service.py`)
- [x] `/ponytail-review`: Lean already. Ship.(지적 0)
- [x] `closing-bet-reviewer` 리뷰(2026-09-25, CHANGES_REQUIRED, max medium, 코드 결함 0): (1) medium, 설계 기록의 「상세 API·점수 경로가 함께 바뀐다」가 틀렸다. VCP 스크리너의 최신 점수(`engine/screener.py:333` → `engine/screener_supply_helpers.py:224` `_normalize_toss_supply_payload`)는 Toss 를 직접 읽어 이 판정을 지나지 않고, 부분 합계를 점수화하는 것은 `[INFRA-109]` 가 「5일 합계는 값 있는 날의 합」으로 승인한 동작이다. 통합 서비스가 점수에 닿는 곳은 직접 조회 실패 때의 폴백 `_calculate_supply_score_csv`(`engine/screener.py:367`)뿐이다 → 코드 변경 없이 기록 정정. 바뀌는 소비처는 상세 API 와 그 폴백이며, 설계 승인 줄의 「점수 경로」 주장은 이 줄로 정정한다. 스크리너 최신 경로는 `[INFRA-109]` 의 결정으로 두고 새 항목을 만들지 않았다 (2) low, 참조를 버린 뒤 통합 서비스가 None 이면 상세 API 는 레거시 CSV 경로(`services/kr_market_stock_detail_service.py:434-473`, 행 수 검사 없음·`fillna(0)`)로 떨어져 CSV 1~4행 종목은 k일 합계가 표시 없이 나간다. 수정 전에도 같은 자리에 Toss 4일 합계가 표시 없이 나갔으므로 회귀는 아니다 → QA 문서 한계로 기록, `[FLOW-023]` 으로 이월 (3) low(확신도 낮음), 부분 참조는 판정 전에 날짜 토큰으로 캐시되어 그날 내내 버려진다(종전에는 그날 내내 채택) → QA 문서 한계로 기록 (4) low, 범위 밖: CSV `_build_trend_map` 의 `fillna(0)`(`services/investor_trend_5day_service.py:378-379`), pykrx 참조의 `_safe_int` NaN→0(`:709-710`) → `[FLOW-023]` 으로 이월 (5) nit, 주석 「합계는 4일치다」가 두 날 이상 빈 경우를 말하지 않는다 → 「합계가 5일치가 아니다」로 반영
- [x] `/review`(T3, `oh-my-claudecode:code-reviewer`, 2026-09-25) COMMENT, 코드 결함 0: R1 Minor, CSV 가 `stale_csv`·`extreme_abs_total` 이고 pykrx 참조가 없으면 하루 빈 Toss 참조를 버린 뒤 낡거나 손상된 CSV 가 표시 없이 남아 수정 전보다 나빠질 수 있다(QA S-4 900204 가 그 예) → 코드 불변. 퇴화 참조보다 CSV 를 남기는 기존 정책(`test_a_zero_reference_does_not_overwrite_a_stale_but_real_csv`)과 승인된 설계가 허용한 갈래라 QA 한계로 기록하고, `get_investor_trend_5day_for_ticker` docstring 의 「정확한 CSV 가 살아남는다」를 정정, None 으로 돌릴지는 `[FLOW-023]` 설계 질문에 추가 | R2 Nit, 사유 재사용으로 빈도를 가를 수 없음 → 승인된 선택, QA 기록 | R3(확신도 낮음) 장중 오늘 행 수량이 비면 장중 Toss 참조가 모두 버려질 수 있음 → 확인 불가 전제, QA 기록 | 점수 경로 기록을 「target_date 없이 돌 때만」으로 더 좁힘 → QA 문서 반영 | 테스트 docstring 「4일치」 → 반영 | 통합 검사에 `csv_anomaly_flags == ["stale_csv"]` 와 CSV 에 없는 종목 `None` 확인 추가 → 반영(40 passed)
- [x] 정적 검증: `KRX_ID= KRX_PW= venv/bin/python -m pytest -q -p no:cacheprovider` 2855 passed·2 skipped exit 0(리뷰 반영 뒤 재실행, 출력에 「KRX 로그인 완료」 0회). 자격 증명을 비운 이유는 `[INFRA-121]`. frontend 변경이 없어 vitest·tsc 는 돌리지 않았다
- [ ] QA 2단계(격리 사본, 가짜 Toss 참조)
...
- [ ] 설계 승인(빈 칸을 결측으로 읽을지, 레거시 경로에 5행 요구를 넣을지 제거할지, 낡은 CSV 폴백을 어떻게 할지), 테스트, T3 리뷰
