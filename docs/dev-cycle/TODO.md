...
- 설계 승인: 2026-09-22 사용자의 「백로그도 설계해서 진행해」 뒤 네 항목 일괄 설계(bounded, 대화 제시) → AskUserQuestion 「수급 이탈만 SELL, 나머지 HOLD」 선택. 저장 행에는 합산 `score`·수축비·수급만 있고 VCP 원점수가 없으므로 점수 축을 뺀다. 5일·1일 수급이 모두 순매도면 SELL, 그 밖에는 HOLD, 폴백은 BUY 를 내지 않는다. 합산 점수와 수축비는 근거 문장에만 남긴다. 정보 부족 갈래의 문구·신뢰도 55 는 유지한다.
- 파일: `engine/vcp_ai_analyzer_helpers.py`(위험 경로 → T3), `tests/engine/test_vcp_ai_analyzer_helpers_refactor.py`. 구현 계획 `docs/superpowers/plans/2026-09-22-vcp-033-fallback-verdict.md` 를 남기고 critic 검토 뒤 구현. QA 는 파싱 실패를 주입한 하네스(실패 경로).
- [x] 설계 승인(bounded)
- [x] 구현 계획·계획 검토: `docs/superpowers/plans/2026-09-22-vcp-033-fallback-verdict.md`. oh-my-claudecode:critic(`vcp033-plan-critic`) 판정 REVISE(원문: 「설계 결정과 구현 코드 자체에는 문제를 찾지 못했습니다 … 계획이 지시하는 테스트 변경 목록이 BUY 를 고정한 검사 한 건을 빠뜨렸고, 실측 하네스가 SELL 경로를 한 번도 지나가지 않는다는 사실이 계획에 반영되어 있지 않습니다」). MAJOR 1(`changes_by_signal_state` 대체 명시)·MAJOR 2(합성 순매도 행을 필수 시나리오로)·MINOR 3(「보수적으로 SELL」 문구 → action 별 맺음 문장)·MINOR 4(0.5 상수화 + `≤ 0.5` 검사)·MINOR 5(5일 음수·1일 0 경계 검사)·MINOR 6(수축비만 있어도 상태 문장)·MINOR 7(수축비 0 이하는 결측)·MINOR 8(하네스의 `None` 수급·ASSERT 스위치·사본 범위) 전부 반영해 계획을 고침. 미반영 없음
- [x] 구현·RED→GREEN: 계획 Task 1 Step 1 대로 검사를 바꾼 뒤 예상한 7건(`never_buys…`·`holds_low_composite…`·`treats_flat_one_day…`·`describes_contraction_without_score`·`partial_sell…`·`complete_data…`·analyzer `all_parsing_fails`)이 실패하고 23건 통과함을 확인, 구현 뒤 두 파일 81 통과. 대체한 검사: `changes_by_signal_state`(폴백 BUY 고정) → `never_buys_and_sells_only_on_outflows`(같은 입력), `preserves_real_zero_and_complete_verdicts` → `complete_data_verdicts_ignore_score`, analyzer `all_parsing_fails` 의 BUY 기대 → HOLD·55
- [x] `/ponytail-review`(인라인): 두 갈래를 한 흐름으로 합쳐 함수가 -34줄. 리뷰 시점의 모듈 상수(`_STRONG_CONTRACTION_RATIO`, 계획 critic MINOR 4 반영)는 뒤에 심층 리뷰 MINOR 2 로 `VCPThresholds.STRONG_CONTRACTION_RATIO` 에 흡수되어 사라짐. 더 줄일 것 없음
- [x] `/code-review`(feature-dev:code-reviewer `vcp033-reviewer`): APPROVE, 블로커 0. `ai_action` 소비처 16개 파일에서 BUY 특별 취급 없음(멤버십 검사만)을 확인. 낮은 확신도 3건: 수축비 0 이하를 결측으로 보는 규칙이 진짜 0 을 흡수(critic MINOR 7 로 검토된 트레이드오프, 미반영), 「약화」 갈래가 저장 행에서는 도달 불가(헬퍼의 일반성, 미반영), QA 미실행 지적(리뷰 시점 기준, 이후 실행)
- [x] `/review`(oh-my-claudecode:critic `vcp033-deep-review`, 경계값 직접 탐침 포함): REVISE(원문: 「판정 로직 자체에는 결함을 찾지 못했습니다. 다만 이 설계를 정당화한 전제 하나가 사실과 다르고, 계획이 스스로 요구한 경계 검사 두 개가 빠져 있습니다」). MAJOR 1(「`vcp_score` 열이 없다」 전제 오류: `scripts/init_data.py:1400` 이 0~10 양자화 값을 싣고 `build_vcp_prompt` 도 읽음 → 계획·QA 문서 정정, 판정 축은 영향 없음) · MINOR 2(0.5 문턱이 `engine/vcp.py:164` 리터럴과 중복 → `VCPThresholds.STRONG_CONTRACTION_RATIO` 추가, `engine/vcp.py` 두 리터럴과 helper 가 상수를 읽음) · MINOR 3(경계 0.5·0.7·음수 미고정 → 매개변수 검사 5벌 + 음수 검사 추가) · MINOR 4(0.4952·0.5027 이 둘 다 「0.50」 으로 보이며 서술이 갈림 → 소수 둘째 자리 반올림 값으로 판정) 반영, MINOR 5(종목명 480자 초과 시 600자 절단, 정보)는 미반영. 반영 뒤 재확인 ACCEPT(돌연변이 `<=`→`<` 로 경계 검사 4건 실패 확인, 경계 주변 20개 값으로 표시·서술 일치 탐침, 결측→반올림 순서 확인, `tests/engine` 664 통과). 확인된 무결점: None/NaN/inf/문자열 수급, 0·-0.0 수급, 1일 결측, 신뢰도 55 의 하위 문턱 없음, BUY 제거의 파급 없음(소비처는 멤버십만), 순환 import 없음
- [x] pytest 전체 2595 통과 2 skipped → 심층 리뷰 반영 뒤 2601 통과 2 skipped. `pytest tests/engine -k vcp` + `tests/test_vcp.py` 129 통과(`engine/vcp.py` 상수 치환 회귀 없음). vitest 전체는 frontend 무변경이라 `[VCP-032]` 의 exit 0 유지
- [ ] QA(하네스): `docs/dev-cycle/qa/VCP-033.md` (S-1~S-4 실행 완료, 필수 2/2·인접 2/2 통과. 아카이브 커밋에서 마감)
