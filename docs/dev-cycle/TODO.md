...
- 누적 성과 화면(`frontend/src/app/dashboard/kr/cumulative/CumulativeClientPage.tsx`)의
  S·A·B 등급 카드 세 개가 표에 그려진 50행만으로 건수와 승률과 평균 수익률을 다시
  계산합니다. 백엔드는 같은 응답의 `kpi.roiByGrade` 에 전체 기간 값을 이미 보내고 있으며,
  같은 화면 위쪽의 `누적 수익률 (S+A+B 합산)` 카드는 그 값을 제대로 쓰고 있습니다.
- 페이지를 넘기면 같은 자리의 숫자가 통째로 바뀝니다. S 등급은 1페이지에서 `7건 / -3%`,
  2페이지에서 `3건 / +4.3%` 로 나오지만 전체 기간 값은 `16건 / +0.25%` 입니다. 부호가
  뒤집히며, 그 잘못된 부호에 근거해 "진입을 자제하거나 비중을 축소하고 관망하는 것이
  좋습니다" 라는 조언 문구까지 함께 표시됩니다.
- 실측 증거는 `.gstack/qa-reports/qa-report-localhost-3500-2026-09-03.md` 의 ISSUE-001 과
  `screenshots/issue-001-page1.png`, `screenshots/issue-001-page2.png` 에 있습니다.
- [ ] 등급 카드의 집계 원천을 화면의 현재 페이지에서 `kpi.roiByGrade` 로 바꿈
- [ ] 승률과 성공/실패 건수도 같은 원천에서 나오도록 맞춤. 응답에 없으면 백엔드에서 함께 보냄
- [ ] 페이지를 넘겨도 등급 카드가 바뀌지 않는 것을 vitest 로 고정
