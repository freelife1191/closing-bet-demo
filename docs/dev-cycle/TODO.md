...
- 설계 승인: bounded(2026-09-23 23:34 사용자 AskUserQuestion 「앞말 제외 목록 (Recommended)」). 짧은 이름 바로 앞에 「분석·투자·관심·매수·매도·편입·검토·적용·지원」 같은 수식 명사가 오면 잡지 않는다. 이름 자체가 일반 명사인 경우(「진도 어때」·「노을 어때」)는 `ponytail:` 주석으로 한계를 남긴다
- [x] 설계 승인(bounded) - [x] 구현·RED→GREEN(구현 전 1 failed) - [x] `/ponytail-review`(oh-my-claudecode:code-reviewer 레인, 「Lean already. Ship.」) → `closing-bet-reviewer`(CHANGES_REQUIRED max medium: 발견 1 테스트 변이 생존 → 양성 2·음성 1 추가로 `$` 제거·첫 매칭만·편입 제거 변이 모두 검출, 발견 2·3 은 QA 문서 한계로 기록) - [ ] QA
