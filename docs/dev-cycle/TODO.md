...
- 설계 승인: 승인 일자 2026-09-23 | 승인 확인 시각 2026-09-23 22:41 | 범위: `chatbot/stock_query_service.py` 에서 두 글자 이하 종목명은 바로 뒤(조사 0~1자, 공백 허용)에 종목 질의 신호어(`주가|주식|어때|전망|차트|실적|수급|시세|매수|매도|목표가|배당`)가 오거나 메시지 전체가 그 이름일 때만 잡고, 6자리 티커 뒤에 `원` 이 오면 티커로 보지 않는다. 신호어 없는 짧은 이름 질문은 문맥이 빠지는 쪽을 받아들인다 | 근거: 대화에서 bounded 설계 제시 후 AskUserQuestion 「승인 (Recommended)」. 티어 T2(위험 경로 아님)
- [x] 구현·RED→GREEN: 새 테스트 3개 가운데 일반 단어 음성 사례와 「원」 가격 사례 2개가 구현 전 실패, 신호어 양성 사례 1개는 회귀 방지로 통과. 구현 뒤 `tests/chatbot` 283 통과
- [x] `/ponytail-review`(oh-my-claudecode:code-reviewer 레인) 「Lean already. Ship.」, 쓰이지 않던 테스트 맵 항목 하나 삭제. `closing-bet-reviewer` APPROVE(max medium): 3(이름 앞 경계를 고정하는 테스트 없음, lookbehind 제거 변이가 통과) → 「현대기아 주가」 음성 사례 추가, 같은 변이가 이제 실패함을 확인. 4(「298000 원」 공백) → `(?!\d|\s*원)` 으로 반영하고 사례 추가. 1(「주식·매수·매도」 가 일반 명사 뒤에 붙어 「분석 대상 주식」 류 오탐이 남음)·2(신호어 뒤 경계 없음, 「주가지수」) 는 신호어 목록을 좁히려면 설계 재승인이 필요하므로 `[CHAT-043]` 으로 이월. 5(대소문자·줄바꿈·호출자 계약) 는 정보
- [x] 정적 검증: `venv/bin/python -m pytest -q -p no:cacheprovider` 2725 passed 2 skipped(exit 0). frontend 변경 없음
- [ ] QA(`docs/dev-cycle/qa/CHAT-038.md`, 격리 58120·58121, 가짜 LLM 이 받은 프롬프트의 종목 문맥을 답변에 싣는다)
