...
- 상태: RESUME — 구현·리뷰·정적 검증 완료, 첫 커밋 뒤 최종 QA 실행
- 설계 승인: 2026-09-08 현재 대화 사용자 「진행해」 | 로그인 전용·legacy 보존·개인 모의자금 1억 원
- 설계: docs/superpowers/specs/2026-09-08-infra-060-owner-isolation-design.md
- 계획: docs/superpowers/plans/2026-09-08-infra-060-owner-isolation.md (critic 수정 후 OKAY)
- 검증: docs/dev-cycle/qa/INFRA-060.md
- 검증 전제: baseline에서도 실패한 5개 테스트의 Vertex 구성/모듈 모킹을 현행화하며 기존 업무 기대값은 유지한다. 제품 코드 변경 없이 외부 의존성을 격리한다.
...
- [x] 기존 전역 자료는 예약 legacy 소유자로 보존하고 일반 계정에서 접근하지 않음
- [x] 저장 계층에 소유자 식별자 도입
- [x] GET 3개·POST 5개가 자기 계정에만 닿는지 검사 추가
