...
- 수정 방향 둘을 리뷰가 제시했습니다. (a) `update_env_file` 이 값을 작은따옴표로 감싸고
  내부 작은따옴표를 이스케이프해 쓴다. (b) 기동 스크립트가 `.env` 를 `source` 하지 않고
  순수 `KEY=VALUE` 파서로 읽는다. 리뷰는 (b) 가 더 근본적이라고 봤습니다. **둘 다 필요할
  수 있으므로 설계에서 정합니다.**
- (a) 를 택하면 `frontend/.env` 가 루트 `.env` 의 심볼릭 링크(`restart_all.sh:17`)이므로
  python-dotenv·bash·Next.js 세 파서가 같은 파일을 읽습니다. 따옴표 형식을 셋이 같게
  읽는지 먼저 확인해야 하고, 기존의 따옴표 없는 값과 혼재할 때도 봐야 합니다.
