...
- 설계 승인: 2026-09-23 23:43 사용자 승인(bounded, 대화에서 제시, AskUserQuestion 응답 「디렉터리 0700 (Recommended)」). 티어 T2(위험 경로 없음).
- 실측(값은 가리고 건수만): `logs/backend.log` 에 `engine.messenger_senders` 의 「이메일 발송 성공: <구독자 이메일>」 2줄. `critical_errors.log` 는 예외 메시지·traceback. `frontend.log` 의 IP 형태 문자열은 리슨 주소뿐. `gunicorn-access.log` 는 운영 경로가 만들지 않는 `[CHAT-022]` QA 산출물(`restart_all.sh` 에 `--access-logfile` 없음).
- 승인 범위: `restart_all.sh` 의 `mkdir -p logs` 다음에 `chmod 700 logs`(디렉터리가 막으므로 파일 모드와 무관하게 다른 계정이 열 수 없다), 개인정보처리방침 10항 「활동 로그와 데이터베이스 파일은」→「로그 파일과 데이터베이스 파일은」, `tests/scripts/test_service_lifecycle.py` 의 restart 픽스처에 모드 단언. 파일별 0600 과 `python flask_app.py` 직접 실행 경로는 제외.
- 스킬: `.claude/skills/closing-bet-python/`(테스트), `.claude/skills/closing-bet-nextjs/`·`frontend-skills.md` §2(방침 페이지 문구), `.claude/skills/closing-bet-verify/`
- [x] 설계 승인(bounded)
- [x] 구현·RED→GREEN: 새 테스트 `test_restart_narrows_logs_dir_to_the_server_account` 가 구현 전 실패(0755 그대로), `chmod 700 logs` 뒤 통과. `legal-pages.test.tsx:68` 이 옛 방침 문구를 고정하고 있어 vitest 1건이 실패했고 새 문구로 단언과 주석을 고침(의도한 문구 변경).
- [x] `/ponytail-review`(oh-my-claudecode:code-reviewer 레인): 「Lean already. Ship.」(`mkdir -m 700` 은 기존 디렉터리를 좁히지 못해 합칠 수 없음).
- [x] `/code-review`(closing-bet-reviewer): APPROVE, 최고 low. F3(주석의 「umask(0644)」 부정확)·F4(chmod 실패 시 ❌ 안내 없음) 반영. F1(restart 를 거치지 않는 직접 실행·유닛 재생성 때 `ActivityLogger` 의 `os.makedirs` 가 logs/ 를 0755 로 만듦)은 승인 범위 밖이라 `[INFRA-085]` 로 이월. F2(Caddy 접근 로그는 logs/ 밖이나 caddy 계정 소유 0600 으로 문서화됨) 는 문구 유지. 리뷰 해시는 `legal-pages.test.tsx` 수정과 F3·F4 반영 전 기준이며, 두 변경은 문구·주석·안내 한 줄이라 재리뷰하지 않음.
- 정적 검증: 전체 `pytest -q -p no:cacheprovider` 2726 passed 2 skipped(exit 0, F3·F4 전), F3·F4 뒤 `tests/scripts/test_service_lifecycle.py` 22 passed, `bash -n restart_all.sh` 통과. `npm run type-check` 통과, `npx vitest run` 97 파일 682 passed.
- [ ] QA
