...
- **`[INFRA-061]` 이후에 착수합니다.** 순서를 뒤집으면 없던 노출을 스스로 만듭니다.
  종전에는 `[INFRA-042]` 를 선행 조건으로 적었으나, 그 라운드가 이 라우트를 부류 B 로
  이월하고 `[INFRA-061]` 이 그 인가를 맡게 되면서 조건이 옮겨졌습니다. `config/interval` 의
  저장이 실제로 동작하게 만드는 것은 그 라우트가 닫힌 뒤입니다.
