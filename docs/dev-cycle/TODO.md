...
- [x] 화면 제목을 `h1` 으로 올리고 그 아래 계층을 한 단계씩 맞춤
- [x] 빵부스러기를 `nav` + `ol`/`li` 로 감싸고 마지막 항목에 `aria-current="page"` 추가
- [x] 대시보드의 다른 화면에도 같은 구조가 쓰이는지 확인하고 함께 정리
- [x] 제목 계층과 빵부스러기 구조를 vitest 로 고정

- 설계 경로: bounded | 실행 티어: T2 (FE-034·FE-040 공유 검토, 위험 경로 없음)
- 승인 근거: 직전 대화의 제목·탐색 경로 및 모바일 표 4곳 설계 제안 이후 사용자가 2026-09-09 다음 라운드를 묶어 연속 진행하도록 요청. 동일 범위 구현·리뷰·검증·커밋 재개.
- 범위: Header·종가베팅 제목 계층 및 누적성과·모의투자·종목거래이력 빈 표 안내, 해당 회귀 테스트.
- [x] 공유 ponytail → code-reviewer·architect 독립 리뷰 (각 15분 상한), 전체 pytest·Vitest·type-check·lint
- [ ] UltraQA App 대응 행렬·실제 agent-browser 모바일 375×812 및 데스크톱 검증·소유 fixture 정리 (60분 상한)

- 리뷰 증거: evidence/accessibility-batch-20260909 (ponytail APPROVE, code-review APPROVE, architect CLEAR;8파일 SHA 일치). 정적검증 pytest2281/2skip·Vitest446·typecheck0·lint0오류199경고.
...
- [x] 네 자리를 스크롤 영역 밖으로 옮김
- [x] 공용 컴포넌트로 모을지 판단하고 근거를 남김
