...
### [CHAT-036] 챗봇 스트림의 500 폴백 누락·프록시 30초 절단·평문 오류 노출
- 카테고리: 챗봇 | 티어: T2 | 근거: 2026-09-22 사용자 보고(「⚠️ 오류가 발생했습니다: Unexpected token 'I', "Internal S"... is not valid JSON」과 「⚠️ 스트리밍 응답 처리 오류: 500 INTERNAL」). ① `chatbot/response_flow_errors.py` 의 `is_retryable_stream_error` 가 503·429 만 보아 Gemini 의 500 INTERNAL 이 폴백 체인(모델 넷)을 타지 못하고 `response_flow_fallback.py` 가 즉시 `raise` 해 원문이 그대로 화면에 뜬다. `engine/llm_analyzer_retry.py` 의 `RetryConfig.RETRY_CONDITIONS` 는 500·502 를 이미 재시도 대상으로 보므로 같은 오류를 두 판정기가 다르게 본다. ② `friendly_error_message` 가 `"400" in error_msg` 같은 부분 문자열로 판정해 세부 문구에 낀 숫자에 오분류되고, Vertex 인증 실패(401·403)와 5xx 는 매핑이 없다. ③ Next 의 `/api/*` rewrite 는 `httpxy` 에 기본 30초 소켓 무활동 제한을 건다(`frontend/node_modules/next/dist/server/lib/router-utils/proxy-request.js:37`). 챗봇이 문맥을 모으고 Gemini 첫 토큰을 기다리는 동안 한 바이트도 흐르지 않으면 upstream 소켓을 끊고 `res.end('Internal Server Error')` 평문 500 을 보낸다. ④ `frontend/src/app/chatbot/useChatStream.ts` 와 `frontend/src/app/components/ChatWidget.tsx` 가 `text/event-stream` 이 아니면 상태 코드를 보지 않고 `res.json()` 을 불러 SyntaxError 문구가 그대로 노출되고, 스트림이 `done` 없이 끊기면 잘린 답변이 아무 표시 없이 남는다. 인용된 운영 로그는 이 기기의 `logs/` 에 없어 원인은 코드로 확정했다.
- 설계 승인: 승인 일자 2026-09-22 | 승인 확인 시각 2026-09-22 22:05 | 범위: 폴백 판정을 `RetryConfig.is_retryable_error` 로 통일하고 `is_retryable_stream_error` 삭제, 맨 앞 상태 코드로 판정하는 `friendly_error_message`(429 한도·401/403 인증·5xx 일시 장애), `next.config.js` 의 `experimental.proxyTimeout` 120초(gunicorn `--timeout 120` 과 동일, SSE 하트비트는 침묵이 그 값에 가까워질 때로 보류), `chatHelpers.ts` 의 `readChatJsonResponse` 공용 함수와 절단 안내 문구를 두 화면에 적용, 회귀 테스트 | 실제 대화 근거: 2026-09-22 사용자 「꼼꼼히 파악해서 원인 파악해서 문제 해결해줘 추가로 또 확인되는 문제들도 잘 파악해서 수정해줘」와 AskUserQuestion 「Next 프록시 30초 절단은 어느 깊이로 막을까요?」에 「proxyTimeout 120초만 (Recommended)」 선택
- QA 시나리오: 격리 /chatbot 에서 첫 모델이 500 INTERNAL 을 내는 가짜 Gemini 클라이언트로 질문을 보내면 둘째 모델의 답변이 스트리밍으로 도착한다. 첫 토큰이 35초 늦어도 답변이 도착한다. 백엔드 자리에 평문 500 스텁을 두면 「서버 응답을 받지 못했습니다 (HTTP 500)」 문구가 뜬다. 브라우저 실측 required.
- [x] 회귀 테스트 RED: pytest 5건(`test_response_flow.py` 4, `test_chat_execution.py` 1)·vitest 6건(`chatHelpers.test.ts` 4, `useChatStream.regression-chat-036.test.tsx` 2) 실패 확인
- [x] 구현 GREEN: 같은 검사 통과, `tsc --noEmit` 통과. 타입 좁히기가 콜백 안에서 풀려 `responseText` 로 먼저 잡음
- [x] `/ponytail-review`: 지적 2건(`usage_metadata` 미사용 필드 삭제, 한 곳에서만 부르는 `_leading_status_code` 헬퍼·모듈 상수를 `re.match` 두 줄로) 모두 반영, net -5줄
- [x] `/code-review`(`feature-dev:code-reviewer`, `chat036-reviewer`): verdict APPROVE. 유효 지적 1건 반영: 폴백 모델 전부 소진 경로의 `_build_stream_fallback_error` 가 원문 JSON 을 「(상세: …)」 로 노출하고 그 안의 `friendly_error_message` 분기는 도달 불가 → 함수를 지우고 호출부에서 마지막 오류를 `friendly_error_message` 로 분류(5xx 는 일시 장애, 코드 없으면 접두사 + 원문). 테스트 공백 지적(사용자 중단·세션 전환에 끊김 안내가 붙지 않음) → 중단 검사 1건 추가. 부분 문자열 오탐 가능성은 engine 의 기존 패턴이라 새 결함 아님으로 기록
- [ ] pytest·vitest·type-check·lint - [ ] QA 계획·실행 - [ ] 아카이브
