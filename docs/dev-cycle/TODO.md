...
- 카테고리: 인프라 | 티어: 문서 | 근거: 2026-09-07 사용자 요청 「전체적으로 검토해서 제거해야될 건 제거하고 진행해야될 사항들 확인해서 todo, 백로그 현행화해줘」
- 설계 승인: 승인 일자 2026-09-07 | 승인 확인 시각 2026-09-07 15:20
  | 범위: 근거를 확인한 제거 9건, 병합 26건, 내용 정정 3건, P1 순서 조정(보안 2건을 맨 앞으로)
  | 실제 대화 근거: 2026-09-07 AskUserQuestion 「적용 범위」에 「1·3·4 만 적용」, 「P1 순서」에 「보안 2건을 맨 앞으로」 응답
- 101개 가운데 감사에서 온 것은 6개, 옛 계획 문서 이관이 2개이고 나머지 93개는 라운드마다 붙은
  리뷰·QA 의 범위 밖 발견입니다. 발견 하나가 항목 하나가 되면서 같은 파일·같은 원인이 여러 ID 로
  쪼개졌습니다. 제거·병합 목록과 사유는 `archive/2026-09.md` 의 「백로그 정리」 절에 남깁니다.
- [x] 101개 항목을 전부 읽고 코드·자료·로그와 대조
- [x] 제거 9건·병합 26건·정정 3건의 목록을 사용자에게 제시하고 승인
- [x] TODO 반영과 남은 항목의 교차 참조 확인 — 제거·흡수된 33개 ID 를 남은 항목이 「흡수」 문맥 밖에서 참조하는 곳 0건
- [ ] 제거·병합 목록을 월별 아카이브에 기록
