...
- [x] `_normalize_confidence_value` 와 `safe_confidence` 를 한 벌로 합친다 — 95cab19 / VCP 묶음QA
- [x] `getRec` 의 원시 캐시 후보가 병합 결과와 같은 규칙을 쓰는지 확인한다 — 95cab19 / VCP 묶음QA
- [x] 0~1 척도로 확신도를 저장하는 생산자가 실제로 있는지 확인한다 — 95cab19 / VCP 묶음QA
- [x] 같은 파일의 `inf` 구멍 두 자리를 함께 막는다 — 95cab19 / VCP 묶음QA
...
- [x] 세 탭의 게이지를 같은 자료로 실측해 대조한다 — 95cab19 / VCP 묶음QA
