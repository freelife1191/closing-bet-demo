...
- 카테고리: 인프라 | 티어: 문서 | 근거: 2026-09-01 사용자 지시 "커밋은 물어보지말고 해"
- 지시와 규약이 어긋난 상태입니다. `SKILL.md` 의 [4] 마감 4번과 `CLAUDE.md` 의 "승인
  게이트는 두 곳입니다" 는 여전히 커밋 전 승인을 요구합니다. 고치지 않으면 다음 세션이
  규약을 읽고 다시 물어봅니다.
- 게이트 1(계획 확정)은 그대로 둡니다. 없애는 것은 게이트 2 하나입니다.
- [4] 마감의 보고 자체는 없애지 않습니다. 커밋 전에 승인을 기다리는 대신 커밋 후에
  같은 내용을 보고하는 형태로 바꿉니다. 반영하지 않은 리뷰 지적과 검증 결과가 기록에
  남아야 하기 때문입니다.
- 2026-09-01 grep 결과 `게이트 2` 를 부르는 자리는 다섯 곳입니다.
- [ ] `SKILL.md:95` [4] 마감 4번의 승인 대기를 없애고 3·5번의 보고 시점을 커밋 뒤로 이동
- [ ] `SKILL.md:74` [2] 구현 4번의 "반영하지 않기로 한 지적은 게이트 2 에서 함께
      보고한다" 를 새 보고 시점에 맞춰 수정
- [ ] `SKILL.md` 규칙 절의 "게이트를 건너뛰지 않는다. 사소해 보이는 항목도 두 게이트를
      모두 거친다" 를 게이트 1 기준으로 수정
- [ ] `CLAUDE.md:231` 의 "승인 게이트는 두 곳입니다" 문장 교체
- [ ] `tier-rules.md:164` (§4 목록 갱신)의 게이트 2 참조를 커밋 시점 표현으로 정정
- [ ] `archive-format.md:128` (§7 기록 시점)의 "게이트 2 를 통과한 직후" 를 정정
- [ ] `tier-rules.md:23` 은 `qa` 스킬을 배제하는 근거로 게이트 2 를 듭니다. 게이트가
      하나가 되면 근거를 "사용자가 검토하기 전에 스스로 커밋한다" 로 바꿔야 하는지
      판단하고 필요하면 정정
