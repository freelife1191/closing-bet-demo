...
### [INFRA-090] 일별 가격 수집이 평일 휴장일을 전 종목 0원 행으로 저장한다
- 카테고리: 인프라 | 티어: T3(위험 경로 `scripts/init_data.py`) | 근거: `[INFRA-088]`·`[INFRA-089]` 실측 중 발견(2026-09-24, 기록 `qa/INFRA-088.md` 「휴장일 0원 행」). KRX 로그인 상태에서 `stock.get_market_ohlcv("20260924", market="ALL")`(추석 휴장일)이 빈 결과가 아니라 2,870행을 돌려줬다. pykrx 1.2.9 는 휴장일에 시가·고가·저가·종가가 모두 0 인 행을 주고 직전 영업일로 바꾸는 것은 `alternative=True` 일 때뿐이다. `create_daily_prices`(`scripts/init_data.py:972-1044`, 주말 `:981`, 빈 결과 `:999`)는 주말만 건너뛰고 `df.empty` 만 보므로 그 행을 저장한다. 로컬 원본 `daily_prices.csv` 에 2026-02-16·17·18(설 연휴)이 각 2,882종목 종가 0 으로 들어 있다
- 급한 이유: 다음 수집 구간은 마지막 저장일 다음 날부터이므로, 추석 연휴 뒤 첫 거래일 17:00 실행에서 pykrx 가 정상이면 09-24 이후의 평일 휴장일이 0원 행으로 저장된다. 이미 운영 파일에 들어 있는 과거 휴장일 행이 있는지는 운영자 확인이 필요하다
- 영향 미확인: 0원 행이 VCP 스크리너·수급 점수·차트에 어떻게 들어가는지는 아직 보지 않았다
- 설계 승인: 승인 일자 2026-09-24 | 승인 확인 시각 2026-09-24 10:17
  | 범위: 종가가 전 종목 0 인 날짜를 휴장일로 보고 건너뜀(거래정지 종목 보존), 기존 파일의 0원 날짜는 읽을 때 빼서 다음 저장 때 정리, 휴장일만 있는 구간은 yfinance 폴백 생략. `fetch_prices_yfinance`·`[INFRA-088]` 범위 제외
  | 실제 대화 근거: 2026-09-24 사용자 선택 「저장 시 자동 정리 (권장)」, 현재 세션의 대화 설계. 폴백 생략은 설계를 제시한 뒤 계획에서 더한 경계 처리다
- [x] 설계 승인(휴장일 판정 기준, 과거 0원 행 정리 여부). 계획 `docs/superpowers/plans/2026-09-24-infra-090-holiday-zero-rows.md`
- [x] 계획 검토 `oh-my-claudecode:critic` REVISE: 혼합 구간 대조, 테스트 구간 고정, 정상일 종가 0 종목, 제거 위치(`:907` 앞), `STOP_REQUESTED` 시 종전 경로, 로그 문구, `df is None` 제거. 모두 반영(계획 「계획 검토 반영」 절)
- [x] 테스트 먼저(가짜 pykrx, 수정 전 5건 실패 확인), 구현(`_all_zero_close_dates`, 기존 파일 로드 직후 제외, 루프 건너뜀, 휴장일만 있는 구간 폴백 생략)
- [x] 과잉설계 리뷰 `/ponytail-review`: Lean already(줄 수가 줄지 않는 `df.empty` 조건 하나는 유지)
- [x] 코드 리뷰 `closing-bet-reviewer` APPROVE(max low): 중단 조건 테스트 추가(조건을 지우면 실패함을 변이로 확인), 변수명 `l` → `lo` 반영. 숫자가 아닌 종가를 0 으로 보는 선택과 「전부 0」 가정은 기록만(`qa/INFRA-090.md`)
- [x] 심층 리뷰 `/review`(oh-my-claudecode:code-reviewer) 차단 결함 없음: L2(재조회 확인, 전체 0원 파일 테스트)·L3(헬퍼에서 `STOP_REQUESTED` 고정) 반영. M1 은 `[INFRA-089]` 로 이월, L1·L4 는 기록만
- [x] 정적 검증: `venv/bin/python -m pytest -q` 2,717 passed·2 skipped(exit 0, 리뷰 반영 전), 반영 뒤 `venv/bin/python -m pytest tests/scripts -q` 160 passed
- [ ] QA 실행(`qa/INFRA-090.md` S-1 은 KRX 로그인 승인 필요, S-2·S-3)
- [ ] 운영자 확인: 운영 `daily_prices.csv` 에 종가 0 비율이 높은 날짜가 있는지(100% 가 아닌 날짜가 있는지도). 이미 09-24 0원 행이 저장돼 있으면 연휴가 끝나고 첫 저장 전까지 VCP·종가베팅이 그 행을 읽는다
