...
### [INFRA-059] 무인증으로 남은 라우트 셋이 갱신 파이프라인과 서버 설정에 닿는다
- 카테고리: 인프라 | 티어: T3(인가 경계, `.env` 접촉) | 근거: 2026-09-08 `[INFRA-042]`
  사이클의 적대적 리뷰(F1)와 보안 리뷰. 셋을 한 항목으로 묶으라는 것은 보안 리뷰의 권고이며,
  **셋으로 쪼개면 같은 분석을 세 번 다시 하게 된다**는 것이 그 이유입니다.
- 대상은 `POST /api/kr/refresh`, `POST /api/kr/config/interval`,
  `POST /api/kr/market-gate/update` 셋입니다. `[INFRA-042]` 가 부류 B 로 미룬 것들이며
  셋 다 **같은 판단(누가 이 동작을 할 수 있는가)과 같은 수단(데코레이터 한 줄, 다만 화면
  결정이 선행)** 을 공유합니다.
- **`/api/kr/refresh` 가 가장 급합니다.** 게이트가 없는데
  `_resolve_common_update_handlers()` 로 `[INFRA-042]` 가 닫은 `/init-data`·`/start-update`
  와 **같은 핸들러와 같은 상태 파일**에 닿습니다. 익명 요청이 LLM 호출 비용을 유발하고
  `isRunning` 을 세워 관리자 전용으로 만든 자리를 거꾸로 잠급니다. 잠근 문 옆에 열린 문이
  남은 모양입니다.
- `/api/kr/config/interval` 은 단순 설정 변경이 아니라
...
- **회귀는 아닙니다.** 셋 다 `[INFRA-042]` 전에도 후에도 똑같이 열려 있었습니다. 익명 쪽
  능력은 그대로이고 관리자 쪽 능력만 좁아졌으므로, 판정은 「새 구멍이 생겼다」가 아니라
  「기존 위험이 그대로 남았다」입니다. 보안 리뷰가 그렇게 판단했습니다.
- **가장 신경 쓰이던 갈래는 이미 닫혔습니다.** 보안 리뷰가 이 묶음에서 유일하게 데이터
  정합성 문제로 지목한 것은 `/finish-update` 였습니다. 실제 스레드가 도는 중에 익명이 그것을
  부르면 상태 파일만 「완료」가 되고 스레드는 계속 도는데, 그 상태에서 다음 시작이 허용되어
  **같은 CSV 를 두 파이프라인이 동시에 쓸 수 있었습니다.** `[INFRA-042]` 가 닫았습니다.
  남은 셋의 위험은 익명 LLM 비용과 성가심 수준의 작업 방해입니다.
- **분류 기준을 바꿉니다.** `[INFRA-042]` 는 라우트를 「화면이 그 버튼을 누구에게 보이는가」로
  갈랐는데, 그 기준은 화면에 없더라도 서버에서 같은 파이프라인이나 같은 잠금에 닿는 형제
  라우트를 세지 못합니다. 이 항목은 **핸들러와 상태 파일을 기준으로** 호출 그래프를 훑습니다.
- **화면 결정이 선행합니다.** `frontend/src/app/dashboard/kr/page.tsx` 가 `useAdmin` 을
  import 조차 하지 않아 세 버튼이 일반 사용자에게 보입니다. 게이트를 세우면 403 을 받으므로
...
- **`[INFRA-059]` 이후에 착수합니다.** 순서를 뒤집으면 없던 노출을 스스로 만듭니다.
  종전에는 `[INFRA-042]` 를 선행 조건으로 적었으나, 그 라운드가 이 라우트를 부류 B 로
  이월하고 `[INFRA-059]` 가 그 인가를 맡게 되면서 조건이 옮겨졌습니다. `config/interval` 의
