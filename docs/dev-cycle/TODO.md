...
- 카테고리: 종가베팅 | 티어: T3 (등록 시 T1, 위험 경로로 상향) | 근거: 2026-09-22 저장 경로 재점검. `engine/generator.py:170` 의 `run_screener` 가 `save_result_to_json` 으로 일자 파일과 최신 파일을 `atomic_write_text` 로 이미 쓰는데, 스케줄러 진입 함수 `scripts/init_data.py:1685-1687` 의 `create_jongga_v2_latest` 가 같은 내용을 `open(..., 'w')` 로 최신 파일에 다시 쓴다. `services/kr_market_route_service.py:103` 과 `services/kr_market_jongga_runtime_service.py:128` 도 `run_screener` 뒤에 `save_result_to_json` 을 한 번 더 부른다. 두 번째 쓰기 도중 프로세스가 죽으면 최신 파일만 잘린 채 남고, `kr_market_data_cache_core.py:229` 의 `json.load` 가 예외를 내므로 「최신」 조회가 다음 저장 때까지 실패한다. 일자 파일은 온전해 이력은 잃지 않는다.
- 범위: 중복 쓰기 세 곳을 지운다. `create_jongga_v2_latest` 가 파일을 직접 열지 않는 것을 고정하는 회귀 테스트 한 건.
- 티어 정정: `scripts/init_data.py` 가 위험 경로(스케줄러와 데이터 적재)이므로 T1 이 아니라 T3 이다. 계획 문서·계획 검토·`/ponytail-review` → `/code-review` → `/review` 를 거친다.
- 설계 승인: 승인 일자 2026-09-23 | 승인 확인 시각 2026-09-23 14:00 | 범위: `scripts/init_data.py` 의 최신 파일 직접 쓰기 제거(반환 계약 유지), `services/kr_market_route_service.py`·`services/kr_market_jongga_runtime_service.py` 의 두 번째 `save_result_to_json` 호출과 import 제거, 회귀 테스트 두 건(init_data 한 건 + 두 서비스 매개변수화 한 건), 격리 사본 QA(가짜 `run_screener` + 브라우저 확인) | 근거: 대화에서 bounded 설계를 제시하고 사용자가 「응 진행해」로 승인
- 계획: `docs/superpowers/plans/2026-09-23-jongga-040-single-latest-write.md`
- [x] 설계 승인(bounded)
- [x] 계획 검토(oh-my-claudecode:critic): ACCEPT-WITH-RESERVATIONS. 저장 누락 경로 없음, RED 3건이 의도한 이유로 실패함을 HEAD 사본에서 확인. R1(`send_jongga_notification` 이 `BASE_DIR` 절대 경로로 최신 파일을 읽는데 계획이 이를 빠뜨렸고 `[INFRA-082]` 목록에도 없음) → 계획의 수용 차이에 명시하고 `[INFRA-082]` 에 추가. R2(init_data 테스트가 cwd 기준 쓰기나 `save_result_to_json` 재호출을 잡지 못함) → `monkeypatch.chdir(tmp_path)` 와 `saves == []` 단언 추가. 경미 1건(계획의 줄 범위) 정정. 절차 지적 1건: 검토 시작 시 구현이 이미 작업 트리에 있었다. 계획 Task 2 와 일치했고 커밋 전이다
- [x] 구현·RED→GREEN: 신규 3건이 구현 전 의도한 이유로 실패(파일 생성, save 1회 호출)한 뒤 통과. 관련 범위 284 통과
- [x] `/ponytail-review`: 지적 없음(Lean already). 테스트의 `_fake_result` 필드는 옛 코드의 RED 재현에 필요해 유지
- [x] `/code-review`(closing-bet-reviewer): APPROVE, max low. 저장 누락 경로 없음, 지운 페이로드 키 12개가 저장소 페이로드와 일치, 반환·알림 계약 유지, 테스트의 원본 `data/` 접촉 경로 없음. low 1(CLI 호출자 두 곳이 `[INFRA-082]` 근거에 빠짐) → 추가. low 2(None·예외 갈래 미고정)는 이 diff 가 바꾸지 않은 갈래라 미반영
- [x] `/review`(T3, oh-my-claudecode:code-reviewer, review 스킬 절차): CHANGES REQUESTED → 반영 후 APPROVE 조건 충족. H1(이 항목의 리뷰 기록이 문자열 전체 치환으로 `[CHAT-035]`·`[VCP-030]` 의 체크 줄까지 덮음) → 두 항목을 HEAD 원문으로 되돌리고 `git diff` 에 이 항목과 `[INFRA-082]` 두 hunk 만 남음을 확인. 코드 쪽 지적 없음: 캐시 무효화(`atomic_write_text` 가 `invalidate_file_cache` 를 부르고 다른 워커는 mtime·size 서명으로 판정), 다른 쓰기 주체(GET 등급 재계산, 단일 종목 재분석)와의 lost-update 창이 사라져 개선, 스케줄러·관리자 실행이 겹칠 때 일자·최신 파일 불일치가 줄어듦, 테스트 격리 확인
- [x] 전체 pytest(사본, `[INFRA-083]`): 2649 통과 3 skip, 실패·오류 33건은 모두 환경 원인. 사본에 `frontend/node_modules` 가 없어 `@next/env` 를 못 찾은 32건은 `node_modules` 를 clone 한 뒤 재실행해 통과, git 저장소가 아니라 실패한 `test_gitignore_hides_macos_appledouble_and_ds_store_files` 1건은 원본 작업 트리에서 통과. 새 실패 0
- [ ] QA
