...
- [x] 두 자리의 예외 로그에서 예외 문자열을 걷어냄
- [x] 같은 형태가 다른 sender 나 모듈에 더 있는지 확인
- [x] 정지 스위치가 세 sender 를 실제로 덮게 함
- [x] sender 의 반환 bool 을 응답에 반영
- [x] 로그에 비밀이 남지 않는 것을 고정하는 테스트 추가


- [x] ponytail/code/deep/security 리뷰와 독립 architecture CLEAR, 정적 검증 통과
- [ ] UltraQA 실제 브라우저4개필수행·정리·아카이브
