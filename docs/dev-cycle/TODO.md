...
- 카테고리: VCP 시그널 | 티어: T2 | 근거: 2026-09-02 VCP-007 마감 qa-only
- `/api/kr/signals` 는 여섯 건 모두 `stop_loss` 와 `target_price` 를 `null` 로
  내려보냅니다. 화면은 그 자리를 `현재가 × 0.97` 과 `현재가 × 1.05` 로 채웁니다.
  실측한 여섯 행의 비율이 전부 정확히 0.9700 과 1.0500 이었습니다.
- 기준가가 진입가가 아니어서, 신호 발생 이후 주가가 오른 종목은 손절가가 진입가보다
  높게 표시됩니다. 2026-05-05 화면에서 여섯 행 중 네 행이 그렇습니다. SK 는 진입
  ₩475,500 에 손절 ₩568,420 으로 19.5% 위입니다. 매수 포지션에서 손절가가 진입가
  위에 있으면 그 주문은 체결되는 순간 이익 실현이 되므로 성립하지 않습니다. 손절가
  열이 빨간색으로 표시되어 손실 구간이라는 인상까지 함께 줍니다.
- `[JONGGA-007]` 과 같은 성격이지만 대상이 다릅니다. 그쪽은 종가베팅의 백엔드 리터럴
  이고 이쪽은 VCP 화면의 폴백입니다.
- [ ] 손절가·목표가의 기준가를 진입가로 바꾸거나, 백엔드가 값을 내려보내도록 함
- [ ] 값이 없을 때 임의 생성 대신 빈 값으로 표시할지 결정하고 반영
- [ ] 진입가 대비 방향이 뒤집히지 않는지 확인하는 검사 추가
...
### [FE-014] 떠 있는 채팅 위젯이 VCP 표의 마지막 열을 가린다
- 카테고리: 프론트엔드 | 티어: T1 | 근거: 2026-09-02 VCP-007 마감 qa-only
...
- [ ] 표 아래쪽에 위젯 높이만큼 여백을 두거나 위젯을 표 밖으로 옮김
- [ ] 두 해상도에서 마지막 행의 오른쪽 셀이 보이는지 확인
