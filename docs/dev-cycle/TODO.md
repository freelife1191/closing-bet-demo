...
- 내용: (1) 통합 서비스 `_build_trend_map`(`services/investor_trend_5day_service.py:378-379`)은 `foreign_buy`·`inst_buy` 의 빈 칸을 `fillna(0)` 으로 0 으로 읽고 그날을 하루로 세므로 `insufficient_days` 플래그도 붙지 않는다 (2) 상세 API 의 레거시 CSV 경로 `_build_investor_trend_5day_map`(`services/kr_market_stock_detail_service.py:434-473`)은 같은 `fillna(0)` 에 더해 `tail(5)` 에 행 수 검사가 없어, 통합 서비스가 None 을 돌려준 종목의 CSV 가 1~4행이면 k일 합계를 5일 값으로 내보낸다 (3) pykrx 참조 `_fetch_pykrx_reference_trend`(`:709-710`)는 `_safe_int` 로 NaN 을 0 으로 읽는다. `[INFRA-095]` 는 쓰는 쪽만 고쳤고 읽는 쪽과 기존 저장분은 바꾸지 않았다
- 확인 수준: 코드로만 확인. 운영 CSV 에 빈 칸이나 1~4행 종목이 실제로 있는지, pykrx 가 NaN 을 주는 조건은 확인하지 않았다
- 추가 설계 질문(`[FLOW-022]` T3 심층 리뷰 R1): 참조를 모두 버렸고 CSV 플래그가 `stale_csv`·`extreme_abs_total` 이면 `_resolve_best_payload`(`services/investor_trend_5day_service.py:948-955`)가 그 CSV 를 그대로 돌려준다. 상세 API 는 플래그를 debug 로그에만 남기고 값을 표시한다. None 으로 돌릴지, 표시를 붙일지
- [x] 설계 승인(승인 일자 2026-09-25, 확인 시각 19:49, 대화에서 설계 제시 → 사용자 `/effort high` 후 「다음 진행해」, bounded·권장안 채택): (1) `_build_trend_map` 의 `fillna(0)` 을 지우고 반복문 except 에 `OverflowError` 추가. 최근 5행에 빈 칸·무한대가 있는 종목은 map 에서 빠진다(더 오래된 행으로 채우지 않음). SQLite 스냅숏 키는 올리지 않는다(다음 CSV 갱신 때 서명이 바뀌고 로컬 빈 칸 0개) (2) 상세 API 의 레거시 CSV 경로 제거(권장안 A): `kr_market_stock_detail_service.py` 의 레거시 집계·캐시 함수와 전역을 지우고 `append_investor_trend_5day`·`fetch_stock_detail_payload`·realtime 래퍼·라우트 호출에서 `load_csv_file` 인자를 뺀다. 통합 서비스가 None·예외이거나 `data_dir` 가 없으면 `investorTrend5Day` 를 넣지 않는다(모달은 `[FE-048]` 의 Toss 합계 「(N일)」) (3) `verify_with_references=True` 이고 CSV 플래그에 `stale_csv`·`extreme_abs_total` 이 있으며 채택된 참조가 없으면 `_resolve_best_payload` 가 None. `single_day_spike` 만 붙은 CSV 는 남기고 verify=False 계약은 그대로. docstring 정정 (4) `_fetch_pykrx_reference_trend` 가 하루 값을 `optional_volume` 으로 읽어 NaN 을 None 으로 두고 합계에서 뺀다 → `pykrx:insufficient_days` 로 버려져 최신 창이면 Toss 를 조회한다(과거 기준일은 Toss 를 부르지 않아 spike 만 붙은 CSV 는 남고 extreme 이면 None). 티어 T3(§2 수급 집계 위험 경로). 스킬: `.claude/skills/closing-bet-python/`, frontend 변경 없음. 계획 문서 `docs/superpowers/plans/2026-09-25-flow-023-supply-read-missing-as-missing.md`
- [x] 계획 검토(`oh-my-claudecode:critic`, 2026-09-25) ACCEPT-WITH-RESERVATIONS, 줄 번호·기대값 직접 계산으로 일치 확인: (1) 중, 과거 기준일 pykrx 참조 캐시는 토큰이 그 날짜라 만료되지 않아 수정 전 NaN→0 저장분이 계속 채택된다 → 빈도 미확인이라 코드 불변, 계획 Review Focus 정정·아카이브 기록 (2) 중·범위 밖, `engine/signal_tracker_supply_helpers.py:51-58` 도 NaN 을 건너뛴 합을 5일 값으로 점수화 → `[FLOW-024]` 등록 (3) 하, T3 는 vitest 전체도 요구 → 정적 검증에 추가 (4) 하, 새 검사 건수 4→3 정정 (5) 하, `data_dir` 없음 케이스는 호출 기록으로 단언 → 반영 예정 (6) 하·ponytail, 호출자 없는 realtime 래퍼 `_append_investor_trend_5day` 는 인자 수정 대신 삭제 → 반영 예정 (7) 하, 테스트 파일의 레거시 import 삭제 명시 → 반영 예정
- [x] Task 1 통합 서비스 (1)(3)(4): 새 검사 3건(빈 칸·무한대 map, pykrx NaN 거부, extreme CSV None·verify=False 유지)과 기대값 변경 2건(0 참조 검사 → None 으로 이름·단언 변경, `[FLOW-022]` Toss 검사 → 최근 날짜 급등 CSV 로 재구성). RED 4건(무한대 OverflowError, `[5,4,0,2,1]`, extreme·낡은 CSV 반환), 구현 뒤 43 passed, 수급 소비처 `-k "supply or trend or screener or collector"` 214 passed
- [x] Task 2 레거시 경로 제거: 레거시 전역·함수 7개와 import 정리, `load_csv_file` 인자 삭제(서비스·realtime 래퍼·라우트), 호출자 없는 realtime `_append_investor_trend_5day` 삭제(critic 6). 레거시 검사 5건 삭제(대상 코드 삭제: 최근 5행 합계, 통합 서비스 None 때 CSV 폴백, 필수 열 누락 생략, 레거시 map 캐시 두 건), 새 검사 1건 3케이스(None·예외·`data_dir` 없음은 호출 기록 0으로 단언, critic 5). RED 는 시그니처 TypeError(레거시 폴백 동작은 삭제한 `..._falls_back_to_csv_...` 가 반대로 고정하고 있었다), 구현 뒤 대상+`tests/app` 699 passed. pytest 전체 2853 passed·2 skipped exit 0, 「KRX 로그인」 출력 0회
- [x] `/ponytail-review`(2026-09-25): Lean already. Ship.(지적 0)
- [x] `closing-bet-reviewer`(2026-09-25, xhigh) APPROVE, max medium(범위 안 low, 범위 밖 medium 1): F1 중·범위 밖, 수급 CSV 작성부(`scripts/init_data.py:389-395`·`:1294-1303`)는 빈 값인 날을 빈 칸이 아니라 행 누락으로 남겨 (1) 이 잡지 못하고 6거래일 이상의 합이 `days: 5` 로 나간다 → 작성부 코드로 확인, `[FLOW-025]` 등록 | F2 하, 스크리너 Toss 실패 대체 경로(`engine/screener.py:367`)도 (3) 으로 낡은 CSV 대신 MISSING_SUPPLY → 의도한 동작, 계획 Review Focus·아카이브 기록 | F3 하, pykrx 부분합이 참조 캐시에 남음 → 「판정 없이 쓰지 않는다」 주석 반영 | F4 하, 삭제 검사가 지키던 날짜 정렬 성질 → 새 map 검사의 CSV 를 날짜 역순으로 써서 고정, 정렬을 지우는 mutation 에서 실패 확인 후 복원(필수 열 누락 가드는 이번에 바꾸지 않은 코드라 검사 추가 안 함) | F5 nit, 쓰는 곳 없는 `_LOGGER` → 삭제 | F6 nit, `get_ticker_padded_series` 검사가 `test_kr_market_csv_utils.py:141` 과 완전 중복 → 검사와 import(`pandas` 포함) 삭제 | F7 하, 낡은 CSV None 검사가 갈래를 구분하지 못함 → verify=False 로 `stale_csv` 단언 추가 | F8 문서 → TODO (4) 문구·정적 검증 줄·`append_investor_trend_5day` docstring 정정 | F9 하, 참조 일시 실패 때 키 없는 상세 페이로드가 15분 캐시 → 계획 Review Focus, QA 기록 예정 | F10 info, 라우트 시그니처는 브라우저 QA 가 덮음. 반영 뒤 대상 3파일 61 passed. vitest 전체 104 files·701 passed(exit 0, frontend 변경 없음)
- [x] `/review`(T3, `oh-my-claudecode:code-reviewer`, 2026-09-25, xhigh) APPROVE, 범위 안 코드 결함 0: M1 Minor, fetch → append 호출의 인자 모양을 지키는 검사가 없어 어긋나면 `except Exception` 이 삼키고 Naver 폴백의 0 이 표시됨 → `test_fetch_stock_detail_payload_reuses_sqlite_snapshot_after_memory_clear` 가 append 대신 통합 서비스 함수를 바꿔 끼워 실제 append 를 돌림, 호출에 `load_csv_file=None` 을 넣는 mutation 에서 `KeyError: 'investorTrend5Day'` 로 실패 확인 후 복원 | M2 Minor·범위 밖, 수집기 믹스인의 자체 pykrx 폴백이 빈 프레임을 0·0 으로 저장하고 `tail(5).sum()` 이 NaN 인 날을 건너뛴 부분합을 5일 값으로 씀(krx 믹스인은 빈 프레임이면 플래그 붙은 CSV 를 그대로 씀) → 코드로 확인, `[FLOW-026]` 등록 | L1 확신도 중간·범위 밖, `_detect_csv_anomaly_flags` 는 target 이 None 일 때만 stale 을 보므로 스크리너가 오늘 날짜 target 으로 Toss 실패 대체 경로(`engine/screener.py:367`)를 타면 낡은 CSV 가 플래그 없이 채택됨 → 코드로 확인, 운영 파이프라인이 CSV 를 먼저 갱신해 가능성 낮음, `[FLOW-025]` 설계 때 함께 판단하도록 그 항목에 기록 | L2 (1) 로 빠진 종목은 verify=True 호출마다 참조 조회(확신도 낮음)·L3 NaN 참조의 거부 판정이 정상 결과로 캐시됨(조치 불필요 판단) → 기록만. 반영 뒤 대상 3파일 61 passed
- [x] 정적 검증(리뷰 반영 뒤, 2026-09-25): `KRX_ID= KRX_PW= venv/bin/python -m pytest -q -p no:cacheprovider` 2852 passed·2 skipped exit 0(중복 검사 1건 삭제로 2853→2852), 「KRX 로그인」 출력 0회. `cd frontend && npx vitest run` 104 files·701 passed exit 0. 첫 커밋 뒤 QA 진입 전 압축 정지(QA 계획→실행→기록은 한 문맥에서 이어간다)
- [ ] QA 2단계(`/qa-only` → `/qa`, `docs/dev-cycle/qa/FLOW-023.md`): 격리 사본(`secrets/`·`data/`·`.env` 삭제, 3500·5501 제외 포트), 가짜 Toss HTTP·pykrx, 소켓 차단, 브라우저로 상세 모달(1~4행 종목·낡은 CSV·빈 칸·정상 종목). 리뷰 F9(참조 일시 실패 때 15분 캐시)·F10(라우트 시그니처) 기록
