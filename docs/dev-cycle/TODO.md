...
### [INFRA-016] 승인 게이트를 없애고 압축 지점을 규약에 명시한다
- 카테고리: 인프라 | 티어: 문서 | 근거: 2026-09-01 사용자 지시 "계속 물어보지말고 진행할
  수 있는 데까지 진행하고 중간중간 qa 전이나 사이클별로 완료되면 compact 할 수 있게 해줘"
- 남아 있던 게이트 1(계획 승인)까지 없앱니다. 계획과 결과를 알리는 일 자체는 그대로
  두고 답을 기다리는 동작만 없앱니다.
- 멈추는 자리를 압축 지점과 `## 중단` 의 조건 두 가지로 한정합니다. 게이트를 없애면서
  멈추는 자리를 함께 정하지 않으면 어디서도 멈추지 않게 되어, 단계 도중에 자동 압축이
  걸리고 진행하던 맥락이 통째로 끊깁니다.
- 압축 지점을 두 곳에서 세 곳으로 늘립니다. `/qa-only` 호출 직전이 새로 들어갑니다.
  이 스킬은 브라우저를 열고 긴 리포트를 내므로 맥락이 무거운 상태로 들어가면 리포트를
  받는 도중에 압축이 걸립니다.
- [ ] `SKILL.md` frontmatter 의 "exactly two approval gates" 정정
- [ ] `SKILL.md:9` 의 게이트 한 곳 서술을 게이트 없음으로 교체
- [ ] `SKILL.md` [1] 계획 4·5번을 알린 뒤 곧바로 구현으로 넘어가는 형태로 수정
- [ ] `SKILL.md` 규칙 절의 게이트 1 조항을 멈추는 자리 규정으로 교체
- [ ] `SKILL.md` 컨텍스트 관리 절에 `/qa-only` 직전 압축 지점과 턴 종료 규정 추가
- [ ] `tier-rules.md` §1 의 `/qa-only` 호출 절에 압축 지점 참조 한 줄 추가
- [ ] `CLAUDE.md:231` 의 게이트 서술 교체와 압축 지점 명시
- [ ] `.claude/agents/dev-workflow.md` 에 리포트 반환 뒤의 압축 지점 추가
