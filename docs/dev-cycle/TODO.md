...
### [JONGGA-009] 종가베팅의 `Latest Report` 가 존재하는 리포트를 불러오지 않는다
- 카테고리: 종가베팅 | 티어: T1 | 근거: 2026-09-02 FE-012 마감 qa-only
- 페이지에 처음 들어가면 리포트 선택기가 `Latest Report` 로 놓이고 화면이 비어 있습니다.
  CANDIDATES 0, FILTERED 0, DATA STATUS `NO DATA`, 본문 `No Signals Found`, TRENDING
  THEMES 는 `데이터 갱신 후 표시됩니다` 입니다. 그런데 바로 위 배너는 "오늘(2026-09-02)
  기준 종가베팅 데이터가 없습니다. 최신 저장 데이터는 2026-09-01입니다." 라고 알립니다.
- 선택기에는 `2026-09-01` 을 포함해 날짜 리포트 11개가 들어 있습니다. `2026-09-01` 을
  직접 고르면 CANDIDATES 2, DATA STATUS `OLD DATA` 로 바뀌고 SK이노베이션(A등급)과
  삼성생명(B등급) 카드가 정상적으로 나옵니다. 즉 `Latest Report` 는 가장 최근 저장분이
  아니라 오늘 날짜를 조회하고 있습니다.
- 같은 상황에서 VCP 화면은 `과거` 버튼으로 직전 저장분을 열어 주므로, 자매 페이지끼리
  처리 방식이 어긋나 있기도 합니다.
- [ ] `Latest Report` 가 실제로 존재하는 가장 최근 리포트를 고르도록 수정
- [ ] 오늘 자료가 없을 때 배너가 지목한 날짜와 화면에 실린 자료가 일치하는지 확인하는 검사 추가
