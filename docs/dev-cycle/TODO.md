...
- 설계 승인: 승인 일자 2026-09-23 | 승인 확인 시각 2026-09-23 09:12
  | 범위: `chatbot/storage_memory_manager.py`·`chatbot/storage.py` 에 인스턴스 RLock, `_save_single_entry` 레코드 인자, `view()` 사본 반환, `_save()` 전체 동기화 폴백과 무델타 전체 동기화 제거(델타 실패는 스냅샷만), `_load()` 레거시 이관을 SQLite 가 실제로 빈 경우(`{}`)로 한정, 동시성·폴백 회귀 테스트. `save_history_sessions_to_sqlite` 와 스키마 파일은 그대로. 외부 호출자의 get_session→수정→_save 경합은 ponytail 주석으로 표시만
  | 실제 대화 근거: 2026-09-23 사용자 「진행해」 응답, 현재 세션의 bounded 설계 제안(T2, 폴백은 재적재가 아니라 제거로 결정)
- [x] 설계 승인(bounded)
- [x] 구현·RED→GREEN: 신규 `tests/chatbot/test_storage_concurrency.py`. 구현 전 메모리 update 경합(다른 스레드 재적재를 `datetime.now` 자리에 끼움, 결정적)·델타 실패 뒤 다른 워커 세션 보존·읽기 실패 뒤 이관 금지 세 건 실패, 히스토리 두 스레드 테스트는 확률적 실패(작성자 3/4, 리뷰어 2/6). 공용 데코레이터 `locked`(`storage_history_helpers.py`)로 두 매니저의 공개 메서드 직렬화, `_save_single_entry` 레코드 인자, `view()` 사본, `_save()` 전체 동기화 폴백·무델타 전체 동기화 제거, 레거시 이관은 DB 없음 또는 빈 표일 때만. DB 파일이 없을 때 로더가 None 을 돌려주는 경우를 첫 구현이 이관 금지로 오판해 기존 이관 테스트가 실패했고 `not db_path.exists()` 로 바로잡음
- [x] `/ponytail-review`: 메모리 쪽 `_*_locked` 보조 메서드 네 개를 스스로 발견해 공용 데코레이터로 통일한 뒤 리뷰 → Lean already, net -0
- [x] `/code-review`(closing-bet-reviewer, name `chat033-reviewer`) CHANGES_REQUIRED(medium 2) → 반영 → APPROVE(low 1) → 반영. M1 델타 실패 뒤 서명을 갱신해 저장되지 않은 삭제가 이 워커 응답에 남고 뒤이은 쓰기가 DB 를 덮어씀 → 실패 시 서명 None. M2 재적재 중 읽기 실패에서 늦은 JSON 이 사본이 됨 → `_load` 가 None, `_reload_failed` 동안 `_save` 는 SQLite·JSON 모두 쓰지 않음, 다음 재적재 성공 시 해제. L 기동 시점 읽기 실패도 같게 처리. L 잠금 안의 busy 대기는 ponytail 주석. 스레드 테스트의 확률적 RED 는 QA 문서에 기록. 4(같은 세션 user/model 사이 끼어듦)·6(view 사본 무해)·7(잠금 밖 외부 호출자, ponytail 주석)·8(데코레이터 위치)은 수용. 변이 4종(서명 유지, 거부 무력화, 재적재 JSON 교체, 기동 조건 복원) 각각 해당 테스트 실패
- [ ] QA
