...
- 설계 승인: 2026-09-23 사용자 「응 진행해」(bounded, 대화 설계). 순서는 사용자 선택 「CHAT-039 먼저」로 `[CHAT-035]` 보다 앞에 둔다. 건너뛰기 안은 다음 `_save` 의 주기 스냅샷이 결국 JSON 을 덮으므로 기각하고, 되살리기 안을 채택했다.
- 설계: `chatbot/storage.py` `_load` 에서 SQLite 세션이 있는데 메시지가 모든 세션에서 0개일 때만 JSON 스냅샷을 읽는다. 두 쪽 `updated_at` 이 같고 SQLite 는 비었는데 JSON 에는 메시지가 있는 세션만 되살리고, 그 세션만 `apply_history_session_deltas_in_sqlite` 로 다시 쓴다(전체 동기화 금지). 한계(일부 행 손실, 직전 15초 안에 갱신된 세션)는 `ponytail:` 주석으로 남긴다. 공용 래퍼 서명은 바꾸지 않는다. 티어 T2(위험 경로 파일 없음). 읽은 정본: `.claude/skills/closing-bet-python/SKILL.md`.
- [x] 구현·RED→GREEN: 첫 2건 중 되살리기 1건 RED 확인(판이 다른 경우는 종전에도 되살리지 않아 처음부터 통과). 리뷰 반영으로 `chatbot/storage_sqlite_history.py` 에 조건부 쓰기 `restore_lost_history_messages_in_sqlite`(BEGIN IMMEDIATE, `updated_at` 같고 메시지 행이 없을 때만 INSERT)와 퍼사드 export 추가, 테스트 총 6건. 변이 7종(`updated_at` 조건 제거, SQLite 재기록 생략, 호출 제거, 조건부 쓰기 제거, 실패 시 사본 반환, 재적재 대신 메모리 사본 반환, 재적재 경로에서 호출 제거) 모두 실패·원본 통과. 파일이 위험 경로 밖이라 티어 T2 유지.
- [x] `/ponytail-review`(oh-my-claudecode:code-reviewer 레인) 「Lean already. Ship.」 → `closing-bet-reviewer` CHANGES REQUESTED: medium 1(되살리기 쓰기 실패를 무시해 일시 오류가 영구 유실, 재현) → 실패 시 `_load` 가 None, medium 2(조건 없는 upsert 가 다른 워커의 새 쓰기를 덮음, 재현) → 조건부 쓰기와 DB 재적재, low 3(스냅샷 `messages` 형식) → `isinstance`, low 4(재적재 경로도 JSON 을 읽고 다시 쓸 수 있음) → `ponytail:` 주석. 재판정 APPROVE. 남은 low 2건(되살린 직후 다른 워커가 모두 지우면 스냅샷이 `{}` 로 맞춰짐 = DB 와 같아 유실 아님, 리뷰어 재현 스크립트 하나가 새 함수 대신 옛 함수를 가로챔)은 기록만. 테스트 공백 권고 중 재적재 경로 1건 추가, 「한 세션이라도 메시지가 있으면 되살리지 않음」은 첫 번째 변이로 간접 확인되어 미추가.
- [x] 정적 검증: `pytest tests/chatbot` 262 통과. 사본 전체 pytest(INFRA-083): 변경 사본·HEAD 사본 모두 25 failed 2646 passed 3 skipped 8 errors, 실패 목록 33줄 동일(새 실패 0, node·frontend `node_modules` 없는 사본 환경).
- [ ] QA(격리 사본, 메시지 테이블 DROP 뒤 새로고침): `docs/dev-cycle/qa/CHAT-039.md`
