...
- 카테고리: 프론트엔드 | 티어: T1 | 근거: 2026-09-02 VCP-007 진행 중 code-review
- `[VCP-007]` 이 백엔드에서 실패 추천을 걸러 냈지만, 화면은 병합된 값이 비면
  `aiData?.signals` 를 다시 봅니다. 그 자료는 원시 캐시라서 실패 기록이 그대로
  들어 있습니다. 겹치는 자리는 세 곳입니다.
  `frontend/src/app/dashboard/kr/vcp/page.tsx:427-430`(`secondaryAI`),
  `:444-446`(`hasData`), `:1679-1680`.
- 배지 자체는 안전합니다. `AI_ACTION_BADGES`(`:187`)에 `N/A` 항목이 없어 실패
  기록은 `-` 로 나갑니다. 실제 영향은 데이터가 없는 AI 탭이 활성 상태로 보여
  사용자가 눌렀을 때 모든 행이 `-` 인 빈 열을 만나는 것입니다.
- [ ] 세 자리의 존재 판정에 백엔드와 같은 유효성 기준 적용
- [ ] 실패 기록만 든 캐시에서 탭이 활성화되지 않는지 확인하는 회귀 검사 추가
