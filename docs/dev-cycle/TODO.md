...
- 설계 승인: 승인 일자 2026-09-22 | 승인 확인 시각 2026-09-22 23:34
  | 범위: Flask 에 서명 신원으로만 동작하는 `DELETE /api/kr/user/data` 를 두어 챗봇 세션·메시지·메모리(프로필 포함), 모의투자 네 테이블(`balance` 포함), 무료 사용량·충전 기록을 지우고 저장소별 결과를 돌려준다. 관리자가 남을 지우는 기능은 두지 않는다. 설정 모달의 「계정 삭제」는 서버 삭제가 성공한 뒤에만 브라우저 저장소를 비우고 로그아웃하며, 실패하면 지워지지 않았다고 알린다. 활동 로그는 30일 자동 삭제만 안내한다.
  | 실제 대화 근거: 2026-09-22 사용자의 선택 「INFRA-077 승인, 끝나면 FE-045」(현재 세션의 bounded 설계 제안에 대한 AskUserQuestion 응답). `[INFRA-077]` 은 커밋 `7887bf8`·`4fffc14` 로 마감됨
- 조사 정정(2026-09-22): 근거의 「`usage.db` 의 `usage_log`」는 틀렸다. 무료 사용량과 충전 기록은 `data/user_quota.json`(`app/routes/kr_market.py` 의 `QUOTA_FILE`, 키는 이메일과 `recharge_day:<이메일>`)에 있고, `services/usage_tracker.py` 는 어떤 운영 코드도 import 하지 않는 유물이다(로컬 `data/usage.db` 행 0, 읽기 전용 확인). 삭제 대상은 실제 저장소 세 곳이다.
- 티어: T3. `services/paper_trading_trade_account_mixin.py`(모의투자 계좌와 거래)에 닿는다. `superpowers:writing-plans` 계획 → `oh-my-claudecode:critic` 검토 → 구현.
- [x] 설계 승인(bounded, T3): 위 메타 줄. 읽은 정본 `.claude/skills/closing-bet-python/SKILL.md`, `.claude/skills/closing-bet-nextjs/SKILL.md`, `.claude/skills/closing-bet-verify/SKILL.md`, `frontend-skills.md` §2, Next 번들 문서 `01-app/01-getting-started/05-server-and-client-components.md`(`SettingsModal.tsx` 는 순수 클라이언트 변경: 클릭 핸들러와 문구, 라우트 핸들러 없음)와 `03-layouts-and-pages.md`(`(legal)/privacy/page.tsx` 는 `metadata` 를 export 하는 서버 컴포넌트이며 문구만 바뀜)
- [x] 구현 계획: `docs/superpowers/plans/2026-09-22-fe-045-account-data-deletion.md`. 계획 검토(`oh-my-claudecode:critic` `fe045-critic`) 판정 REVISE(차단 3·중요 3·경미 5·관찰 3). B1(핸들러가 getter 를 인자로 먼저 평가해 챗봇 생성 예외 하나가 나머지 삭제를 막음 → 단계 안에서 부르고 실패 검사 추가), B2(`clear_for_owner` 는 SQLite 표식을 남기지 않고 되살아나는 갈래는 폴백 upsert → 서술 정정, `[CHAT-033]` 범위 추가), B3(`signOut` 실패를 삭제 실패로 오보고 → `try` 는 서버 삭제만), M1(`api_usage` 도 유물, 로컬 행 0, 운영은 `[INFRA-079]`), M2(`GET /api/portfolio` 가 잔고·자산 이력을 되살림 → S-1 기대값 완화), M3(세션 0건 검사), m1~m4, `timeout: 30000` 반영. 미반영: m5 getter 주입 제거(실패 주입 검사가 씀), 검사 파일 이름(관례 혼재). 지적 반영 뒤 재검토 없이 진행(SKILL [1] 4번).
- [x] 구현·RED→GREEN: `delete_account`(모의투자, `_forget_asset_history_snapshot` 헬퍼 공유)·`delete_owner`(메모리)·`delete_user_usage`(사용량)·`DELETE /api/kr/user/data`(`app/routes/kr_market_user_data_routes.py`)·`SettingsModal.performResetData`·개인정보처리방침 세 셀과 한 문장. pytest 신규 9건(4 파일)은 AttributeError·ImportError·ModuleNotFoundError 로 실패를 확인한 뒤 통과, vitest 신규 2건은 실패 경로가 「실패 모달 없음」으로 실패한 뒤 통과. 구현 중 발견: `useQuota` 도 `fetchAPI` 를 쓰므로 한 번짜리 mock 은 사용량 조회가 소비한다 → URL 로 가르는 `mockServer`.
- [x] `/ponytail-review`(직접 수행): `delete_owner` 의 중복 실패 로그 두 줄 삭제(`clear_memories_in_sqlite` 가 자기 실패를 기록). 나머지 Lean.
- [x] `closing-bet-reviewer`(`fe045-reviewer`) 1차 CHANGES_REQUIRED(medium 1·low 3·관찰 2)와 `oh-my-claudecode:security-reviewer`(`fe045-security`, 병행) 1차 CHANGES_REQUIRED(중 1·중하 2·하 1·정보 1). 겹치는 핵심: `HistoryManager._save()` 가 저장 실패를 삼켜 `clear_for_owner` 가 커밋되지 않은 삭제를 건수로 돌려주고 라우트가 200 을 냄 → `_save()` 가 `bool` 을 돌려주고 `clear_for_owner` 는 `_reload_sessions(force=True)` 뒤 저장 실패에 `RuntimeError`, `/clear all` 명령은 경고 문구(검사 2건 RED→GREEN). `delete_owner` 는 스냅샷 기록 실패도 실패로(`deleted and written`, 검사 1건). 보안 중하 「`status === 'authenticated'` 로만 서버 호출을 감싸 만료·재조회 순간에 조용히 생략」과 코드 리뷰 관찰 「익명 갈래는 UI 로 도달 불가」 → 분기 삭제, 항상 서버 삭제 선행. 코드 low 「실패 문구가 부분 삭제를 부정」 → 문구 정정. 보안 하 「다른 기기 세션이 `GET /api/portfolio` 로 잔고 행을 되살림」 → 확인 대화에 다른 기기 로그아웃 안내. 코드 low 계획 S-3 문구 → 계획·QA 정정. 정보(레이트 리밋 부재, 이 DELETE 의 활동 로그 기록은 `[FE-046]`)는 기록만. 보안 2차 APPROVE(아홉 파일 해시 일치. 정보성: `clear_for_owner` 의 강제 재적재로 호출당 비용 증가, 스냅샷만 계속 실패하면 자료는 지워진 채 500 반복은 안전한 방향, 실패 문구는 401 에서 다소 넓지만 가능성 서술). 코드 리뷰 2차 APPROVE(max low. 저장 실패 재현·재시도 검사가 실제로 가름을 확인. low: 계획 Task 5 가 폐기된 분기와 옛 문구를 담음 → 최종 판으로 교체. 관찰: 강제 재적재로 스레드 경합 창 빈도 증가 → `[CHAT-033]` 근거에 추가, 저장 실패 뒤 그 워커의 대화 목록이 다음 재적재 전까지 비어 보임(SQLite 에는 남음, 재시도로 해소), `/clear all` 은 저장 실패에서 메모리도 건드리지 않음).
- [x] `/review`(T3 심층 리뷰, `oh-my-claudecode:critic` `fe045-deep`, scratchpad 재현 포함) ACCEPT-WITH-RESERVATIONS. 반영: 삭제 요청 중 버튼 비활성화·「삭제 중…」(`isDeleting`, 검사 1건), `signOut` 실패에 「기록은 지워졌지만 로그아웃만 실패」 안내(`window.location.assign` 은 Next lint 규칙에 걸려 모달로), Danger Zone 제목·버튼 「계정 삭제」. 미반영: SQLite 삭제 커밋 뒤 스냅샷 예외의 500 은 안전한 방향이라 유지. 깨끗 확인: `_save()` 반환형 무회귀, 강제 재적재와 장부 정합(재현), `delete_account` 스키마 재시도·`price_cache` 미접촉, CASCADE, 라우트 분기 5건, 삭제 범위, 다중 워커 서술.
- [ ] 정적 검증: `pytest -q` 전체, `(cd frontend && npx vitest run)`, `npm run type-check`, `npm run lint`
- [ ] QA: `docs/dev-cycle/qa/FE-045.md`. 격리 환경(가짜 Gemini `qa_app.py`, 더미 비밀, 비관리자 세션 쿠키)에서 대화·모의 매수 → 계정 삭제 → 세 저장소 행 0 → 재로그인 시 빈 상태. 브라우저 required
...
- 범위: `MemoryManager` 의 재적재·쓰기를 인스턴스 잠금으로 직렬화, `_save_single_entry` 에 레코드를 인자로 전달, `_save()` 전체 동기화 폴백의 삭제 절 제거, 델타 장부와 세션 사전 접근 잠금, 두 스레드 동시 쓰기에서 유실·`KeyError` 가 없음을 확인하는 테스트. 테이블을 정의하는 `storage_sqlite_common.py` 는 건드리지 않는다. diff 가 300줄을 넘으면 T3 로 올린다. 추가(2026-09-22, `[FE-045]` 계획 검토 발견): 폴백의 삭제 절을 없애도 `save_history_sessions_to_sqlite` 의 upsert 가 남아, 델타 저장 실패로 폴백에 떨어진 워커가 재적재하지 못한 낡은 인메모리 사본으로 다른 워커가 지운 세션(계정 삭제 포함)을 되살릴 수 있다. 폴백 전에 재적재하거나 폴백 자체를 없앨지 이 항목에서 정한다. `[FE-045]` 가 `clear_for_owner` 에 `_reload_sessions(force=True)` 를 넣어 파일 서명이 같아도 `self.sessions` 를 통째로 교체하므로, 다른 스레드가 같은 사전을 들고 저장하는 경합 창의 빈도가 늘었다(코드 리뷰 관찰, 확신도 낮음). 잠금을 넣을 때 이 자리도 함께 본다.
