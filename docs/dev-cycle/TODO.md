...
### [JONGGA-010] 오늘 자료가 아닌 종가베팅 리포트에서 행별 매수 버튼이 열려 있다
- 카테고리: 프론트엔드 공통 | 티어: T1 | 근거: 2026-09-02 JONGGA-009 마감 qa-only ISSUE-001
- 같은 화면에서 `종가베팅 전체 10주 매수` 버튼은 `disabled` 이고 툴팁이 "현재 최신
  리포트가 오늘 데이터가 아닙니다." 인데, 카드마다 붙은 `모의 매수` 버튼은 눌립니다.
  `모의 투자 매수` 모달이 정상적으로 열리고 `매수 주문` 까지 진행됩니다. 일괄 매수가
  막으려던 결과에 종목 수만큼 반복해서 도달합니다.
- VCP 화면의 같은 결함은 `[FE-013]` 에서 고쳤습니다. 그때 만든 `buyDisabledReason`
  파생 변수 방식을 그대로 옮기면 됩니다.
- `[JONGGA-009]` 가 `Latest Report` 에 과거 저장분을 싣게 되면서 이 버튼이 기본 화면에서
  바로 닿게 되었습니다. 결함 자체는 그 전부터 있었고 선택기로 날짜를 고르면 같은
  상태였습니다. 노출 경로가 짧아졌으므로 P1 로 둡니다.
- [ ] 행별 `모의 매수` 버튼을 일괄 매수와 같은 날짜 조건으로 막는다
- [ ] 두 버튼이 같은 사유 문구를 쓰는지 확인하는 회귀 검사 추가

### [VCP-010] VCP 상세 차트가 선택한 과거 날짜를 반영하지 않는다
- 카테고리: VCP 시그널 | 티어: T2 | 근거: 2026-09-02 FE-012 마감 qa-only
- `2026-05-05` 를 고른 상태에서 SK(034730) 행을 눌러 상세 패널을 열면, 캔들 차트의
  가로축이 6월 4일부터 9월까지 이어집니다. 시그널이 발생한 2026-05-05 는 차트 범위
  밖에 있어서, 그 시그널을 만든 구간을 화면에서 볼 수 없습니다.
- 차트에 겹쳐 그리는 VCP 전반부·후반부 범위도 그 최근 구간에서 계산되므로 수축비율이
  두 가지로 표시됩니다. 표의 CONT. 열은 0.41 인데 차트 하단의 `Ratio` 는 0.77 입니다.
  같은 패널의 VCP SCORE 카드는 "수축비율 0.41 로 기술적 압축이 양호하고" 라고 적혀
  있어서, 한 화면 안에 0.41 과 0.77 이 나란히 놓입니다.
- [ ] 과거 날짜를 고른 상태에서는 차트 구간도 그 날짜를 포함하도록 수정
- [ ] 표의 수축비율과 차트의 `Ratio` 가 같은 구간에서 나오는지 확인하는 검사 추가
