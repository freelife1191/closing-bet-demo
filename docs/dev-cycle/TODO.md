...
### [INFRA-073] 재시작 시 의존성 변경 감지와 설치 실패 차단
- 카테고리: 인프라 | 티어: T2 | 상태: 구현·검증 중
- 설계 승인: 사용자 「보완해줘 업데이트가 필요하면 업데이트 하도록」 및 기존 자동승인 위임. 받은 코드의 검증된 의존성 적용, 자동 Git pull/임의 최신버전 승격 제외.
- [ ] Python은 가상환경만 사용해 requirements 적용 및 pip check.
- [ ] 프론트엔드 manifest/lock/Node환경/설치 상태 변경 시 npm ci, 정상동일환경은 생략.
- [ ] 설치·검증 실패 시 restart의 서버 기동 차단.
- [ ] 격리 실행 회귀·실제 설치·리뷰·전체검증 후 마감.
