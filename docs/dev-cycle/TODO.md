...
- 설계 승인: 승인 일자 2026-09-24 | 승인 확인 시각 2026-09-24 22:06 | 범위: Toss 백필 행에 `source="toss"` 열을 붙이고, 기대 최신일 기준 `lookback_days` 창 안에서 Toss 행이 있거나 행 수가 종목의 80% 미만인 날짜를 재수집 대상으로 정해 시작일을 당기고 Skip 에서 제외한다. 창 밖은 재수집하지 않는다(`ponytail:` 표시). 한쪽 0 기본값·`or 0` 은 `[INFRA-095]` 로 분리, `[INFRA-088]` 과는 따로 진행 | 근거: bounded 설계 제시 뒤 사용자 「진행해」
- [x] 계획 `docs/superpowers/plans/2026-09-24-infra-094-refetch-approx-trend-dates.md` 와 계획 검토(`oh-my-claudecode:critic`): REVISE. 지적 1 HIGH 과거 날짜만 재수집되고 최신일이 빈 응답이면 백필·stale 판정을 건너뛰고 `True`(회귀) → 저장 뒤 최신일이 기대일보다 이르면 같은 백필·stale 꼬리로 넘기도록 수정, 테스트 추가 | 지적 2 MEDIUM pykrx 가 주지 않는 종목의 Toss 행이 창 안에서 매 실행 재수집을 부름 → 비용 한계(날짜당 2회, 창 제한)로 수용하고 `ponytail:` 주석·계획에 명시 | 대안(창 안은 무조건 재조회)은 승인 설계와 달라 미반영
- [x] 테스트(창 안 Toss·부족 날짜 재수집, 완전한 날짜·창 밖 날짜 Skip, 백필 행의 `source`, 최신일 빈 응답 때 백필) RED→GREEN
- [x] 리뷰: `.claude/skills/closing-bet-python/SKILL.md` · `/ponytail-review` Lean already · `/review`(T3, `oh-my-claudecode:code-reviewer`) HIGH 없음: M1 재수집이 정확값을 한쪽 0 으로 덮음 → 재수집 날짜는 두 프레임 교집합만 저장, 테스트 추가 | L 꼬리 계약 변경 → 의도한 변경으로 계획·QA 에 기록 | L 형식이 어긋난 날짜로 수집 중단 → `[:10]` | L 당긴 구간의 빈 날짜 매 실행 조회 → 계획에 비용 기록 | 살아남은 변이 2(시작일 `min`, 꼬리 로그) → 영향 낮아 미반영 · `closing-bet-reviewer` 1차 CHANGES_REQUIRED(max medium): 1 한쪽 0 덮음 → 위와 같음 | 2 중단 뒤 Toss 백필 → 백필이 필요한 갈래 앞에서 `False`, 테스트 추가 | 3 반환값 계약 변경 → 기록 | 4 Focus 1·2 테스트 → 추가 | 5 docstring → 보완. 2차 APPROVE(max low): N1 중단 확인이 백필 불필요 갈래도 `False` → 백필이 필요할 때만 확인하도록 이동 | N2 계약 변경 QA 기재 → 반영
- [ ] pytest 전체, 격리 사본 하네스 QA(`docs/dev-cycle/qa/INFRA-094.md`)
