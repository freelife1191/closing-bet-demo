...
- 카테고리: 인프라 | 티어: T3(`.env` 접촉) | 근거: 2026-09-08 `[INFRA-044]` 사이클의
  `feature-dev:code-reviewer` 가 확신도 높음으로 지적하고 재현했습니다. 저도 임시
  디렉터리에서 재현해 확인했습니다.
- 설계 승인: 승인 일자 2026-09-08 | 승인 확인 시각 2026-09-08 00:46
  | 범위: `restart_all.sh`·`stop_all.sh` 의 `source .env` 제거, `scripts/env_value.sh` 신규,
  회귀 검사 추가. `.env` 파일과 `services/common_env_service.py` 는 건드리지 않는다
  | 실제 대화 근거: 2026-09-08 사용자 「진행해」 응답, 같은 세션의 해당 설계 제안
  | 계획 문서: `docs/superpowers/plans/2026-09-08-infra-049-env-source-removal.md`
- `restart_all.sh:11` 과 `stop_all.sh:16` 이 `set -a; source .env; set +a` 로 `.env` 를
  **bash 스크립트째 실행**합니다. bash 는 `VAR=값` 대입의 오른쪽에서 명령 치환(`$(...)`,
  백틱)을 항상 평가하므로, 설정 화면으로 저장할 수 있는 열두 키 가운데 아무 값에나
  `$(...)` 를 심는 것만으로 다음 기동 때 임의 명령이 실행됩니다.
- 재현: `update_env_file` 에 `{"SMTP_HOST": "x$(touch ./PWNED)"}` 를 주면 `.env` 에 그 값이
  그대로 들어가고, `bash -c 'set -a; source ./.env'` 가 `./PWNED` 를 만듭니다. `;`·`` ` ``·
  `&&`·`|`·`>` 도 같은 근거로 위험합니다.
- `[INFRA-044]` 가 넣은 개행·캐리지리턴 검사로는 막히지 않습니다. **개행이 전혀 필요
  없습니다.** 그 사이클이 닫은 것은 목록 밖의 새 `KEY=VALUE` 줄을 끼워 넣는 경로뿐입니다.
- 심각도는 `[INFRA-044]` 가 고친 것보다 큽니다. 설정 편집 권한이 스크립트를 실행하는 OS
  계정의 임의 코드 실행으로 승격됩니다. 전제는 `POST /api/system/env` 게이트 통과이지만,
  `ADMIN_API_TOKEN` 이 유출되면 원격에서 성립합니다.
- **수정 방향은 `source` 제거입니다.** 보안 리뷰가 세 파서를 실측해 따옴표 방식을 물리쳤고
  근거가 넷입니다. (1) `bash`·python-dotenv·`@next/env` 가 따옴표를 서로 다르게 읽습니다.
  특히 `'\''` 이스케이프에서 python-dotenv 는 **그 줄을 통째로 버립니다.** (2) 문자 필터로
  대체할 수 없습니다. bash 는 **공백 하나만으로도** 뒤 토큰을 명령으로 실행하는데, Google 앱
  비밀번호에는 공백이 들어가고 `EMAIL_RECIPIENTS` 는 `, ` 로 구분합니다. (3) 원인은 값의
  모양이 아니라 데이터 파일을 셸이 코드로 실행하는 것입니다. (4) 두 스크립트가 `.env` 에서
  실제로 쓰는 값은 `FLASK_PORT`·`FRONTEND_PORT`·`FLASK_HOST` 셋뿐이고
  (`restart_all.sh:23,35,81,91`, `stop_all.sh:26,66,67`), Flask 와 Next 는 각자 `.env` 를
  스스로 읽으므로 `set -a` export 는 이미 중복입니다.
- 따옴표 방식은 왕복 경로도 깨뜨립니다. `read_masked_env_vars` 는 78행에서 `=` 로 한 번만
  자르고 따옴표를 벗기지 않으므로, 화면이 받은 `'smtp.gmail.com'` 을 되돌려 저장하면
  `''smtp.gmail.com''` 이 되고 python-dotenv 가 그 줄을 버립니다. 관리자가 알림 테스트를 한 번
  누른 것만으로 다음 기동에서 SMTP 설정이 사라집니다(리뷰 실측).
- 리뷰가 제안한 대체 형태는 `sed -n "s/^$1=//p" .env | tail -1` 로 필요한 세 값만 읽는
  것입니다. `tail -1` 은 중복 줄에서 나중 것이 이기는 세 파서의 동작과 맞습니다.
- **확인할 것 하나**: export 가 사라져도 두 서비스가 정상 기동하는지, 즉 `.env` 의 다른 값을
  스크립트의 export 에만 기대던 소비자가 없는지 실제로 돌려 봅니다.
- `[INFRA-044]` 가 `services/common_env_service.py` 의 값 필터에서 `$(` 를 함께 막아 두었고
  `${VAR}`·`$VAR` 도 같은 정규식이 거릅니다. **그 필터는 설정 화면을 지나는 값만 봅니다.**
  이미 `.env` 에 들어 있는 값, 손으로 적은 값, `persist_market_gate_interval_to_env` 가 쓰는
  값은 그대로 셸을 지나므로 이 항목이 남습니다. 공백도 그 필터가 막지 않습니다.
- QA 시나리오: 값에 `$(...)` 를 넣어 저장한 뒤 `.env` 를 읽어도 명령이 실행되지 않는다
- [x] 쓰기 인용과 기동 스크립트 파싱 가운데 무엇을 고칠지 정함 — 기동 스크립트에서
  `source` 를 걷어낸다. 따옴표 방식은 세 파서 불일치와 왕복 파괴로 물리쳤다
- [x] 세 파서(python-dotenv·bash·Next.js)가 택한 형식을 같게 읽는지 확인 — `FLASK_PORT`·
  `FRONTEND_PORT`·`FLASK_HOST` 셋을 `sed`·python-dotenv·bash 로 읽어 전부 일치를 확인했다
- [x] export 를 없애도 값을 잃지 않는지 확인 — `app` 임포트 추적에서 `load_dotenv()` 이전에
  읽히는 키는 로케일 넷뿐이고, `env -i` 로 띄운 `@next/env` 가 `.env` 키 58개를 채운다
- [x] 계획 검토 (`oh-my-claudecode:critic`) — 판정 `REVISE`. 지적 일곱을 전부 반영했다.
  (1) 「sed 가 세 파서와 같은 값을 준다」가 거짓이라 세 키의 현재 값 모양으로 한정하고
  한계 표·주석·검사 둘을 더했다. (2) 포트가 오염되면 `stop_all.sh` 가 포트를 못 죽인 채
  「✅ Port freed」를 출력하는 것을 재현해 `env_port` 숫자 검사를 넣었다. (3) TODO 의
  「실제로 돌려 본다」를 충족하는 Step 둘을 만들고 `NOTIFICATION_ENABLED=false` 를 함께
  세우게 했다. (4) `VAR=$(env_value VAR)` 가 상속 환경 변수를 덮어써 우선순위가 깨지는
  것을 `${!1}` 로 막고 세 경우를 재는 검사를 더했다. (5) `source scripts/env_value.sh` 에
  실패 시 종료를 붙였다(두 스크립트에 `set -e` 가 없다). (6) 회귀 검사를 정규식으로 바꿔
  `. .env`·`source "$PROJECT_ROOT/.env"` 도 잡고 `scripts/env_value.sh` 를 대상에 넣었다.
  (7) 돌연변이 검증이 파일 전체 실행 시 운영 `.env` 를 실행하는 것을 막고 QA 문서 작성
  Step 을 추가했다. 미반영 없음
- [x] `scripts/env_value.sh` 신규(52줄) + 두 스크립트에서 `source .env` 제거.
  포트는 `env_port` 가 숫자만 받고, 값 읽기는 `env_value` 가 셸 해석 없이 한다
- [x] 명령 치환·백틱·세미콜론이 실행되지 않는 것을 확인하는 검사 추가 —
  `tests/scripts/test_env_value_sh.py` 8건. 돌연변이 사본으로 각 검사가 실제 결함을 잡는
  것을 확인했다(옛 방식은 `pwned_subst` 를 만들고, `case` 를 지우면 오염된 포트가 통과한다)
- [x] `/ponytail-review` — 두 줄 축약을 지적해 반영했다(중복 조건 검사 한 줄 삭제,
  폴백 두 줄을 `${value:-${!1:-$2}}` 한 줄로). 주석 32줄은 각 문장이 서로 다른 근거를
  담고 `ponytail:` 마커는 규칙 자체가 요구하므로 유지했다
- [x] `feature-dev:code-reviewer` — 지적 둘을 직접 재현한 뒤 반영했다. (1) `${!1}` 간접
  확장이 함수의 `local value` 와 이름이 겹쳐 상속값이 무시되는 것을 재현하고 지역 변수를
  `_env_port_value` 로 바꾸었다. (2) 확신도 낮음으로 온 「pytest 어서트가 값을 흘린다」가
  실재해(리터럴 3회 등장) 파서 일치 검사를 불리언 단언으로 바꾸었다. 미반영 하나: 「읽기
  권한이 없으면 조용히 빈 값」은 `[ -r ]` 로 바꾸면 `sed` 가 stderr 에 내던 오류마저
  사라져 더 조용해지므로 지금 형태를 유지한다
- [x] `/review`(T3 심층) — 0 critical / 2 informational, 고칠 것 없음. `Shell Injection`
  범주에 새 주입 경로가 없음을 확인했다(`sed` 표현식의 `$1` 은 재평가되지 않고,
  `printf '%s'` 가 형식 문자열 주입을 막으며, `case` 의 인용 생략은 단어 분리도 글롭
  확장도 일으키지 않는다 — 실측). 문서 staleness 없음. 남긴 관찰 둘: 파서 일치 검사가
  `.env` 부재 시 skip 되나 `.github/workflows` 가 없어 현재 실효 없음, shellcheck 미설치라
  정적 검사를 `bash -n` 으로 대체함
- [x] `oh-my-claudecode:security-reviewer`(시크릿 접촉) — 지적 셋을 직접 재현한 뒤 반영했다.
  (A) **회귀 검사가 자기가 걷어낸 옛 한 줄 표기를 잡지 못했다.** `^\s*` 앵커 때문에
  `[ -f .env ] && { …; set -a; source .env; set +a; }` 를 놓쳤다. 검사가 통과하면서도
  아무것도 막지 못하는 상태였다. 앵커를 셸의 명령 시작 자리로 바꾸고 주석을 걷어냈다.
  **리뷰가 준 형태(행 앞 앵커 제거 + 주석 제거)는 그대로는 오탐이 났다** — 오류 메시지의
  `다. .env` 가 걸렸는데 리뷰는 문자열 리터럴에 `#` 이 없는 것만 확인했다. (B) `${!1}`
  간접 확장이 이름 안의 배열 첨자를 산술 평가해 `a[$(명령)]` 으로 명령이 실행되는 것을
  bash 3.2·5.x 양쪽에서 확인했고, (C) GNU sed 의 `s///e` 주입과 함께 키 이름 검사 한
  자리로 막았다. 덧붙여 `dotenv_values` 반환값을 지역 변수로 붙들면 `pytest -l` 이 `.env`
  전부를 덤프한다는 지적도 반영해 임시 객체로 바꾸었다. 시크릿 확인 세 가지는 리뷰의
  대조 결과가 제 확인과 일치했다. 미반영 없음
- [x] 시크릿 확인 세 가지 통과. 추적되는 `.env*` 는 `.env.example` 뿐, 새 코드가 값을
  출력하지 않음, `.env` 에 `NEXT_PUBLIC_` 접두 변수 없음. **유출면은 오히려 줄었다** —
  옛 `set -a` 는 `.env` 전체를 gunicorn 과 npm 자식 프로세스 환경으로 넘겼다
- [x] 검증: `pytest` 1809 통과·2 skip(exit 0), `bash -n` 셋 통과, 임시 포트 5599 에
  `env -i` 로 띄운 gunicorn 이 `/health` 200(오류 0줄), `@next/env` 가 다섯 키를 채움,
  세 키 값이 옛 방식과 일치. QA 2단계 필수 18/18 통과
- [x] 범위 밖 발견 둘을 이월: `[INFRA-053]`(`.env` 모드 0644),
  `[INFRA-054]`(`pkill` 다중 패턴이 통째로 무동작)
...
- [ ] 세 라우트에 요구할 권한 수준을 정함. 기존 게이트 둘의 판정 방식이 다르므로 어느
  쪽을 표준으로 삼을지 함께 정한다
- [ ] 두 언어의 관리자 판정 구현이 같은 입력에 같은 답을 내는지 재는 검사 추가
- [ ] 관리자를 `ADMIN_EMAILS` 에서 지웠을 때 실제로 무엇이 닫히는지 문서화하고
  `ADMIN_API_TOKEN` 회전 절차를 함께 적음
