...
- 설계 승인: 승인 일자 2026-09-24 | 승인 확인 시각 2026-09-24 00:49 | bounded, 사용자 「진행해 함께고쳐」(`.toml` 도 함께 고침). 검토 기준 본문은 `.md` 에만 두고 `.toml` 은 description 과 「볼 것」을 가리키는 한 문장만 고친다
- [x] 설계 승인(bounded) - [x] 문서 수정(`.md` +11 -1, `.toml` +2 -2) - [x] §5 검토(참조 `[INFRA-047]`·`--threads 8` 실재, `git diff --stat` 실행 코드 0, 「쓰는 곳 없음」 주장은 `git grep` 으로 `docs/`·`tests/` 밖 `vendor/pykrx/rebuild.py` 빌드 도구 하나만 확인) - [x] `test_skill_set.py` 6 passed(exit 0)
