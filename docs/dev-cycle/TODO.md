...
- 확인 수준: 키 공유는 탐침으로 확인, 나머지는 코드로만. 장중·17시의 KRX·Toss 응답(당일 행이 부분 값으로 오는지, 아예 없는지)을 확인하지 않았다
- 설계 승인: 승인 일자 2026-09-26 | 승인 확인 시각 2026-09-26 00:25 | 범위: `[FLOW-031]`·`[JONGGA-044]` 와 묶음(T3). `_get_reference_trend_cached` 는 키 날짜가 오늘보다 이전일 때만 메모리·SQLite 에 둔다. 오늘 키 값은 조회한 호출과 기다린 호출에만 돌려주고 실패 캐시에도 넣지 않는다((1)(2)(3) 해소, Toss 는 늘 오늘 키라 저장하지 않음). (4) 공표 전 60초 재조회는 유지(공표 값을 받으려면 필요, 60초 실패 캐시·상세 15분 슬롯으로 빈도 제한). 참조 SQLite 서명 `v3`. 장중·17시 응답 확인은 두 경우 모두 맞게 동작하는 설계라 하지 않는다(9/8·9/9 로컬 로그의 17:01 Toss 백필이 17시 KRX 당일 자료 부재 근거) | 실제 대화 근거: 2026-09-26 사용자 「FLOW-030 FLOW-031 JONGGA-044 한번에 진행해줘」 뒤 설계 제시, `/effort high` + 「승인 A로 해」 응답
- 계획: `docs/superpowers/plans/2026-09-26-flow-030-031-jongga-044.md`
- [x] 계획 검토(critic): `ACCEPT-WITH-RESERVATIONS`. 반영 — M1 추세 맵 SQLite 접미사를 `unified_v3` 로 올리고 v2 스냅숏 비읽기 검사 추가, m2 목록 휴장일(2/16~18) 매개변수 추가, m3 탐침 산물 판단 기준을 계획에 적음, m1 스크리너 과거 기준일 배치의 pykrx 조회 비용은 아카이브 메모로 기록(운영 코드를 서비스 한 파일로 한정해 screener docstring 은 고치지 않음). 미반영 — n1 대기자 경로 검사는 코드 경로가 바뀌지 않아 생략, i1 자정 경계·i2 UTC 는 보수적 동작이라 조치 없음, 명령 Nit 은 `npm run test -- <경로>` 로 따름
- [x] 구현과 테스트: FLOW-030 새 검사 RED 3건(두 번째 호출 `500 == 1000`, Toss `len(calls) 1 == 2`, `pykrx:v2` 행의 `4995`) → GREEN 4건. 평일(2026-09-23 10:00) 고정 탐침에서 `test_get_investor_trend_5day_for_ticker_reuses_reference_sqlite_after_memory_clear` 1건이 최신 창 키=오늘로 실패해 최근 거래일을 2026-02-24 로 고정(기대값 그대로), 탐침 재실행 55 passed 후 제거. FLOW-031 RED 3건(`True is False`, CSV 값 반환, v2 접미사 스냅숏 재사용) → GREEN 6건(목록 휴장일 매개변수 포함), `tests/services tests/engine -k "trend or supply or screener"` 174 passed. JONGGA-044 는 `beforeEach` 에 23:58 을 임시로 넣어 1 failed(`UPDATED` 없음) 재현, 정오 고정 줄을 더해 5 passed, 임시 줄 제거 뒤 5 passed
- [x] `/ponytail-review`: `Lean already. Ship.`(운영 코드 +38 −14, 오늘 키 조건은 실패 캐시 `elif` 로 떨어지지 않게 중첩이 필요)
- [x] `closing-bet-reviewer`: `APPROVE`, 최대 medium(검토 해시 전체 `de6cbe3b5191`·서비스 `366e325bd624`, 평일 탐침 `TZ=Pacific/Pago_Pago` 로 전체 2871 passed·2 skipped). 반영 — F1 medium 부분 수집일 기준일에서 맵 전체를 버리는 사례는 사용자 선택으로 기준일 호출의 끝날 트리밍을 없앰(새 검사 `test_a_target_run_keeps_a_partially_collected_target_day` RED `[] == ['005930']` → GREEN), F2 docstring 을 「최근 거래일이 오늘로 확정된 최신 창」과 「실패·거부는 오늘 키도 60초 실패 캐시」로 정정, F3 오늘 키 실패의 60초 캐시 검사 추가(변이 `elif fatal is None and target_key < today` 가 `2 == 1` 로 실패함을 확인 후 원복). info — F4 비용 상한(상세 15분 슬롯·60초 실패 캐시, 과거 기준일 배치는 1회 뒤 캐시)은 아카이브 메모, F5 자정·UTC 는 보수적 동작, F6 기준일 실행에서 `stale_csv` 가 사실상 붙지 않음은 동작 결함이 아니라 조치 없음. 반영 뒤 서비스 diff `29137aa3494d`(+45 −15), 관련 범위 248 passed
- [x] `/review`(`oh-my-claudecode:code-reviewer`, opus): `APPROVE`, Critical·Important 0(검토 해시 서비스 `29137aa3494d` 시작·종료 동일, 변이 13건 모두 잡힘, 평일 TZ 탐침 76 passed). 반영 — M-1 휴장일 목록 보충(위 범위 변경 승인, `test_market_schedule_fallback_holidays_cover_known_2026_cases` 에 9일 추가 RED `2026-06-03 True is False` → GREEN)과 서비스 주석 전제를 「pykrx 로 확정한 기준일」로 정정, M-2 비거래일 기준일은 참조 끝 날짜 대조에서 늘 None 이라는 점을 주석에 적고 아카이브 메모로 기록, N-1 docstring 에 기다린 호출만 같은 값을 받는다는 점 추가, N-2 저장 판정의 오늘 날짜를 조회 시작 시점으로 옮김, N-3 빈 줄 PEP 8 정리. 반영 뒤 서비스 diff `a63ba3c6af5f`(+49 −15), `engine/market_schedule.py` +11, 관련 두 파일 73 passed, `git diff --check` 통과. 휴장일 목록과 N-2 는 리뷰 뒤 변경이라 리뷰어가 보지 않았다(검사로만 확인)
- [x] 정적 검증(09-26 01:14~01:16): `KRX_ID= KRX_PW= venv/bin/python -m pytest -q -p no:cacheprovider` 2873 passed·2 skipped(exit 0, 「KRX 로그인」 0회) · `(cd frontend && npx vitest run)` 104 files·701 passed(exit 0) · `npm run type-check` exit 0 · `npm run lint` exit 0(오류 0, 경고 183 중 변경 파일 0) · `git diff --check` exit 0. QA 시나리오 계획 `docs/dev-cycle/qa/FLOW-030.md`(필수 S-1~S-3·H-1~H-5, JONGGA-044 는 테스트만 바꿔 동적 QA 제외) · 첫 커밋
- [ ] QA: CLI 하네스 + 브라우저(`docs/dev-cycle/qa/FLOW-030.md`)

### [FLOW-031] 기준일 실행이 pykrx 를 못 쓰면 CSV 폴백이 기준일 행 없는 전날 끝 5거래일을 플래그 없이 쓴다
- 카테고리: 수급·백테스트 | 티어: T3(위험 경로 `services/investor_trend_5day_service.py`) | 근거: `[FLOW-029]` 묶음 리뷰(`closing-bet-reviewer` F1, 2026-09-25)
- 내용: 17시 기준일 D 실행에서 pykrx 표가 D-1 로 끝나 `[FLOW-029]` 로 거부되면 `get_supply_data` 는 통합 서비스로 넘어간다(`engine/collectors/krx_local_data_mixin.py:1011-1015`). 수급 CSV 도 같은 KRX 원천(`scripts/init_data.py:1283`·`:1287`)이라 D 행이 없으면 `_build_trend_map` 이 `date <= D` 로 걸러 D-1 로 끝나는 5거래일을 만들고(`services/investor_trend_5day_service.py:389-390`), 영업일 간격이 1 이라 `stale_csv`(`:601-609`, 4영업일 초과)도 붙지 않아 그 값이 기준일의 5일 값으로 채택된다. 참조 캐시의 영구 저장은 `[FLOW-029]` 로 막았지만 그 실행이 전날 창을 쓰는 일은 남는다. 회귀는 아니다
- 확인 수준: 코드로만 확인(리뷰어 판독). 17시 시점에 D 행이 CSV·pykrx 에 없는 빈도는 확인하지 않았다
- 설계 승인: 승인 일자 2026-09-26 | 승인 확인 시각 2026-09-26 00:25 | 범위: 안 A(버림). 기준일을 준 `_build_trend_map` 에서 CSV 창 마지막 날 다음날부터 기준일까지 영업일(주말·`known_holidays` 제외)이 하루라도 비면 빈 맵과 warning 로그. 종목마다 `missing_csv` 로 pykrx 기준일 참조, 없으면 None. 그날 종가베팅은 수급 결측으로 S·A·B 가 없고 「신호 없음」 알림이 나갈 수 있음을 설명함 | 실제 대화 근거: `[FLOW-030]` 과 같은 응답 「승인 A로 해」
- 범위 변경 승인: 2026-09-26 00:5x AskUserQuestion 응답 「당일 끝날 유지 (권장)」. `closing-bet-reviewer` F1(기준일 D 가 부분 수집일이면 `recent_trading_dates` 가 D 를 빼 창이 D-1 로 끝나고 맵 전체를 버림)에 따라, 기준일을 준 호출은 끝쪽 부분 수집일을 빼지 않고 CSV 의 마지막 5개 날짜(≤ 기준일)를 창으로 쓴다. 그날 행이 없는 종목만 빠진다. 최신 창은 `[FLOW-025]` 트리밍 유지
- 범위 변경 승인: 2026-09-26 01:0x AskUserQuestion 응답 「이번에 채움 (권장)」. `/review` M-1(휴장일 목록이 2026-05-25 까지만 있어 pykrx 실패 시 목록에 없는 휴장일 기준일에서 맵 전체를 버림)에 따라 `engine/market_schedule.py` 의 `_fallback_holidays` 에 2026 하반기 9일 추가. 6/3·7/17·8/17 은 로컬 `data/daily_prices.csv` 에 거래가 없던 평일(읽기 전용 확인), 9/24·9/25·10/5·10/9·12/25·12/31 은 공휴일 달력(KRX 공고와 대조하지 않음)
- 진행 체크는 `[FLOW-030]` 에 함께 기록

### [JONGGA-044] 종가베팅 화면의 `[JONGGA-037]` 회귀 검사가 KST 자정 전 약 6분 동안 실패한다
- 카테고리: 종가베팅 | 티어: T1(테스트 파일만) | 근거: `[FLOW-029]` 정적 검증(2026-09-25 23:5x 실패 3회 재현, 09-26 00:01 통과)
- 내용: `frontend/src/app/dashboard/kr/closing-bet/page.regression-jongga-037.test.tsx` 의 「안전 timeout 뒤 늦은 상태 응답은 화면이나 polling을 되살리지 않는다」는 `vi.useFakeTimers()` 로 `Date` 까지 가짜로 바꾼 뒤 350초를 앞당긴다. 고정 자료의 날짜는 시작 시각의 KST 오늘이라, 23:54 이후에 돌면 앞당긴 시각이 다음 날이 되어 `page.tsx:1946` 이 「OLD DATA」를 그리고 `UPDATED` 단언이 실패한다
- 설계 승인: 승인 일자 2026-09-26 | 승인 확인 시각 2026-09-26 00:25 | 범위: `beforeEach` 에서 `vi.setSystemTime(new Date(TODAY()))` 로 KST 정오 고정(vitest 4.1.11 소스로 `useFakeTimers` 가 고정 시각에서 시작하고 `useRealTimers` 가 되돌림을 확인) | 실제 대화 근거: `[FLOW-030]` 과 같은 응답
- 진행 체크는 `[FLOW-030]` 에 함께 기록
