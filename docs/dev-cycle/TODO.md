...
- 과거 날짜를 고르면 일괄 매수 버튼이 비활성으로 바뀌고 "최신(오늘) VCP 시그널
  탭에서만 일괄 매수를 실행할 수 있습니다." 라고 사유를 밝힙니다. 그런데 같은 화면의
  행별 매수 버튼 여섯 개는 `disabled=false` 입니다. 개별 주문을 여섯 번 누르면 일괄
  매수가 막으려던 결과와 같아지므로, 한쪽 경로에만 가드가 걸린 상태입니다.
- 행별 버튼에는 툴팁도 없어서 왜 어떤 버튼은 눌리고 어떤 버튼은 눌리지 않는지 화면만
  보고는 알 수 없습니다.
- [ ] 행별 매수 버튼에도 같은 날짜 조건을 적용하고 사유를 툴팁으로 표시
- [ ] 과거 날짜에서 두 경로가 모두 막히는지 확인하는 회귀 검사 추가
