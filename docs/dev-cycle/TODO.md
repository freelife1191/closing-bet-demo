...
### [CHAT-042] 챗봇 스트림 청크가 끝에 한꺼번에 나타난다
- 카테고리: 챗봇 | 티어: T2(설계 때 재판정) | 근거: `[INFRA-080]` QA S-3(2026-09-23 22:28). 격리 사본(Next dev 58120, gunicorn 58121, 1초 간격 다섯 청크를 내는 가짜 genai 클라이언트)에서 「안녕」 을 보내고 페이지 안 MutationObserver 로 청크 수를 재니, 다섯 청크가 전송 뒤 약 5.1초에 한꺼번에 나타났다(`[[70,5],[5141,10]]`). gunicorn keep-alive 기본값(2초)으로도 `[[83,0],[5122,5]]` 로 같아 `[INFRA-080]` 과 무관하다. `[CHAT-037]` QA S-3(3초 간격 열 청크)에서는 「중단」 전에 `조각0` 부터 보였으므로 항상 모이는 것은 아니다. 버퍼링 지점(Flask 응답·Next dev rewrite 프록시·프론트엔드 렌더) 가운데 어디인지 확인하지 않았다. 운영(Next prod·Caddy)에서도 같은지도 모른다
- 범위: 먼저 지점을 가른다(Flask 직접·Next 경유 SSE 도착 시각, 브라우저 렌더 시각). SSE 경로는 익명 세션과 `Accept: text/event-stream` 이 필요하다(`app/routes/kr_market_chatbot_http_routes.py` 의 `legacy_sync_mode`). 원인에 따라 설계한다
- [ ] 지점 확정 - [ ] 설계 승인(bounded) - [ ] 구현·검증 - [ ] QA
