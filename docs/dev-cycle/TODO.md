...
- 카테고리: 프론트엔드 공통 | 티어: T2 | 근거: 2026-09-03 FLOW-004 사이클의 `/qa-only` 실측
...
- 설계 승인: 승인 일자 2026-09-09 | 승인 확인 시각 2026-09-09T17:48+09:00
  | 범위: FE-022·FE-039 사용자 표시 통일·저장 직무 복원, 회귀 검사와 격리 브라우저 QA
  | 실제 대화 근거: 앞선 동일 묶음 제안 뒤 사용자 「다음 라운드 … 묶어서 … 쭉 진행」 요청. bounded 설계 재개.
- [x] 공통 프로필 기본값·갱신 흐름 및 직무 표시 회귀 수정: 8파일, 대상 27검사 통과
- [x] ponytail APPROVE → 독립 code-reviewer APPROVE·architect CLEAR. 증거 `evidence/profile-batch-20260909/`, review-input SHA 8파일 일치. 기존 비작성 에이전트에 설치 역할 프롬프트 적용; 새 전용 역할 호출과 구분.
- [ ] pytest·vitest 전체, typecheck·lint 및 UltraQA agent-browser 실측 후 마감 (라운드 90분 상한)
...
- 카테고리: 프론트엔드 공통 | 티어: T2 | 근거: 2026-09-06 `[CHAT-026]` 사이클의 `/qa-only` ISSUE-003
