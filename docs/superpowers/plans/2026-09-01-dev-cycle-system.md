...
**실행 중 발견**: 프로젝트 로컬 스킬은 세션이 시작될 때 스캔되므로, 스킬을 만든 바로 그
세션에서는 `/dev-cycle` 이 인식되지 않는다. 같은 세션에서 검증하려면 `SKILL.md` 의 [0]
준비 절차를 손으로 따라 수행한다. 이 사실을 CLAUDE.md 의 dev-cycle 섹션에 적어 두었다.
