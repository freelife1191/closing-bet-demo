...
async def test_sync_to_thread_hang():
    print("--- [테스트 1] 동기 클라이언트, 타임아웃 11초 (Vertex AI) ---")

    timeout_sec = 11.0
    client = build_genai_client(http_options={"timeout": timeout_sec})
    ...
    def _call_gemini():
        ...
        try:
            ...
            print(
                f"[{time.strftime('%H:%M:%S')}] _call_gemini 스레드 내부 예외 발생: "
                f"{type(e).__name__} - {e}"
            )
            raise
...
async def test_async_client():
    print("--- [테스트 2] 비동기 클라이언트, timeout=11.0초 (Vertex AI) ---")
    timeout_sec = 11.0
    client = build_genai_client(http_options={"timeout": timeout_sec})
