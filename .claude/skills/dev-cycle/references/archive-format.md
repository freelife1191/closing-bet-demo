...
- **완료** 줄에는 시각, 티어, 커밋 해시를 적는다. 커밋이 여럿이면 쉼표로 잇는다.
  `/qa` 가 시나리오 실패를 고치며 만든 커밋도 이 사이클의 커밋이므로 함께 적는다.
...
- **리뷰** 줄에는 이 사이클에서 실제로 읽거나 돌린 스킬과 에이전트를 모두 적는다.
  `tier-rules.md` §1 표의 리뷰 열에 있는 것뿐 아니라 [1] 계획에서 고르고 [2] 구현에서
...
- **검증** 줄에는 실행한 테스트와 그 결과를 적는다. QA 시나리오는 `통과/전체` 형태로
  ...
  세는 방법은 다음과 같다. 실패했다가 같은 사이클에서 고쳐 다시 통과한 시나리오는 통과로
  세되 괄호로 그 사실을 밝힌다. `QA 시나리오 7/7 통과 (S-4 실패 → 고침)` 처럼 적는다.
  한 번에 통과한 것과 고쳐서 통과한 것을 구분하지 않으면, 사이클이 결함을 하나 남겼다가
  잡았다는 사실이 기록에서 사라진다. `실패 → 이월` 로 끝난 시나리오는 통과로 세지 않고
  `QA 시나리오 6/7 통과 (S-4 이월 → [VCP-012])` 처럼 이월한 항목 ID 를 함께 적는다.
...
`docs/dev-cycle/qa/<항목 ID>.md` 에 사이클마다 하나씩 만든다. 구성은 [3] 검증의 4번에서
하고 실행 결과는 6번을 마친 뒤 7번에서 같은 파일에 덧붙인다. 두 단계를 한 파일에 두는 이유는,
