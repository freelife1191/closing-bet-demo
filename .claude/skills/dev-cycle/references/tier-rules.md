...
| 티어 | 조건 | 리뷰 | 검증 |
|---|---|---|---|
| T1 | 변경 50줄 이하이고 위험 경로에 닿지 않는다 | `/ponytail-review` | 변경 범위의 pytest 또는 vitest |
| T2 | 변경 300줄 이하이고 위험 경로에 닿지 않는다 | `/ponytail-review` → `/code-review` | pytest·vitest 전체. 화면이 바뀌면 agent-browser 로 실측한다 |
| T3 | 변경 300줄을 초과하거나 위험 경로에 닿는다 | T2 전체 + `/review` | pytest·vitest 전체. 화면이 바뀌면 `/qa-only` 로 시나리오를 돌린다 |
...
검증에 `qa` 스킬을 쓰지 않는다. 이 스킬은 찾은 버그를 스스로 고쳐서 커밋하므로 [4] 마감의
게이트 2 를 우회한다. 리포트만 내는 `/qa-only` 를 쓰고, 지적 사항의 반영은 사이클의
[2] 구현으로 되돌려 처리한다.

화면이 바뀌지 않는 항목에는 실측과 시나리오를 요구하지 않는다. 백엔드만 바꾸는 T2 나 T3
에는 열어 볼 화면이 없다. 그 자리는 pytest 전체 통과와, 바뀐 값을 실제로 만들어 내는
경로를 직접 호출해 얻은 결과로 대신한다.
...
지금 존재하는지와 무관하게 T3 이며, §1 마지막에 적은 시크릿 확인 세 가지를 검증에 더한다.
