...
저장소에 커밋된 역할 `closing-bet-reviewer`(`.claude/agents/closing-bet-reviewer.md`)다.
2026-09-22 까지는 `feature-dev:code-reviewer` 플러그인 에이전트였고, 그 역할이 현재 세션의
에이전트 목록에 없을 때만 지금도 그것으로 대체하고 대체한 사실을 기록한다. 역할에 주는
입력(항목 ID, 승인 범위, 변경 파일 목록, 기준 커밋, 실행한 검증, QA 문서 경로)은 역할
정의의 「입력」 절이 정한다. 어느 쪽을 부르든 **확신도가 낮은 관찰도 확신도를 밝혀 함께
적어 달라고 요청한다.** 역할 정의에는 그 요청이 들어 있고, 플러그인 에이전트는 확신도로
지적을 걸러 내므로 프롬프트에 적어야 한다. 낮은 확신도로 분류된 관찰이 실제 결함이었던
사이클이 2026-09-03 에만 두 번 있었다.

아래 두 문단은 Claude Code 에서 그 역할을 띄울 때의 규칙이다. codex 에서는
`$code-review` 스킬이 그 자리를 대신하므로 해당하지 않는다. `.codex/agents/` 의 전용 호출을
새 실행 문맥에서 확인하면 `codex-setup.md` §13 에 적고 실행 환경 표의 Codex 열을 바꾼다.
낮은 확신도의 관찰도 확신도를 밝혀 보고하라는 앞 문단의 요청은 codex 리뷰에도 적용한다.
