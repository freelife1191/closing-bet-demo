...
**리뷰 열과 §1-1 이 쓰는 도구 이름은 Claude Code 의 것이다.** codex 에서는 이름이 다르고
하나는 대응하는 스킬이 없다. 대응표는 `SKILL.md` 의 `## 실행 환경` 에 있으며, 지금 도는
환경의 이름으로 바꿔 부른다. 판정 기준과 순서는 어느 환경에서든 이 문서의 것을 따른다.
...
아래 두 문단은 Claude Code 에서 그 에이전트를 띄울 때의 규칙이다. codex 에서는 `/code-review`
스킬이 그 자리를 대신하므로 해당하지 않는다.
...
1. **회귀 시나리오** — 이번 사이클이 고친 결함의 재현 절차와, 고쳐졌다면 나와야 할 기대값.
   ...
   **그 줄이 없는 항목이 백로그의 대부분이다.** 2026-09-01 감사가 채운 항목들은 이 줄을
   규정하기 전에 만들어졌다. 없으면 그 항목의 체크박스 목록과 「근거」에 적힌 감사 절을
   읽어 회귀 시나리오를 직접 만든다. 줄이 없다는 이유로 회귀 시나리오를 빼지 않는다. 그
   줄은 출발점을 아껴 주는 장치일 뿐이고, 무엇을 검사할지 정하는 것은 이번 사이클이
   무엇을 고쳤는가다.
...
1단계를 부르기 직전에 확인한다. 두 단계가 같은 서버를 쓰고 그 사이를 끊지 않으므로
2단계에서 다시 확인할 필요는 없다.
...
읽는다. 도구의 명령 이름과 탭 전환 방식과 콘솔 버퍼의 성질처럼, 모르면 판정을 그르치는
것들이 거기 모여 있다.
