...
**Next.js 16.3 이 문턱이다.** 아래 표에서 문턱이 붙은 네 스킬은 16.3 미만에서 실행을
거부하도록 만들어져 있다. 우회하지 않는다. 업그레이드는 `[FE-002]` 항목이 담당하며,
그 항목이 끝나기 전까지 해당 스킬을 호출하지 않는다.
...
`tier-rules.md` §1 의 검증 열은 `T2` 와 `T3` 에서 agent-browser 실측을 요구한다.
`frontend/` 를 건드리는 항목에서는 다음 규칙이 그 위에 얹힌다.
