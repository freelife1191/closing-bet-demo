...
고르는 자리는 `SKILL.md` [1] 설계와 계획의 3번이다. 건드릴 파일 목록을 뽑고 티어를
판정하는 자리이므로 아래 판정에 필요한 것을 그 자리에서 이미 보고 있다.
...
계기도 없기 때문이다. 그래서 `SKILL.md` [1] 설계와 계획 3번에서 고른 문서와 스킬을
6번의 범위·스킬 보고에 포함한다. 프론트엔드를 건드리는 사이클의 보고에 문서 이름이 없으면
건너뛴 것이고, 보고가 곧 점검이 된다.
...
- QA 2단계는 이것과 별개로 그대로 돌린다. 보는 것이 다르기 때문이다. 이 절은 개발자가
  ...
  Codex에서도 UltraQA가 agent-browser를 대체하지 않으며, agent-browser는 계속 브라우저
  실측 도구다. QA 메타데이터와 결과·증거·정리는 `archive-format.md` §8에 남긴다.
