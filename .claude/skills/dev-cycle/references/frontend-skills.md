...
`/qa` 로 실행)를 요구하고, 화면이 바뀌는 `T2` 와 `T3` 에서는 agent-browser 로 값을
대조하도록 한다. `frontend/` 를 건드리는 항목에서는 다음 규칙이 그 위에 얹힌다.
...
- `next-dev-loop` 이 대체하는 것은 agent-browser 쪽 실측이며 QA 2단계는 그대로 돌린다.
  보는 것이 다르기 때문이다. 앞의 것은 개발자가 지목한 화면을 프레임워크와 함께
  들여다보고, 뒤의 것은 사용자의 눈으로 확정한 시나리오를 실행한다.
- `next-dev-loop` 으로 실측한 값이 곧 시나리오의 기대값이 된다. 실측에서 읽은 건수와
  비율과 배지 문구를 그대로 `docs/dev-cycle/qa/<ID>.md` 의 `기대` 줄에 옮겨 적으면,
  같은 값을 두 번 조사하지 않아도 된다.
