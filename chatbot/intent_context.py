...
def build_watchlist_context_bundle(
    ...
    contains_any_keyword: Callable[[str, List[str]], bool],
    build_watchlist_detailed_context: Callable[[List[str]], str],
) -> Tuple[str, str]:
    """관심종목 상세 요청이면 상세 컨텍스트와 의도 지시문 오버라이드를 생성한다."""
    if not watchlist or not contains_any_keyword(user_message, _WATCHLIST_DETAIL_KEYWORDS):
        return "", ""

    context = build_watchlist_detailed_context(watchlist)
...
def build_additional_context(
    ...
    build_watchlist_context_bundle_fn: Callable[[str, Optional[list]], Tuple[str, str]],
) -> Tuple[str, str]:
