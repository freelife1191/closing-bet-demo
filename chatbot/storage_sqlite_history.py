...
def _load_existing_message_count_cursor(
    *,
    cursor: sqlite3.Cursor,
    session_id: str,
) -> int:
...
def _load_message_row_cursor(
    *,
    ...
    row = cursor.execute(
        """
        SELECT role, parts_json, timestamp
        FROM chatbot_messages
        WHERE session_id = ?
          AND message_index = ?
        LIMIT 1
        """,
        (session_id, int(message_index)),
    ).fetchone()
    if not row:
        return None
...
def _is_append_only_message_update(
    *,
    ...
    existing_count = _load_existing_message_count_cursor(
        cursor=cursor,
        session_id=session_id,
    )
    if existing_count == 0:
    ...
    existing_last = _load_message_row_cursor(
        cursor=cursor,
        session_id=session_id,
    ...
    return existing_last == (last_role, last_parts_json, last_timestamp), existing_count
...
def clear_history_sessions_in_sqlite(
    ...
    try:
        with connect_sqlite(str(db_path), timeout_seconds=30, pragmas=_SQLITE_PRAGMAS) as conn:
            ...
            conn.execute("DELETE FROM chatbot_sessions")
            conn.commit()
        return True
    except Exception as error:
