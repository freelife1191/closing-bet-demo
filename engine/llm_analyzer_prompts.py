...
4. reason: 다음 5개 섹션을 **모두** 포함하여 **전체 최소 350자 이상**, 각 섹션 **최소 60자 이상**으로 구체적으로 작성하세요.
   ...
   ③ 수급 동향: 외인/기관 순매수 방향성, 매집 강도 해석
   ④ 리스크 요인: 단기 과열도, 변동성, 업종/지수 리스크, 차익 실현 매물 가능성
   ⑤ 매매 전략: 진입(시가·종가·눌림목 중 어느 것), 시스템 손절·목표 기준 준수, 분할/관망 조건 중 최소 한 가지

[출력 규칙 — 매우 중요]
- reason은 숫자 없는 정성 분석만 작성하세요. ①~⑤ 마커 외 아라비아/한글 수치, 금액, 절대가격, 배수, 퍼센트, 점수를 쓰지 마세요. 제품 세대나 기간 숫자도 생략하세요.
- 원자료 수치는 시스템이 검증 후 붙입니다. 임의의 손절/목표 가격을 제안하지 말고 화면의 시스템 계산값을 따르도록 안내하세요. score/confidence JSON 필드에는 기존처럼 숫자를 사용하세요.
...
[
    {{
        ...
        "action": "BUY",
        "confidence": 78,
        "reason": "① 뉴스/재료 분석: 입력된 뉴스와 기업 실적의 연결성을 살피고 실제 수요로 이어질 수 있는지를 검토합니다. 단순한 기대감과 확인된 사실을 구분하며 업종 내 경쟁 상황과 재료의 지속성을 함께 판단해야 합니다. ② 거래대금/거래량 평가: 거래대금과 거래량의 강도를 입력 자료에 근거해 해석합니다. 거래가 집중되는 흐름이 확인되더라도 일시적인 관심인지 지속 가능한 참여인지 구분하며 가격 상승과 거래 흐름의 동반 여부를 살핍니다. ③ 수급 동향: 외국인과 기관의 매매 방향을 입력 자료로 확인합니다. 양쪽의 방향이 일치하는지와 참여가 이어지는지를 살피되 단기적인 수급 변화만으로 향후 상승을 단정하지 않고 추가적인 흐름을 확인합니다. ④ 리스크 요인: 시장 전체의 변동성과 업종의 변화에 유의해야 합니다. 단기적인 상승 이후 차익 실현이 나올 수 있으며 뉴스의 기대가 가격에 이미 반영됐는지 확인하고 예상과 다른 움직임에는 보수적으로 대응합니다. ⑤ 매매 전략: 진입을 결정한다면 분할 접근과 관망 조건을 함께 고려합니다. 임의의 가격을 새로 제시하지 않고 화면의 시스템 손절 및 목표 계산값을 따르며 입력된 전제가 무너지면 기존 판단을 다시 점검합니다."
    }}
]
"""
