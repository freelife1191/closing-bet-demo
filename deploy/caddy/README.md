...
전에 서버에서 `grep -n "import common" /etc/caddy/Caddyfile` 로 누가 쓰는지 봅니다. 저장소에 두는
이유는 하나입니다. 서버에만 있던 설정은 코드가 바뀌어도 따라오지 못하고, 그 어긋남은 장애로만
드러납니다(`deploy/systemd/README.md` 의 유닛 사고가 그 예입니다). 앞으로는 여기를 고친 뒤 서버에
반영합니다.
서버에는 Next dev 서버 시절 `/_next/hmr` 의 `Origin` 을 바꿔 넘기던 `@hmr` 땜질 블록이 남아 있을 수
있는데, 이 기준본에는 없습니다. 운영을 `NEXT_MODE=prod` 로 전환한 뒤 지웁니다(`deploy/systemd/README.md`).
